...
class Logic(h):
 ...
 def refresh(req):
  try:
   ...
   return w
 @E
 def get_show_info_on_home_title(title,daum_id=J):
  try:
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @E
 def get_daum_tv_info(search_name,daum_id=J,on_home=w,force_update=w):
  try:
   ...
   if not force_update:
    if daum_id is not J:
     ...
     if entity.update_time is not J and entity.status==1:
      return entity
   if daum_id is not J:
      ...
      if F(span_tag)==1:
       if item.attrib['data-clip']in entity.episode_list:
        if entity.episode_list[item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=Q(K(entity.episode_list[item.attrib['data-clip']][idx])-K(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if F(entity.episode_list):
     entity.episode_count_one_day=K(T(N(F(items))/F(entity.episode_list)))
