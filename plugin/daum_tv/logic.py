...
class Logic(K):
 ...
 def refresh(req):
  try:
   ...
   Logic.get_daum_tv_info(title,force_update=P)
   return P 
  except z as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=L(entity['series'],key=lambda k:B(k['id']))
   entity['equal_name']=[]
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @D
 def get_daum_tv_info(search_name,daum_id=b,on_home=n,force_update=n):
  try:
   ...
   for item in items:
    try:
      ...
      if d(span_tag)==1:
       if item.attrib['data-clip']in entity.episode_list:
        if entity.episode_list[item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=d(entity.episode_list[item.attrib['data-clip']])-1
     ...
     logger.error(traceback.format_exc())
   try:
    if d(entity.episode_list):
     entity.episode_count_one_day=B(W(E(d(items))/d(entity.episode_list)))
   ...
   entity.save()
   logger.debug('daum tv len(entity.episode_list) : %s %s %s',d(items),d(entity.episode_list),entity.episode_count_one_day)
   return entity 
  except z as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
