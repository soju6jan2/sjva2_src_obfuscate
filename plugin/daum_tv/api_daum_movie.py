...
class MovieSearch(K):
 ...
 def search_movie_web(movie_list,movie_name,movie_year):
  try:
   ...
   data=res.json()
   for index,item in z(data['items']['movie']):
    tmps=item.split('|')
    score=85-(index*5)
  ...
  try:
    ...
    if need_another_search:
     ...
     if tmp is not G:
      ...
      for tag in tag_list:
       ...
       if match:
        ...
        elif match.group(2)==movie_year and first_url is not G:
         first_url='https://search.daum.net/search?%s'%tag.attrib['href']
      ...
      if need_another_search and first_url is not G:
     ...
     if tmp is not G:
      ...
      for tag in tag_list:
       ...
       elif year==movie_year and first_url is not G:
        first_url='https://search.daum.net/search?%s'%tag.attrib['href']
   ...
   log_error('Exception:%s',exception)
   log_error(traceback.format_exc())
  movie_list=r(v(w(movie_list,key=lambda k:k['score'])))
  return movie_list
 @U
