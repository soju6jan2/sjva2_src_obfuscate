...
class MovieSearch(u):
 ...
 def search_movie(movie_name,movie_year):
  try:
   ...
   for index,c in L(movie_name):
    if O['config']['is_py2']:
     if h(u'가')<=h(c)<=h(u'힣'):
      is_include_kor=H
      split_index=-1
     elif h('a')<=h(c.lower())<=h('z'):
      is_include_eng=H
      if split_index==-1:
       split_index=index
     elif h('0')<=h(c.lower())<=h('9')or h(' '):
      pass
     else:
      split_index=-1
    else:
   ...
   if kor is not B:
    ...
    for i in Y(e(tmps)):
     if O['config']['is_py2']:
      if h(u'가')<=h(tmps[i][0])<=h(u'힣')or h('0')<=h(tmps[i][0])<=h('9'):
       pass
      else:
       index=i
       break
 ...
 def get_movie_info_from_home(url):
  try:
   ...
   tmp_year=''
   match=E(r'(?P<year>\d{4})\s제작').search(tmp)
   more={}
   if match:
 ...
 def search_movie_web(movie_list,movie_name,movie_year):
  try:
   url='https://suggest-bar.daum.net/suggest?id=movie&cate=movie&multiple=1&mod=json&code=utf_in_out&q=%s'%(m(movie_name.encode('utf8')))
   from framework.common.daum import headers,session
   from system.logic_site import SystemLogicSite
   ...
   log_error(s())
  try:
   url='https://search.daum.net/search?nil_suggest=btn&w=tot&DA=SBC&q=%s%s'%('%EC%98%81%ED%99%94+',m(movie_name.encode('utf8')))
   ret=MovieSearch.get_movie_info_from_home(url)
   if ret is not B:
    if ret['year']==movie_year:
     score=100
     need_another_search=z
    else:
     score=90
     need_another_search=H
    ...
    if need_another_search:
     ...
     if tmp is not B:
      ...
      first_url=B
      for tag in tag_list:
       match=E(r'(.*?)\((.*?)\)').search(tag.text_content())
       if match:
        ...
        elif match.group(2)==movie_year and first_url is not B:
         first_url='https://search.daum.net/search?%s'%tag.attrib['href']
      ...
      if need_another_search and first_url is not B:
     ...
     if tmp is not B:
      ...
      for tag in tag_list:
       ...
       elif year==movie_year and first_url is not B:
        first_url='https://search.daum.net/search?%s'%tag.attrib['href']
   ...
   log_error('Exception:%s',e)
   log_error(s())
  movie_list=a(d(A(movie_list,key=lambda k:k['score'])))
  return movie_list
 @U
