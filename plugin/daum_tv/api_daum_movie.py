...
class MovieSearch(h):
 ...
 def search_movie(movie_name,movie_year):
  try:
   ...
   for index,c in X(movie_name):
    if app.config['config']['is_py2']:
     if y(u'가')<=y(c)<=y(u'힣'):
      is_include_kor=f
      split_index=-1
     elif y('a')<=y(c.lower())<=y('z'):
      is_include_eng=f
      if split_index==-1:
       split_index=index
     elif y('0')<=y(c.lower())<=y('9')or y(' '):
      pass
     else:
      split_index=-1
    else:
   ...
   if kor is not J:
    ...
    for i in W(F(tmps)):
     if app.config['config']['is_py2']:
      if y(u'가')<=y(tmps[i][0])<=y(u'힣')or y('0')<=y(tmps[i][0])<=y('9'):
       pass
      else:
       index=i
       break
 ...
 def search_movie_web(movie_list,movie_name,movie_year):
  ...
  try:
   ...
   log_error('Exception:%s',e)
   log_error(traceback.format_exc())
  movie_list=P(o(D(movie_list,key=lambda k:k['score'])))
  return movie_list
 @E
