...
class MovieSearch(B):
 ...
 def search_movie(movie_name,movie_year):
  try:
   ...
   split_index=-1
   is_include_kor=e
   for index,c in G(movie_name):
    if app.config['config']['is_py2']:
     if x(u'가')<=x(c)<=x(u'힣'):
      is_include_kor=a
      split_index=-1
     elif x('a')<=x(c.lower())<=x('z'):
      is_include_eng=a
      if split_index==-1:
       split_index=index
     elif x('0')<=x(c.lower())<=x('9')or x(' '):
      pass
     else:
      split_index=-1
    else:
   ...
   if kor is not r:
    ...
    for i in K(W(tmps)):
     if app.config['config']['is_py2']:
      if x(u'가')<=x(tmps[i][0])<=x(u'힣')or x('0')<=x(tmps[i][0])<=x('9'):
       pass
      else:
       index=i
       break
 ...
 def search_movie_web(movie_list,movie_name,movie_year):
  try:
   ...
   data=res.json()
   for index,item in G(data['items']['movie']):
    tmps=item.split('|')
    score=85-(index*5)
  ...
  try:
   ...
   if ret is not r:
    if ret['year']==movie_year:
     score=100
     need_another_search=e
    else:
     score=90
     need_another_search=a
    ...
    if need_another_search:
     ...
     if tmp is not r:
      ...
      for tag in tag_list:
       ...
       if match:
        ...
        elif match.group(2)==movie_year and first_url is not r:
         first_url='https://search.daum.net/search?%s'%tag.attrib['href']
      ...
      if need_another_search and first_url is not r:
     ...
     if tmp is not r:
      ...
      for tag in tag_list:
       ...
       elif year==movie_year and first_url is not r:
        first_url='https://search.daum.net/search?%s'%tag.attrib['href']
   ...
   log_error('Exception:%s',exception)
   log_error(traceback.format_exc())
  movie_list=y(N(o(movie_list,key=lambda k:k['score'])))
  return movie_list
 @I
