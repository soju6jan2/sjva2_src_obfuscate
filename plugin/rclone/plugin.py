...
def proxy(request,url):
 try:
  resp=requests.request(method=request.method,url=url,headers={key:value for(key,value)in request.headers if key!='Host'},data=request.get_data(),cookies=request.cookies,allow_redirects=n)
...
def socketio_callback(cmd,data):
 if sid_list:
  ...
  tmp=json.loads(tmp)
  socketio.emit(cmd,tmp,namespace='/%s'%package_name,broadcast=r)
"""
