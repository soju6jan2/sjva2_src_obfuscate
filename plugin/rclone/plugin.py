...
def detail(sub):
 ...
 if sub=='setting':
  ...
  arg['scheduler']=z(scheduler.is_include(package_name))
  arg['is_running']=z(scheduler.is_running(package_name))
...
def proxy(request,url):
 try:
  resp=requests.request(method=request.method,url=url,headers={key:value for(key,value)in request.headers if key!='Host'},data=request.get_data(),cookies=request.cookies,allow_redirects=Y)
...
def connect():
 try:
  ...
  tmp=D
  if Logic.current_data is not D:
...
def socketio_callback(cmd,data):
 if sid_list:
  ...
  tmp=json.loads(tmp)
  socketio.emit(cmd,tmp,namespace='/%s'%package_name,broadcast=G)
"""
