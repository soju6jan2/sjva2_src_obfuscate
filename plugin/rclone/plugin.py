...
def detail(sub):
 ...
 if sub=='setting':
  ...
  arg['scheduler']=Q(scheduler.is_include(package_name))
  arg['is_running']=Q(scheduler.is_running(package_name))
...
def connect():
 try:
  ...
  tmp=v
  if Logic.current_data is not v:
...
def socketio_callback(cmd,data):
 if sid_list:
  ...
  tmp=json.loads(tmp)
  socketio.emit(cmd,tmp,namespace='/%s'%package_name,broadcast=W)
"""
