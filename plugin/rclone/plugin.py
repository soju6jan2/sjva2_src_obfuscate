...
def detail(sub):
 ...
 if sub=='setting':
  ...
  arg['scheduler']=c(scheduler.is_include(package_name))
  arg['is_running']=c(scheduler.is_running(package_name))
...
def proxy(request,url):
 try:
  resp=requests.request(method=request.method,url=url,headers={key:value for(key,value)in request.headers if key!='Host'},data=request.get_data(),cookies=request.cookies,allow_redirects=P)
...
def connect():
 try:
  ...
  tmp=W
  if Logic.current_data is not W:
...
def socketio_callback(cmd,data):
 if sid_list:
  ...
  tmp=json.loads(tmp)
  socketio.emit(cmd,tmp,namespace='/%s'%package_name,broadcast=d)
"""
