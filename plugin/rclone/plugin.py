...
def detail(sub):
 ...
 if sub=='setting':
  ...
  arg['scheduler']=y(scheduler.is_include(package_name))
  arg['is_running']=y(scheduler.is_running(package_name))
...
def socketio_callback(cmd,data):
 if sid_list:
  ...
  tmp=json.loads(tmp)
  socketio.emit(cmd,tmp,namespace='/%s'%package_name,broadcast=h)
"""
