...
def detail(sub):
 ...
 if sub=='setting':
  ...
  arg['scheduler']=F(scheduler.is_include(package_name))
  arg['is_running']=F(scheduler.is_running(package_name))
...
def proxy(request,url):
 try:
  resp=requests.request(method=request.method,url=url,headers={key:value for(key,value)in request.headers if key!='Host'},data=request.get_data(),cookies=request.cookies,allow_redirects=T)
...
def connect():
 try:
  ...
  tmp=l
  if Logic.current_data is not l:
