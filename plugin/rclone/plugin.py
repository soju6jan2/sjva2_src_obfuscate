...
def detail(sub):
 ...
 if sub=='setting':
  ...
  arg['scheduler']=k(scheduler.is_include(package_name))
  arg['is_running']=k(scheduler.is_running(package_name))
...
def connect():
 try:
  ...
  tmp=s
  if Logic.current_data is not s:
...
def socketio_callback(cmd,data):
 if sid_list:
  ...
  tmp=json.loads(tmp)
  socketio.emit(cmd,tmp,namespace='/%s'%package_name,broadcast=P)
"""
