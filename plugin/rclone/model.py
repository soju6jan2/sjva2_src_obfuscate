...
class ModelRcloneJob(db.Model):
 ...
 def as_dict(self):
  ret={x.name:I(self,x.name)for x in self.__table__.columns}
  ret['last_run_time']=self.last_run_time.strftime('%m-%d %H:%M:%S')if self.last_run_time is not D else ''
  return ret
class ModelRcloneFile(db.Model):
 ...
 def as_dict(self):
  ...
  ret['finish_time']=self.finish_time.strftime('%m-%d %H:%M:%S')if self.finish_time is not D else ''
  ret['delta']=''
  try:
