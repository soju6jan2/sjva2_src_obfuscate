...
class ModelRcloneJob(db.Model):
 ...
 def as_dict(self):
  ...
  ret['last_run_time']=self.last_run_time.strftime('%m-%d %H:%M:%S')if self.last_run_time is not U else ''
  return ret
class ModelRcloneFile(db.Model):
 ...
 def as_dict(self):
  ...
  ret['finish_time']=self.finish_time.strftime('%m-%d %H:%M:%S')if self.finish_time is not U else ''
  ret['delta']=''
  try:
   ret['delta']=r(self.finish_time-self.created_time).split('.')[0]
  except:
   pass
  return ret
