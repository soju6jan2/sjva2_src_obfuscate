...
class Logic(l):
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,package_name,interval,Logic.scheduler_function,u"Rclone 스케쥴링",T)
 ...
 def scheduler_function():
  try:
   ...
   job_list=db.session.query(ModelRcloneJob).filter_by(is_scheduling=T).with_for_update().all()
 ...
 def execute(job):
  try:
   ...
   Logic.current_process=subprocess.Popen(tmp,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=T,bufsize=1)
   Logic.current_data={}
   ...
   job.last_file_count=p(Logic.current_data['files'])
   db.session.commit()
 ...
 @U
 def rclone_job_by_ktv(local,remote,remove=z):
  try:
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(b(t['id'])in Logic.mount_process and Logic.mount_process[b(t['id'])]is not i)
   return ret
  except X as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
