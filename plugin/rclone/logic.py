...
class Logic(N):
 ...
 path_bin=path_rclone=path_config=m
 ...
 def execute(job):
  try:
   ...
   job.last_file_count=A(Logic.current_data['files'])
   db.session.commit()
 ...
 def kill():
  try:
   if Logic.current_process is not m and Logic.current_process.poll()is m:
    import psutil
    ...
    if p(data,FileFinished):
     ...
     if f is not m:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is m else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
  ...
  try:
   ...
   if where=='serve':
    ...
    if item is not m:
     ...
     ret['ret']='fail'
   if log_filename is not m:
    logger.debug(log_filename)
    import codecs
    f=codecs.E(log_filename,'r',encoding='utf8')
    ret['data']=[]
    for line in f:
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(u(t['id'])in Logic.mount_process and Logic.mount_process[u(t['id'])]is not m)
   return ret
  except X as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not m and process.poll()is m:
     import psutil
   ...
   Logic.mount_process[mount_id]=m
 @I
