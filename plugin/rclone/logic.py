...
class Logic(d):
 ...
 path_bin=path_rclone=path_config=o
  ...
  try:
   ...
   for job in job_list:
    ...
    if not scheduler.is_include(package_name):
     ...
     break
   Logic.current_process=o 
  except V as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
  finally:
   Logic.running_status=s
 @v
 ...
 def kill():
  try:
   if Logic.current_process is not o and Logic.current_process.poll()is o:
    import psutil
    ...
    if y(data,FileFinished):
     ...
     if f is not o:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is o else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
 ...
 def set_file(match):
  ...
  instance.percent=w(match.group('percent'))
 ...
 @v
 def rclone_job_by_ktv(local,remote,remove=s):
  try:
 ...
 def mount_save(req):
  try:
   ...
   if mount_id=='-1':
    ...
    item=db.session.query(ModelRcloneMount).filter_by(id=w(mount_id)).with_for_update().first()
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(a(t['id'])in Logic.mount_process and Logic.mount_process[a(t['id'])]is not o)
   return ret
  except V as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not o and process.poll()is o:
     import psutil
     ...
     p.kill()
     try:
      job=db.session.query(ModelRcloneMount).filter_by(id=w(mount_id)).first()
      import platform
      if platform.system()!='Windows':
   ...
   Logic.mount_process[mount_id]=o
 @v
