...
class Logic(z):
 ...
 path_bin=path_rclone=path_config=l
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,package_name,interval,Logic.scheduler_function,u"Rclone 스케쥴링",r)
 ...
 def scheduler_function():
  try:
   ...
   job_list=db.session.query(ModelRcloneJob).filter_by(is_scheduling=r).with_for_update().all()
   ...
   for job in job_list:
    ...
    if not scheduler.is_include(package_name):
     ...
     break
   Logic.current_process=l 
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  finally:
   Logic.running_status=T
 @U
 ...
 def execute(job):
  try:
   ...
   logger.debug('type : %s',h(tmp))
   logger.debug('tmp : %s',tmp)
   Logic.current_process=subprocess.Popen(tmp,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=r,bufsize=1)
   Logic.current_data={}
 ...
 def log_thread_fuction():
  with Logic.current_process.stdout:
   ts=l
   for line in C(Logic.current_process.stdout.readline,b''):
    line=line.strip()
    try:
     try:
 ...
 def kill():
  try:
   if Logic.current_process is not l and Logic.current_process.poll()is l:
    import psutil
   ...
   return 'fail'
 @U
 def trans_callback(cmd,data=l):
  try:
   if data is not l:
    if E(data,FileFinished):
     ...
     if f is not l:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is l else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
    elif E(data,TransStatus):
 ...
 def set_file(match):
  ...
  instance.percent=f(match.group('percent'))
 ...
 def filelist(req):
  try:
   ret={}
   page=1
   page_size=f(db.session.query(ModelSetting).filter_by(key='web_page_size').first().value)
   job_id=''
   search=''
   if 'page' in req.form:
 ...
 @U
 def rclone_job_by_ktv(local,remote,remove=T):
  try:
  ...
  try:
   ...
   if where=='serve':
    ...
    if item is not l:
     ...
     ret['ret']='fail'
   if log_filename is not l:
    logger.debug(log_filename)
    import codecs
    f=codecs.d(log_filename,'r',encoding='utf8')
    ret['data']=[]
    for line in f:
 ...
 def mount_save(req):
  try:
   ...
   if mount_id=='-1':
    ...
    item=db.session.query(ModelRcloneMount).filter_by(id=f(mount_id)).with_for_update().first()
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(F(t['id'])in Logic.mount_process and Logic.mount_process[F(t['id'])]is not l)
   return ret
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not l and process.poll()is l:
     import psutil
     p=psutil.Process(process.pid)
     for proc in p.children(recursive=r):
      proc.kill()
     p.kill()
     try:
      job=db.session.query(ModelRcloneMount).filter_by(id=f(mount_id)).first()
      import platform
      if platform.system()!='Windows':
   ...
   Logic.mount_process[mount_id]=l
 @U
