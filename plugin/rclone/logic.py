...
class Logic(R):
 ...
 path_bin=path_rclone=path_config=z
  ...
  try:
   ...
   for job in job_list:
    ...
    if not scheduler.is_include(package_name):
     ...
     break
   Logic.current_process=z 
  except p as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
  finally:
   Logic.running_status=i
 @I
 ...
 def execute(job):
  try:
   ...
   job.last_file_count=G(Logic.current_data['files'])
   db.session.commit()
 ...
 def log_thread_fuction():
  with Logic.current_process.stdout:
   ts=z
   for line in d(Logic.current_process.stdout.readline,b''):
    line=line.strip()
    try:
     try:
 ...
 def kill():
  try:
   if Logic.current_process is not z and Logic.current_process.poll()is z:
    import psutil
   ...
   return 'fail'
 @I
 def trans_callback(cmd,data=z):
  try:
   if data is not z:
    if q(data,FileFinished):
     ...
     if f is not z:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is z else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
    elif q(data,TransStatus):
 ...
 def set_file(match):
  ...
  instance.percent=g(match.group('percent'))
 ...
 def filelist(req):
  try:
   ret={}
   page=1
   page_size=g(db.session.query(ModelSetting).filter_by(key='web_page_size').first().value)
   job_id=''
   search=''
   if 'page' in req.form:
 ...
 @I
 def rclone_job_by_ktv(local,remote,remove=i):
  try:
  ...
  try:
   ...
   if where=='serve':
    ...
    if item is not z:
     ...
     ret['ret']='fail'
   if log_filename is not z:
    logger.debug(log_filename)
    import codecs
    f=codecs.y(log_filename,'r',encoding='utf8')
    ret['data']=[]
    for line in f:
 ...
 def mount_save(req):
  try:
   ...
   if mount_id=='-1':
    ...
    item=db.session.query(ModelRcloneMount).filter_by(id=g(mount_id)).with_for_update().first()
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(P(t['id'])in Logic.mount_process and Logic.mount_process[P(t['id'])]is not z)
   return ret
  except p as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not z and process.poll()is z:
     import psutil
     ...
     p.kill()
     try:
      job=db.session.query(ModelRcloneMount).filter_by(id=g(mount_id)).first()
      import platform
      if platform.system()!='Windows':
   ...
   Logic.mount_process[mount_id]=z
 @I
