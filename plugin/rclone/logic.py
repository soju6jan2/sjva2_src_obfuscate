...
class Logic(b):
 ...
 path_bin=path_rclone=path_config=G
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,package_name,interval,Logic.scheduler_function,u"Rclone 스케쥴링",C)
 ...
 def scheduler_function():
  try:
   ...
   job_list=db.session.query(ModelRcloneJob).filter_by(is_scheduling=C).with_for_update().all()
   Logic.running_status=C
   for job in job_list:
    Logic.execute(job)
    if not scheduler.is_include(package_name):
     logger.debug('scheduler is stopped by user button')
     break
   Logic.current_process=G 
  except g as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  finally:
 ...
 def execute(job):
  try:
   ...
   Logic.current_process=subprocess.Popen(tmp,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=C,bufsize=1)
   Logic.current_data={}
   ...
   job.last_file_count=W(Logic.current_data['files'])
   db.session.commit()
 ...
 def log_thread_fuction():
  with Logic.current_process.stdout:
   ts=G
   for line in z(Logic.current_process.stdout.readline,b''):
    line=line.strip()
    try:
     try:
 ...
 def kill():
  try:
   if Logic.current_process is not G and Logic.current_process.poll()is G:
    import psutil
    process=psutil.Process(Logic.current_process.pid)
    for proc in process.children(recursive=C):
     proc.kill()
    process.kill()
    return 'success'
   return 'not_running'
    ...
    if y(data,FileFinished):
     ...
     if f is not G:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is G else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(T(t['id'])in Logic.mount_process and Logic.mount_process[T(t['id'])]is not G)
   return ret
  except g as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not G and process.poll()is G:
     import psutil
     p=psutil.Process(process.pid)
     for proc in p.children(recursive=C):
      proc.kill()
     p.kill()
     try:
   ...
   Logic.mount_process[mount_id]=G
 @E
