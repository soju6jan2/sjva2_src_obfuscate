...
class Logic(h):
 ...
 path_bin=path_rclone=path_config=p
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,package_name,interval,Logic.scheduler_function,u"Rclone 스케쥴링",D)
 ...
 def scheduler_function():
  try:
   ...
   job_list=db.session.query(ModelRcloneJob).filter_by(is_scheduling=D).with_for_update().all()
   Logic.running_status=D
   for job in job_list:
    Logic.execute(job)
    if not scheduler.is_include(package_name):
     logger.debug('scheduler is stopped by user button')
     break
   Logic.current_process=p 
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  finally:
   Logic.running_status=n
 @v
 ...
 def execute(job):
  try:
   ...
   Logic.current_process=subprocess.Popen(tmp,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=D,bufsize=1)
   Logic.current_data={}
 ...
 def log_thread_fuction():
  with Logic.current_process.stdout:
   ts=p
   for line in j(Logic.current_process.stdout.readline,b''):
    line=line.strip()
    try:
     try:
 ...
 def kill():
  try:
   if Logic.current_process is not p and Logic.current_process.poll()is p:
    import psutil
   ...
   return 'fail'
 @v
 def trans_callback(cmd,data=p):
  try:
   if data is not p:
    if r(data,FileFinished):
     ...
     if f is not p:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is p else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
    elif r(data,TransStatus):
 ...
 def filelist(req):
  try:
   ret={}
   page=1
   page_size=L(db.session.query(ModelSetting).filter_by(key='web_page_size').first().value)
   job_id=''
   search=''
   if 'page' in req.form:
 ...
 @v
 def rclone_job_by_ktv(local,remote,remove=n):
  try:
  ...
  try:
   ...
   if where=='serve':
    ...
    if item is not p:
     ...
     ret['ret']='fail'
   if log_filename is not p:
    logger.debug(log_filename)
    import codecs
    f=codecs.X(log_filename,'r',encoding='utf8')
    ret['data']=[]
    for line in f:
 ...
 def mount_save(req):
  try:
   ...
   if mount_id=='-1':
    ...
    item=db.session.query(ModelRcloneMount).filter_by(id=L(mount_id)).with_for_update().first()
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(V(t['id'])in Logic.mount_process and Logic.mount_process[V(t['id'])]is not p)
   return ret
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not p and process.poll()is p:
     import psutil
     p=psutil.Process(process.pid)
     for proc in p.children(recursive=D):
      proc.kill()
     p.kill()
     try:
      job=db.session.query(ModelRcloneMount).filter_by(id=L(mount_id)).first()
      import platform
      if platform.system()!='Windows':
   ...
   Logic.mount_process[mount_id]=p
 @v
