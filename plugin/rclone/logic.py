...
class Logic(k):
 ...
 def set_file(match):
  ...
  instance.percent=e(match.group('percent'))
 ...
 def get_log(req):
  try:
   ...
   if log_filename is not A:
    ...
    import codecs
    f=codecs.f(log_filename,'r',encoding='utf8')
    ret['data']=[]
    for line in f:
 ...
 def mount_save(req):
  try:
   ...
   if mount_id=='-1':
    ...
    item=db.session.query(ModelRcloneMount).filter_by(id=e(mount_id)).with_for_update().first()
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(C(t['id'])in Logic.mount_process and Logic.mount_process[C(t['id'])]is not A)
   return ret
  except O as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
  ...
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not A and process.poll()is A:
     ...
     p.kill()
     try:
      job=db.session.query(ModelRcloneMount).filter_by(id=e(mount_id)).first()
      import platform
      if platform.system()!='Windows':
