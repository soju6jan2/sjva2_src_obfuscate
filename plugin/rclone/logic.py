...
class Logic(c):
 ...
 path_bin=path_rclone=path_config=D
 ...
 def rclone_version():
  try:
   ...
   process=subprocess.Popen(command,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=n,bufsize=1)
   ret=[]
   with process.stdout:
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,package_name,interval,Logic.scheduler_function,u"Rclone 스케쥴링",n)
 ...
 def scheduler_function():
  try:
   ...
   job_list=db.session.query(ModelRcloneJob).filter_by(is_scheduling=n).with_for_update().all()
   ...
   for job in job_list:
    ...
    if not scheduler.is_include(package_name):
     ...
     break
   Logic.current_process=D 
  except z as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  finally:
   Logic.running_status=y
 @x
 ...
 def execute(job):
  try:
   ...
   logger.debug('type : %s',F(tmp))
   logger.debug('tmp : %s',tmp)
   Logic.current_process=subprocess.Popen(tmp,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=n,bufsize=1)
   Logic.current_data={}
   ...
   job.last_file_count=K(Logic.current_data['files'])
   db.session.commit()
 ...
 def log_thread_fuction():
  with Logic.current_process.stdout:
   ts=D
   for line in v(Logic.current_process.stdout.readline,b''):
    line=line.strip()
    try:
     try:
 ...
 def kill():
  try:
   if Logic.current_process is not D and Logic.current_process.poll()is D:
    import psutil
    process=psutil.Process(Logic.current_process.pid)
    for proc in process.children(recursive=n):
     proc.kill()
    process.kill()
    return 'success'
   return 'not_running'
    ...
    if m(data,FileFinished):
     ...
     if f is not D:
      if f.log!='':
       ...
       f.log=data.status
      f.finish_time=datetime.now()if f.finish_time is D else f.finish_time
      db.session.add(f)
      db.session.commit()
     pass
 ...
 @x
 def rclone_job_by_ktv(local,remote,remove=y):
  try:
 ...
 def mount_list():
  try:
   ...
   ret=[x.as_dict()for x in job_list]
   for t in ret:
    t['current_status']=(r(t['id'])in Logic.mount_process and Logic.mount_process[r(t['id'])]is not D)
   return ret
  except z as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def mount_kill(mount_id):
  try:
   if mount_id in Logic.mount_process:
    ...
    if process is not D and process.poll()is D:
     import psutil
     p=psutil.Process(process.pid)
     for proc in p.children(recursive=n):
      proc.kill()
     p.kill()
     try:
   ...
   Logic.mount_process[mount_id]=D
 @x
