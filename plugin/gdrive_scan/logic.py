...
class Logic(J):
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(X=m(args[0])).first()
 ...
 def receive_scan_result(X,filename):
  try:
   if t:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(X=m(X)).with_for_update().first()
