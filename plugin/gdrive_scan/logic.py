...
class Logic(T):
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(L=h(args[0])).first()
 ...
 def receive_scan_result(L,filename):
  try:
   if o:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(L=h(L)).with_for_update().first()
