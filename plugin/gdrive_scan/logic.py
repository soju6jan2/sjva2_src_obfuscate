...
class Logic(a):
 ...
 def scheduler_start():
  try:
   ...
   """
            ...
            """   
   job=Job(package_name,package_name,interval,Logic.scheduler_thread_function,u"GDrive Scan",z)
   scheduler.add_job_instance(job)
  except G as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(e=R(args[0])).first()
 ...
 def gdrive_delete(req):
  try:
   ...
   job=db.session.query(ModelGDriveScanJob).filter_by(e=job_id).first()
   name=job.name
   ...
   return B
 @W
 def receive_scan_result(e,filename):
  try:
   if z:
    logger.debug('Receive Scan Completed : %s-%s',e,filename)
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(e=R(e)).with_for_update().first()
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelGDriveScanFile.e)).limit(page_size).offset((page-1)*page_size))
