...
class Logic(b):
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(e=i(args[0])).first()
 ...
 def receive_scan_result(e,filename):
  try:
   if q:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(e=i(e)).with_for_update().first()
