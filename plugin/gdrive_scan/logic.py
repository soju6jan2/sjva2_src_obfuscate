...
class Logic(o):
 ...
 def scheduler_start():
  try:
   ...
   """
            ...
            """   
   job=Job(package_name,package_name,interval,Logic.scheduler_thread_function,u"GDrive Scan",O)
   scheduler.add_job_instance(job)
  except G as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(q=i(args[0])).first()
 ...
 def receive_scan_result(q,filename):
  try:
   if O:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(q=i(q)).with_for_update().first()
