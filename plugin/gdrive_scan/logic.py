...
class Logic(i):
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(g=P(args[0])).first()
 ...
 def gdrive_delete(req):
  try:
   job_id=P(req.form['id'])
   job=db.session.query(ModelGDriveScanJob).filter_by(g=job_id).first()
   name=job.name
 ...
 def receive_scan_result(g,filename):
  try:
   if k:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(g=P(g)).with_for_update().first()
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelGDriveScanFile.g)).limit(page_size).offset((page-1)*page_size))
