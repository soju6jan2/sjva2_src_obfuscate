...
class Logic(E):
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(B=v(args[0])).first()
 ...
 def receive_scan_result(B,filename):
  try:
   if p:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(B=v(B)).with_for_update().first()
