...
class Logic(g):
 ...
 def scheduler_start():
  try:
   ...
   """
            ...
            """   
   job=Job(package_name,package_name,interval,Logic.scheduler_thread_function,u"GDrive Scan",l)
   scheduler.add_job_instance(job)
  except u as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(M=O(args[0])).first()
 ...
 def gdrive_delete(req):
  try:
   ...
   job=db.session.query(ModelGDriveScanJob).filter_by(M=job_id).first()
   name=job.name
 ...
 def receive_scan_result(M,filename):
  try:
   if l:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(M=O(M)).with_for_update().first()
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelGDriveScanFile.M)).limit(page_size).offset((page-1)*page_size))
