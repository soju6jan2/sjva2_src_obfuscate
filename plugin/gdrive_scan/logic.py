...
class Logic(X):
 ...
 def scheduler_start():
  try:
   ...
   """
            ...
            """   
   job=Job(package_name,package_name,interval,Logic.scheduler_thread_function,u"GDrive Scan",t)
   scheduler.add_job_instance(job)
  except U as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(n=C(args[0])).first()
 ...
 def gdrive_delete(req):
  try:
   job_id=C(req.form['id'])
   job=db.session.query(ModelGDriveScanJob).filter_by(n=job_id).first()
   name=job.name
 ...
 def receive_scan_result(n,filename):
  try:
   if t:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(n=C(n)).with_for_update().first()
 ...
 def filelist(req):
  try:
   ret={}
   page=1
   page_size=C(db.session.query(ModelSetting).filter_by(key='web_page_size').first().value)
   job_id=''
   search=''
   if 'page' in req.form:
    page=C(req.form['page'])
   ...
   count=query.count()
   query=(query.order_by(desc(ModelGDriveScanFile.n)).limit(page_size).offset((page-1)*page_size))
