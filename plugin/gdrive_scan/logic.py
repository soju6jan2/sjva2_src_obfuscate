...
class Logic(i):
 ...
 def scheduler_start():
  try:
   ...
   """
            ...
            """   
   job=Job(package_name,package_name,interval,Logic.scheduler_thread_function,u"GDrive Scan",k)
   scheduler.add_job_instance(job)
  except P as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(U=u(args[0])).first()
 ...
 def gdrive_delete(req):
  try:
   job_id=u(req.form['id'])
   job=db.session.query(ModelGDriveScanJob).filter_by(U=job_id).first()
   name=job.name
   ...
   return K
 @I
 def receive_scan_result(U,filename):
  try:
   if k:
    logger.debug('Receive Scan Completed : %s-%s',U,filename)
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(U=u(U)).with_for_update().first()
 ...
 def filelist(req):
  try:
   ret={}
   page=1
   page_size=u(db.session.query(ModelSetting).filter_by(key='web_page_size').first().value)
   job_id=''
   search=''
   if 'page' in req.form:
    page=u(req.form['page'])
   ...
   count=query.count()
   query=(query.order_by(desc(ModelGDriveScanFile.U)).limit(page_size).offset((page-1)*page_size))
