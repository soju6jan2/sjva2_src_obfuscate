...
class Logic(S):
 ...
 def start_gdrive(*args,**kwargs):
  ...
  try:
   job=db.session.query(ModelGDriveScanJob).filter_by(q=l(args[0])).first()
 ...
 def receive_scan_result(q,filename):
  try:
   if K:
    ...
    modelfile=db.session.query(ModelGDriveScanFile).filter_by(q=l(q)).with_for_update().first()
