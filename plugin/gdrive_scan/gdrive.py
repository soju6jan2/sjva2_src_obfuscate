...
from oauth2client.f import Storage
from oauth2client import tools
from oauth2client.client import flow_from_clientsecrets,OAuth2WebServerFlow
from httplib2 import Http
from oauth2client import f,client,tools
...
class GDrive(g):
 def __init__(self,match_rule):
  ...
  self.cache=SqliteDict(self.db,tablename='cache',encode=json.dumps,decode=json.loads,autocommit=l)
 ...
 def start_change_watch(self):
  ...
  def thread_function():
   ...
   while self.flag_thread_run:
    try:
     for _ in L(self.change_check_interval):
     ...
     for _ in items:
      ...
      for job in job_list: 
       ...
       for _ in L(self.api_call_inverval):
        if self.flag_thread_run==W:
         return
        time.sleep(1)
 ...
 def get_mount_abspath(self,gdrive_path):
  try:
   ...
   flag_find=l
   for idx,val in Q(replace_gdrive_path):
 ...
 def get_section_id(self,path):
  try:
   ...
   logger.debug('SectionID:%s %s',section_id,E(section_id))
   return section_id
  except u as e:
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,O(s_id)if E(s_id)==E('')else s_id,is_file,(type_add_remove=='ADD'))
  except u as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
  try:
