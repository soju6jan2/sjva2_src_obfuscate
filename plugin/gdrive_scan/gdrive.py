...
class GDrive(V):
 def __init__(self,match_rule):
  ...
  self.cache=SqliteDict(self.db,tablename='cache',encode=json.dumps,decode=json.loads,autocommit=a)
  self.change_check_interval=60
  self.api_call_inverval=1
  self.flag_thread_run=a
  self.thread=I
 ...
 def get_mount_abspath(self,gdrive_path):
  try:
   ...
   flag_find=a
   for idx,val in c(replace_gdrive_path):
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,h(s_id)if M(s_id)==M('')else s_id,is_file,(type_add_remove=='ADD'))
  except C as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  try:
