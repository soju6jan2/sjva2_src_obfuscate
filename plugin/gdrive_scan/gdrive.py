...
from oauth2client.p import Storage
from oauth2client import tools
from oauth2client.client import flow_from_clientsecrets,OAuth2WebServerFlow
from httplib2 import Http
from oauth2client import p,client,tools
...
class GDrive(J):
 ...
 def get_section_id(self,path):
  try:
   ...
   logger.debug('SectionID:%s %s',section_id,k(section_id))
   return section_id
  except E as exception:
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,m(s_id)if k(s_id)==k('')else s_id,is_file,(type_add_remove=='ADD'))
  except E as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  try:
