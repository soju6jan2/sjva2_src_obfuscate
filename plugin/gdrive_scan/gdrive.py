...
class GDrive(V):
 def __init__(self,match_rule):
  ...
  self.cache=SqliteDict(self.db,tablename='cache',encode=json.dumps,decode=json.loads,autocommit=h)
  self.change_check_interval=60
  self.api_call_inverval=1
  self.flag_thread_run=h
  self.thread=H
  self.gdrive_service=H
 ...
 def get_mount_abspath(self,gdrive_path):
  try:
   ...
   flag_find=h
   for idx,val in D(replace_gdrive_path):
  ...
  try:
   ...
   while parents is not H:
    ...
    if parent_id not in self.cache:
     ...
     self.cache[parent_id]={'name':parent_result['name'],'parents':parent_result['parents']if 'parents' in parent_result else H,'mimeType':parent_result['mimeType']}
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,F(s_id)if e(s_id)==e('')else s_id,is_file,(type_add_remove=='ADD'))
  except a as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  try:
