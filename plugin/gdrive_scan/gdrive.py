...
class GDrive(C):
 ...
 def get_section_id(self,path):
  try:
   ...
   logger.debug('SectionID:%s %s',section_id,d(section_id))
   return section_id
  except Q as exception:
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,b(s_id)if d(s_id)==d('')else s_id,is_file,(type_add_remove=='ADD'))
  except Q as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  try:
