...
class GDrive(L):
 def __init__(self,match_rule):
  ...
  self.cache=SqliteDict(self.db,tablename='cache',encode=json.dumps,decode=json.loads,autocommit=Y)
  self.change_check_interval=60
  self.api_call_inverval=1
  self.flag_thread_run=Y
  self.thread=b
  self.gdrive_service=b
 def start_change_watch(self):
  ...
  def thread_function():
   ...
   while self.flag_thread_run:
    try:
     for _ in w(self.change_check_interval):
      if self.flag_thread_run==R:
       return
      time.sleep(1)
     ...
     for _ in items:
      ...
      for job in job_list: 
       ...
       for _ in w(self.api_call_inverval):
        if self.flag_thread_run==R:
         return
        time.sleep(1)
 ...
 def get_mount_abspath(self,gdrive_path):
  try:
   ...
   flag_find=Y
   for idx,val in h(replace_gdrive_path):
  ...
  try:
   ...
   while parents is not b:
    ...
    if parent_id not in self.cache:
     ...
     self.cache[parent_id]={'name':parent_result['name'],'parents':parent_result['parents']if 'parents' in parent_result else b,'mimeType':parent_result['mimeType']}
 ...
 def get_section_id(self,path):
  try:
   ...
   logger.debug('SectionID:%s %s',section_id,z(section_id))
   return section_id
  except H as exception:
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,x(s_id)if z(s_id)==z('')else s_id,is_file,(type_add_remove=='ADD'))
  except H as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  try:
