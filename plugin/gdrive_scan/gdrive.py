...
class GDrive(N):
 def __init__(self,match_rule):
  ...
  self.cache=SqliteDict(self.db,tablename='cache',encode=json.dumps,decode=json.loads,autocommit=w)
  self.change_check_interval=60
  self.api_call_inverval=1
  self.flag_thread_run=w
  self.thread=v
  self.gdrive_service=v
 ...
 def get_mount_abspath(self,gdrive_path):
  try:
   ...
   flag_find=w
   for idx,val in z(replace_gdrive_path):
  ...
  try:
   ...
   while parents is not v:
    ...
    if parent_id not in self.cache:
     ...
     self.cache[parent_id]={'name':parent_result['name'],'parents':parent_result['parents']if 'parents' in parent_result else v,'mimeType':parent_result['mimeType']}
 ...
 def get_section_id(self,path):
  try:
   ...
   logger.debug('SectionID:%s %s',section_id,l(section_id))
   return section_id
  except r as exception:
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,M(s_id)if l(s_id)==l('')else s_id,is_file,(type_add_remove=='ADD'))
  except r as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  try:
