...
from oauth2client.b import Storage
from oauth2client import tools
from oauth2client.client import flow_from_clientsecrets,OAuth2WebServerFlow
from httplib2 import Http
from oauth2client import b,client,tools
...
class GDrive(T):
 def __init__(self,match_rule):
  ...
  self.cache=SqliteDict(self.db,tablename='cache',encode=json.dumps,decode=json.loads,autocommit=o)
  self.change_check_interval=60
  self.api_call_inverval=1
  self.flag_thread_run=o
  self.thread=m
  self.gdrive_service=m
 def start_change_watch(self):
  ...
  def thread_function():
   ...
   while self.flag_thread_run:
    try:
     for _ in S(self.change_check_interval):
      if self.flag_thread_run==W:
       return
      time.sleep(1)
     ...
     for _ in items:
      ...
      for job in job_list: 
       ...
       for _ in S(self.api_call_inverval):
        if self.flag_thread_run==W:
         return
        time.sleep(1)
  ...
  try:
   ...
   while parents is not m:
    ...
    if parent_id not in self.cache:
     ...
     self.cache[parent_id]={'name':parent_result['name'],'parents':parent_result['parents']if 'parents' in parent_result else m,'mimeType':parent_result['mimeType']}
 ...
 def get_section_id(self,path):
  try:
   ...
   logger.debug('SectionID:%s %s',section_id,R(section_id))
   return section_id
  except P as e:
 ...
 def send_command(self,s_id,mount_abspath,type_add_remove,is_file):
  callback_id=-1
  try:
   callback_id=ModelGDriveScanFile.add(self.gdrive_name,mount_abspath,h(s_id)if R(s_id)==R('')else s_id,is_file,(type_add_remove=='ADD'))
  except P as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
  try:
