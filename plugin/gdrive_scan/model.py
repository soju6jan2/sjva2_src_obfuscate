...
class ModelGDriveScanFile(db.Model):
 ...
 def as_dict(self):
  ...
  ret['created_time']=self.created_time.strftime('%m-%d %H:%M:%S')if self.created_time is not w else ''
  ret['scan_time']=self.scan_time.strftime('%m-%d %H:%M:%S')if self.scan_time is not w else ''
  return ret
 @t
