...
class Logic(W):
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.V=='show' and i(section.key)==entity.plex_section_id:
     ...
     try:
      ...
      for season in show.seasons():
       ...
       if item['guid'].lower().find('daum'):
        tmp=o
        if flag_media_season and season_data is not o and m(season_data['series'])>1:
       ...
       for episode in episodes:
        ...
        if episode.index is o:
         ...
         flag_index=C
         if epi_min is o or epi_min>i(episode.index):
          epi_min=i(episode.index)
         if epi_max is o or epi_max<i(episode.index):
