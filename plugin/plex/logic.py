...
class Logic(v):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @m
 def get_section_id(entity,more=M):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.d=='show' and T(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not o else os.path.join(entity.scan_abspath,entity.filename)
     ...
     try:
      ...
      if e(show.seasons())>1:
       for season in show.seasons():
        if T(season.index)>1 and T(season.index)<1900:
         flag_media_season=w
         break
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=o
       if item['guid'].lower().find('daum'):
        tmp=o
        if flag_media_season and season_data is not o and e(season_data['series'])>1:
         search_title=season_data['series'][T(season.index)-1]['title']
         search_id=season_data['series'][T(season.index)-1]['id']
       ...
       season_entity['episode_count']=e(episodes)
       ...
       for episode in episodes:
        ...
        if episode.index is o:
         ...
         flag_index=w
         if epi_min is o or epi_min>T(episode.index):
          epi_min=T(episode.index)
         if epi_max is o or epi_max<T(episode.index):
          epi_max=T(episode.index)
         season_entity['episode_index_list'].append(T(episode.index))
         epi_count_index+=1
        ...
        if e(season_entity['episodes'][episode_key])>1:
       ...
       season_entity['episode_index_list']=N(season_entity['episode_index_list'])
       season_entity['episode_air_list']=N(season_entity['episode_air_list'])
       season_entity['duplicate_list']=N(season_entity['duplicate_list'])
       ...
       msg=''
       if season_entity['daum_info']is not o:
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in A(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in A(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not o and season_entity['daum_info']['last_episode_no']is not o:
         if one_file_how_many_episodes==1:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,season_entity['daum_info']['last_episode_no'])
         else:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,T(season_entity['daum_info']['last_episode_no'])-1)
         if K(epi_max)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         elif one_file_how_many_episodes==2 and K(epi_max+1)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         else:
     ...
     for tag_video in videos:
      ...
      try:
       ...
       if 'duration' not in tag_media.attrib:
        continue
       duration=T(tag_media.attrib['duration'])
       ...
       channel_tag.set('id',K(channel_number))
       ...
       display_name_tag.text=K(channel_number)
       ...
       program_tag.set('channel',K(channel_number))
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not o:
        root.remove(channel_tag)
       if program_tag is not o:
        root.remove(channel_tag)
  except D as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
