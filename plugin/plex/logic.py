...
class Logic(U):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @F
 def get_section_id(entity,more=J):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.d=='show' and B(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not i else os.path.join(entity.scan_abspath,entity.filename)
     ...
     try:
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=i
       if item['guid'].lower().find('daum'):
        tmp=i
        if flag_media_season and season_data is not i and R(season_data['series'])>1:
         search_title=season_data['series'][B(season.index)-1]['title']
         search_id=season_data['series'][B(season.index)-1]['id']
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=V)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=V)
        if tmp:
       ...
       season_entity['episode_count']=R(episodes)
       ...
       for episode in episodes:
        ...
        if episode.index is i:
         ...
         flag_index=V
         if epi_min is i or epi_min>B(episode.index):
          epi_min=B(episode.index)
         if epi_max is i or epi_max<B(episode.index):
        ...
        if R(season_entity['episodes'][episode_key])>1:
       ...
       season_entity['episode_index_list']=G(season_entity['episode_index_list'])
       season_entity['episode_air_list']=G(season_entity['episode_air_list'])
       season_entity['duplicate_list']=G(season_entity['duplicate_list'])
       ...
       msg=''
       if season_entity['daum_info']is not i:
       ...
       if flag_index:
        ...
        if season_entity['daum_info']is not i and season_entity['daum_info']['last_episode_no']is not i:
         if one_file_how_many_episodes==1:
    ...
    if info['type']=='recent_add':
     ...
     for tag_video in videos:
      ...
      try:
       ...
       logger.error('Exception:%s',e)
       logger.error(traceback.format_exc())
       if channel_tag is not i:
        root.remove(channel_tag)
       if program_tag is not i:
        root.remove(channel_tag)
  except k as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc()) 
