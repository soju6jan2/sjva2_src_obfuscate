...
class Logic(g):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @h
 def get_section_id(entity,more=s):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.E=='show' and i(section.key)==entity.plex_section_id:
     ...
     try:
      ...
      for season in show.seasons():
       ...
       if item['guid'].lower().find('daum'):
        tmp=A
        if flag_media_season and season_data is not A and O(season_data['series'])>1:
         search_title=season_data['series'][i(season.index)-1]['title']
         search_id=season_data['series'][i(season.index)-1]['id']
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=R)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=R)
        if tmp:
       ...
       season_entity['episode_index_list']=F(season_entity['episode_index_list'])
       season_entity['episode_air_list']=F(season_entity['episode_air_list'])
       season_entity['duplicate_list']=F(season_entity['duplicate_list'])
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in p(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in p(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not A and season_entity['daum_info']['last_episode_no']is not A:
         ...
         if m(epi_max)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         elif one_file_how_many_episodes==2 and m(epi_max+1)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         else:
    ...
    if info['type']=='recent_add':
     ...
     for tag_video in videos:
      ...
      try:
       ...
       channel_tag.set('id',m(channel_number))
       ...
       display_name_tag.text=m(channel_number)
       ...
       program_tag.set('channel',m(channel_number))
