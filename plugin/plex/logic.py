...
f=DaumTV.get_daum_tv_info
l=DaumTV.get_show_info_on_home_title
i=DaumTV.get_show_info_on_home
...
class Logic(j):
 ...
 @a
 def get_server_hash():
  return B.query(ModelSetting).filter_by(key='machineIdentifier').first().value
 @a
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @a
 def get_section_id(entity,more=L):
  try:
   server_url=B.query(ModelSetting).filter_by(key='server_url').first().value
   server_token=B.query(ModelSetting).filter_by(key='server_token').first().value
    ...
    for show in select_section.all():
     for location in show.locations:
      if entity.plex_abspath.find(location)!=-1:
       ...
       match=p(r'\/\/(?P<id>\d+)?').search(show.guid)
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.A=='show' and y(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   if server_url=='':
    ...
    return
   callback_url='%s/%s/api/scan_completed'%(s('ddns'),plugin_name)
   filename=entity.plex_abspath if entity.plex_abspath is not k else u.join(entity.scan_abspath,entity.filename)
   ...
   url='%s/:/plugins/com.plexapp.plugins.SJVA/function/WaitFile?section_id=%s&filename=%s&callback=%s&callback_id=%s&type_add_remove=ADD&call_from=FILE_MANAGER&X-Plex-Token=%s'%(server_url,entity.plex_section_id,encode_filename,D(callback_url),entity.id,server_token)
   ...
   entity.send_command_time=x()
   scan_server=B.query(ModelSetting).filter_by(key='scan_server').first().value
   if scan_server!='':
 ...
 def send_scan_command2(plugin_name,section_id,filename,callback_id,type_add_remove,call_from):
  ...
  try:
   ...
   encode_filename=Logic.get_filename_encoding_for_plex(filename)
   url='%s/:/plugins/com.plexapp.plugins.SJVA/function/WaitFile?section_id=%s&filename=%s&callback=%s&callback_id=%s&type_add_remove=%s&call_from=%s&X-Plex-Token=%s'%(server_url,section_id,encode_filename,D(callback_url),callback_id,type_add_remove,call_from,server_token)
   logger.debug('URL:%s',url)
   request=z(url)
   response=n(request)
   data=response.read()
 ...
 def analyze_show(key):
  try:
   ...
   for section in sections:
    ...
    if section.key!=key:
     continue
    for index,show in G(section.all()):
     try:
      flag_media_season=L
      if M(show.seasons())>1:
       for season in show.seasons():
        if y(season.index)>1 and y(season.index)<1900:
         flag_media_season=v
         break
      if flag_media_season:
       season_data=i(l(show.title))
      item={}
      ...
      for season in show.seasons():
       ...
       if item['guid'].lower().find('daum'):
        tmp=k
        if flag_media_season and season_data is not k and M(season_data['series'])>1:
         search_title=season_data['series'][y(season.index)-1]['title']
         search_id=season_data['series'][y(season.index)-1]['id']
         tmp=f(search_title,daum_id=search_id,on_home=v)
        else:
         tmp=f(show.title,on_home=v)
        if tmp:
       ...
       for episode in episodes:
        ...
        if episode.index is k:
         ...
         flag_originallyAvailableAt=v
         season_entity['episode_air_list'].append(episode_key)
         epi_count_date+=1
        else:
         episode_key=episode.index
         flag_index=v
         if epi_min is k or epi_min>y(episode.index):
          epi_min=y(episode.index)
         if epi_max is k or epi_max<y(episode.index):
          epi_max=y(episode.index)
         season_entity['episode_index_list'].append(y(episode.index))
         epi_count_index+=1
        ...
        if M(season_entity['episodes'][episode_key])>1:
       ...
       season_entity['episode_index_list']=R(season_entity['episode_index_list'])
       season_entity['episode_air_list']=R(season_entity['episode_air_list'])
       season_entity['duplicate_list']=R(season_entity['duplicate_list'])
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in H(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in H(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not k and season_entity['daum_info']['last_episode_no']is not k:
         if one_file_how_many_episodes==1:
          ...
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,y(season_entity['daum_info']['last_episode_no'])-1)
