...
class Logic(h):
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.f=='show' and Y(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not d else os.path.join(entity.scan_abspath,entity.filename)
 ...
 def analyze_show(key):
  try:
   ...
   for section in sections:
    ...
    if section.key!=key:
     continue
    for index,show in k(section.all()):
     try:
      flag_media_season=w
      if L(show.seasons())>1:
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=d
       if item['guid'].lower().find('daum'):
        tmp=d
        if flag_media_season and season_data is not d and L(season_data['series'])>1:
         search_title=season_data['series'][Y(season.index)-1]['title']
         search_id=season_data['series'][Y(season.index)-1]['id']
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=q)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=q)
        if tmp:
       ...
       season_entity['episode_count']=L(episodes)
       ...
       for episode in episodes:
        ...
        if L(season_entity['episodes'][episode_key])>1:
       ...
       msg=''
       if season_entity['daum_info']is not d:
       ...
       if flag_index:
        ...
        if season_entity['daum_info']is not d and season_entity['daum_info']['last_episode_no']is not d:
         if one_file_how_many_episodes==1:
    ...
    if info['type']=='recent_add':
     ...
     for tag_video in videos:
      ...
      try:
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not d:
        root.remove(channel_tag)
       if program_tag is not d:
        root.remove(channel_tag)
  except J as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
