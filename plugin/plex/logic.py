...
class Logic(N):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @c
 def get_section_id(entity,more=O):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.P=='show' and X(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not E else os.path.join(entity.scan_abspath,entity.filename)
 ...
 def analyze_show(key):
  try:
   ...
   for section in sections:
    ...
    if section.key!=key:
     continue
    for index,show in D(section.all()):
     try:
      flag_media_season=O
      if p(show.seasons())>1:
       for season in show.seasons():
        if X(season.index)>1 and X(season.index)<1900:
         flag_media_season=U
         break
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=E
       if item['guid'].lower().find('daum'):
        tmp=E
        if flag_media_season and season_data is not E and p(season_data['series'])>1:
         search_title=season_data['series'][X(season.index)-1]['title']
         search_id=season_data['series'][X(season.index)-1]['id']
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=U)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=U)
        if tmp:
       ...
       for episode in episodes:
        ...
        if episode.index is E:
         ...
         flag_index=U
         if epi_min is E or epi_min>X(episode.index):
          epi_min=X(episode.index)
         if epi_max is E or epi_max<X(episode.index):
          epi_max=X(episode.index)
         season_entity['episode_index_list'].append(X(episode.index))
         epi_count_index+=1
        ...
        if p(season_entity['episodes'][episode_key])>1:
       ...
       msg=''
       if season_entity['daum_info']is not E:
       ...
       if flag_index:
        ...
        if season_entity['daum_info']is not E and season_entity['daum_info']['last_episode_no']is not E:
         if one_file_how_many_episodes==1:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,season_entity['daum_info']['last_episode_no'])
         else:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,X(season_entity['daum_info']['last_episode_no'])-1)
         if r(epi_max)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         elif one_file_how_many_episodes==2 and r(epi_max+1)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         else:
     ...
     for tag_video in videos:
      ...
      try:
       ...
       if 'duration' not in tag_media.attrib:
        continue
       duration=X(tag_media.attrib['duration'])
       ...
       channel_tag.set('id',r(channel_number))
       ...
       display_name_tag.text=r(channel_number)
       ...
       program_tag.set('channel',r(channel_number))
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not E:
        root.remove(channel_tag)
       if program_tag is not E:
        root.remove(channel_tag)
  except C as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
