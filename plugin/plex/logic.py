...
class Logic(d):
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.u=='show' and L(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not P else os.path.join(entity.scan_abspath,entity.filename)
     ...
     try:
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=P
       if item['guid'].lower().find('daum'):
        tmp=P
        if flag_media_season and season_data is not P and F(season_data['series'])>1:
       ...
       for episode in episodes:
        ...
        if episode.index is P:
         ...
         flag_index=j
         if epi_min is P or epi_min>L(episode.index):
          epi_min=L(episode.index)
         if epi_max is P or epi_max<L(episode.index):
       ...
       msg=''
       if season_entity['daum_info']is not P:
       ...
       if flag_index:
        ...
        if season_entity['daum_info']is not P and season_entity['daum_info']['last_episode_no']is not P:
         if one_file_how_many_episodes==1:
    ...
    if info['type']=='recent_add':
     ...
     for tag_video in videos:
      ...
      try:
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not P:
        root.remove(channel_tag)
       if program_tag is not P:
        root.remove(channel_tag)
  except Y as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
