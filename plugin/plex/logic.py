...
class Logic(h):
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.N=='show' and y(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not a else os.path.join(entity.scan_abspath,entity.filename)
 ...
 def analyze_show(key):
  try:
   ...
   for section in sections:
    ...
    if section.key!=key:
     continue
    for index,show in C(section.all()):
     try:
      flag_media_season=m
      if K(show.seasons())>1:
      ...
      for season in show.seasons():
       ...
       if item['guid'].lower().find('daum'):
        tmp=a
        if flag_media_season and season_data is not a and K(season_data['series'])>1:
       ...
       for episode in episodes:
        ...
        if episode.index is a:
         ...
         flag_index=d
         if epi_min is a or epi_min>y(episode.index):
          epi_min=y(episode.index)
         if epi_max is a or epi_max<y(episode.index):
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in G(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in G(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
