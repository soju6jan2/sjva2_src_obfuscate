...
class Logic(c):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @E
 def get_section_id(entity,more=D):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.o=='show' and P(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not s else os.path.join(entity.scan_abspath,entity.filename)
 ...
 def analyze_show(key):
  try:
   ...
   for section in sections:
    ...
    if section.key!=key:
     continue
    for index,show in H(section.all()):
     try:
      flag_media_season=D
      if M(show.seasons())>1:
       for season in show.seasons():
        if P(season.index)>1 and P(season.index)<1900:
         flag_media_season=g
         break
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=s
       if item['guid'].lower().find('daum'):
        tmp=s
        if flag_media_season and season_data is not s and M(season_data['series'])>1:
         search_title=season_data['series'][P(season.index)-1]['title']
         search_id=season_data['series'][P(season.index)-1]['id']
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=g)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=g)
        if tmp:
       ...
       for episode in episodes:
        episode_key=s
        if episode.index is s:
         episode_key=episode.originallyAvailableAt.strftime('%Y-%m-%d')
         flag_originallyAvailableAt=g
         season_entity['episode_air_list'].append(episode_key)
         epi_count_date+=1
        else:
         episode_key=episode.index
         flag_index=g
         if epi_min is s or epi_min>P(episode.index):
          epi_min=P(episode.index)
         if epi_max is s or epi_max<P(episode.index):
          epi_max=P(episode.index)
         season_entity['episode_index_list'].append(P(episode.index))
         epi_count_index+=1
       ...
       season_entity['episode_index_list']=h(season_entity['episode_index_list'])
       season_entity['episode_air_list']=h(season_entity['episode_air_list'])
       season_entity['duplicate_list']=h(season_entity['duplicate_list'])
       ...
       msg=''
       if season_entity['daum_info']is not s:
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in T(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in T(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not s and season_entity['daum_info']['last_episode_no']is not s:
         if one_file_how_many_episodes==1:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,season_entity['daum_info']['last_episode_no'])
         else:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,P(season_entity['daum_info']['last_episode_no'])-1)
         if G(epi_max)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         elif one_file_how_many_episodes==2 and G(epi_max+1)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         else:
     ...
     for tag_video in videos:
      ...
      try:
       ...
       if 'duration' not in tag_media.attrib:
        continue
       duration=P(tag_media.attrib['duration'])
       ...
       channel_tag.set('id',G(channel_number))
       ...
       display_name_tag.text=G(channel_number)
       ...
       program_tag.set('channel',G(channel_number))
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not s:
        root.remove(channel_tag)
       if program_tag is not s:
        root.remove(channel_tag)
  except r as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
