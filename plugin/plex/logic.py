...
class Logic(A):
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.l=='show' and F(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not f else os.path.join(entity.scan_abspath,entity.filename)
     ...
     try:
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=f
       if item['guid'].lower().find('daum'):
        tmp=f
        if flag_media_season and season_data is not f and h(season_data['series'])>1:
       ...
       for episode in episodes:
        ...
        if episode.index is f:
         ...
         flag_index=H
         if epi_min is f or epi_min>F(episode.index):
          epi_min=F(episode.index)
         if epi_max is f or epi_max<F(episode.index):
       ...
       msg=''
       if season_entity['daum_info']is not f:
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in i(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in i(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not f and season_entity['daum_info']['last_episode_no']is not f:
         if one_file_how_many_episodes==1:
         ...
         if C(epi_max)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         elif one_file_how_many_episodes==2 and C(epi_max+1)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         else:
    ...
    if info['type']=='recent_add':
     ...
     for tag_video in videos:
      ...
      try:
       ...
       channel_tag.set('id',C(channel_number))
       ...
       display_name_tag.text=C(channel_number)
       ...
       program_tag.set('channel',C(channel_number))
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not f:
        root.remove(channel_tag)
       if program_tag is not f:
        root.remove(channel_tag)
  except j as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
