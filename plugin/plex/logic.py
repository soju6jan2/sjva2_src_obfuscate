...
class Logic(r):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @W
 def get_section_id(entity,more=n):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.S=='show' and R(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not J else os.path.join(entity.scan_abspath,entity.filename)
     ...
     try:
      ...
      if o(show.seasons())>1:
       for season in show.seasons():
        if R(season.index)>1 and R(season.index)<1900:
         flag_media_season=k
         break
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=J
       if item['guid'].lower().find('daum'):
        tmp=J
        if flag_media_season and season_data is not J and o(season_data['series'])>1:
         search_title=season_data['series'][R(season.index)-1]['title']
         search_id=season_data['series'][R(season.index)-1]['id']
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=k)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=k)
        if tmp:
       ...
       for episode in episodes:
        episode_key=J
        if episode.index is J:
         episode_key=episode.originallyAvailableAt.strftime('%Y-%m-%d')
         flag_originallyAvailableAt=k
         season_entity['episode_air_list'].append(episode_key)
         epi_count_date+=1
        else:
         episode_key=episode.index
         flag_index=k
         if epi_min is J or epi_min>R(episode.index):
          epi_min=R(episode.index)
         if epi_max is J or epi_max<R(episode.index):
          epi_max=R(episode.index)
         season_entity['episode_index_list'].append(R(episode.index))
         epi_count_index+=1
       ...
       season_entity['episode_index_list']=b(season_entity['episode_index_list'])
       season_entity['episode_air_list']=b(season_entity['episode_air_list'])
       season_entity['duplicate_list']=b(season_entity['duplicate_list'])
       ...
       msg=''
       if season_entity['daum_info']is not J:
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in e(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in e(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not J and season_entity['daum_info']['last_episode_no']is not J:
         if one_file_how_many_episodes==1:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,season_entity['daum_info']['last_episode_no'])
         else:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,R(season_entity['daum_info']['last_episode_no'])-1)
         if d(epi_max)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         elif one_file_how_many_episodes==2 and d(epi_max+1)==season_entity['daum_info']['last_episode_no']:
          msg+=' 일치'
         else:
     ...
     for tag_video in videos:
      ...
      try:
       ...
       if 'duration' not in tag_media.attrib:
        continue
       duration=R(tag_media.attrib['duration'])
       ...
       channel_tag.set('id',d(channel_number))
       ...
       display_name_tag.text=d(channel_number)
       ...
       program_tag.set('channel',d(channel_number))
       ...
       logger.error('Exception:%s',exception)
       logger.error(traceback.format_exc())
       if channel_tag is not J:
        root.remove(channel_tag)
       if program_tag is not J:
        root.remove(channel_tag)
  except K as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
