...
class Logic(V):
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.O=='show' and l(section.key)==entity.plex_section_id:
      ...
      for season in show.seasons():
       ...
       if item['guid'].lower().find('daum'):
        ...
        if flag_media_season and season_data is not A and q(season_data['series'])>1:
         ...
         tmp=DaumTV.get_daum_tv_info(search_title,daum_id=search_id,on_home=N)
        else:
         tmp=DaumTV.get_daum_tv_info(show.title,on_home=N)
        if tmp:
       ...
       for episode in episodes:
        ...
        if episode.index is A:
         ...
         flag_index=N
         if epi_min is A or epi_min>l(episode.index):
          epi_min=l(episode.index)
         if epi_max is A or epi_max<l(episode.index):
