...
class Logic(O):
 ...
 def get_sj_daum_version(req):
  try:
   ...
   return 'fail'
 @P
 def get_section_id(entity,more=f):
  try:
 ...
 def exist_file_in_library(entity):
  sections=Logic.server.library.sections()
  for section in sections:
   if section.b=='show' and C(section.key)==entity.plex_section_id:
 ...
 def send_scan_command(modelfile,plugin_name):
  ...
  try:
   ...
   filename=entity.plex_abspath if entity.plex_abspath is not q else os.path.join(entity.scan_abspath,entity.filename)
     ...
     try:
      ...
      if N(show.seasons())>1:
       for season in show.seasons():
        if C(season.index)>1 and C(season.index)<1900:
         flag_media_season=m
         break
      ...
      for season in show.seasons():
       if season.index==0:
        continue
       season_entity={}
       season_entity['daum_info']=q
       if item['guid'].lower().find('daum'):
        tmp=q
        if flag_media_season and season_data is not q and N(season_data['series'])>1:
         search_title=season_data['series'][C(season.index)-1]['title']
         search_id=season_data['series'][C(season.index)-1]['id']
       ...
       season_entity['episode_count']=N(episodes)
       ...
       for episode in episodes:
        ...
        if episode.index is q:
         ...
         flag_index=m
         if epi_min is q or epi_min>C(episode.index):
          epi_min=C(episode.index)
         if epi_max is q or epi_max<C(episode.index):
          epi_max=C(episode.index)
         season_entity['episode_index_list'].append(C(episode.index))
         epi_count_index+=1
        ...
        if N(season_entity['episodes'][episode_key])>1:
       ...
       season_entity['episode_index_list']=K(season_entity['episode_index_list'])
       season_entity['episode_air_list']=K(season_entity['episode_air_list'])
       season_entity['duplicate_list']=K(season_entity['duplicate_list'])
       ...
       msg=''
       if season_entity['daum_info']is not q:
       ...
       if flag_index:
        if one_file_how_many_episodes==1:
         if epi_max-epi_min+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in j(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],1):
         ...
         if(epi_max-epi_min)/one_file_how_many_episodes+1==epi_count_index:
          ...
          msg='비어 있는 에피소드 있음'
          empty_episode_no=[]
          for idx in j(season_entity['episode_index_list'][0],season_entity['episode_index_list'][-1],one_file_how_many_episodes):
           if idx not in season_entity['episode_index_list']:
            empty_episode_no.append(idx)
          season_entity['empty_episode_no']=empty_episode_no
         else:
        ...
        if season_entity['daum_info']is not q and season_entity['daum_info']['last_episode_no']is not q:
         if one_file_how_many_episodes==1:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,season_entity['daum_info']['last_episode_no'])
         else:
          msg+='<br>마지막 회차 - PLEX:%s, DAUM:%s.'%(epi_max,C(season_entity['daum_info']['last_episode_no'])-1)
