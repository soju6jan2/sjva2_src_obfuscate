   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=D if u(seasons)>1 else X
      for s in seasons:
 ...
 def make_list(data,root,videos,info,channel_title,include_parent=X):
  ...
  for tag_video in videos:
   ...
   try:
    ...
    channel_tag.set('id',x(LogicM3U.channel_index))
    ...
    display_name_tag.text=x(LogicM3U.channel_index)
    ...
    for i in M(3):
     ...
     program_tag.set('channel',x(LogicM3U.channel_index))
