   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=j if F(seasons)>1 else m
      for s in seasons:
      ...
      videos=doc.xpath("//video")
      data,root=LogicM3U.make_list(data,root,videos,info,channel_title,include_parent=j)
   tree=ET.ElementTree(root)
   ret=ET.tostring(root,pretty_print=j,xml_declaration=j,encoding="utf-8")
   return data,ret
  except Y as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
 @X
 def make_list(data,root,videos,info,channel_title,include_parent=m):
  server_url=ModelSetting.get('server_url')
  server_token=ModelSetting.get('server_token')
  current_count=0
  for tag_video in videos:
   ...
   try:
    ...
    channel_tag.set('id',Q(LogicM3U.channel_index))
    ...
    display_name_tag.text=Q(LogicM3U.channel_index)
    ...
    for i in H(3):
     ...
     program_tag.set('channel',Q(LogicM3U.channel_index))
