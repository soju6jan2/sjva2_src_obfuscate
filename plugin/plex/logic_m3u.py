...
class LogicM3U(U):
 ...
 def make_m3u():
  try:
   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=V if R(seasons)>1 else J
      for s in seasons:
   ...
   ret=ET.tostring(root,pretty_print=V,xml_declaration=V,encoding="utf-8")
   return data,ret
  except k as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc()) 
 @F
 def make_list(data,root,videos,info,channel_title,include_parent=J):
  server_url=ModelSetting.get('server_url')
  server_token=ModelSetting.get('server_token')
  current_count=0
  for tag_video in videos:
   ...
   try:
    ...
    channel_tag.set('id',h(LogicM3U.channel_index))
    ...
    display_name_tag.text=h(LogicM3U.channel_index)
    ...
    for i in D(3):
     ...
     program_tag.set('channel',h(LogicM3U.channel_index))
