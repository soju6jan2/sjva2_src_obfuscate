   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=w if e(seasons)>1 else M
      for s in seasons:
      ...
      videos=doc.xpath("//video")
      data,root=LogicM3U.make_list(data,root,videos,info,channel_title,include_parent=w)
   tree=ET.ElementTree(root)
   ret=ET.tostring(root,pretty_print=w,xml_declaration=w,encoding="utf-8")
   return data,ret
