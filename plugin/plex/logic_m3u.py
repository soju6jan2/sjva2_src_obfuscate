   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=H if h(seasons)>1 else x
      for s in seasons:
      ...
      videos=doc.xpath("//video")
      data,root=LogicM3U.make_list(data,root,videos,info,channel_title,include_parent=H)
   tree=ET.ElementTree(root)
   ret=ET.tostring(root,pretty_print=H,xml_declaration=H,encoding="utf-8")
   return data,ret
 ...
 def make_list(data,root,videos,info,channel_title,include_parent=x):
  ...
  for tag_video in videos:
   ...
   try:
    ...
    channel_tag.set('id',C(LogicM3U.channel_index))
    ...
    display_name_tag.text=C(LogicM3U.channel_index)
    ...
    for i in i(3):
     ...
     program_tag.set('channel',C(LogicM3U.channel_index))
