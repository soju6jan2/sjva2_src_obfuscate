   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=F if J(seasons)>1 else I
      for s in seasons:
      ...
      videos=doc.xpath("//video")
      data,root=LogicM3U.make_list(data,root,videos,info,channel_title,include_parent=F)
   tree=ET.ElementTree(root)
   ret=ET.tostring(root,pretty_print=F,xml_declaration=F,encoding="utf-8")
   return data,ret
  except S as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc()) 
 @c
 def make_list(data,root,videos,info,channel_title,include_parent=I):
  server_url=ModelSetting.get('server_url')
  server_token=ModelSetting.get('server_token')
  current_count=0
