   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=f if S(seasons)>1 else U
      for s in seasons:
 ...
 def make_list(data,root,videos,info,channel_title,include_parent=U):
  ...
  for tag_video in videos:
   ...
   try:
    ...
    channel_tag.set('id',F(LogicM3U.channel_index))
    ...
    display_name_tag.text=F(LogicM3U.channel_index)
    ...
    for i in u(3):
     ...
     program_tag.set('channel',F(LogicM3U.channel_index))
