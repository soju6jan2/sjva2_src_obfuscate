   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=B if O(seasons)>1 else l
      for s in seasons:
      ...
      videos=doc.xpath("//video")
      data,root=LogicM3U.make_list(data,root,videos,info,channel_title,include_parent=B)
   tree=ET.ElementTree(root)
   ret=ET.tostring(root,pretty_print=B,xml_declaration=B,encoding="utf-8")
   return data,ret
