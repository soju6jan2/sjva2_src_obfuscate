   ...
   for info in json_info:
    if info['type']=='recent_add':
     ...
     if seasons:
      ...
      include_parent=P if F(seasons)>1 else y
      for s in seasons:
      ...
      videos=doc.xpath("//video")
      data,root=LogicM3U.make_list(data,root,videos,info,channel_title,include_parent=P)
   tree=ET.ElementTree(root)
   ret=ET.tostring(root,pretty_print=P,xml_declaration=P,encoding="utf-8")
   return data,ret
 ...
 def make_list(data,root,videos,info,channel_title,include_parent=y):
  ...
  for tag_video in videos:
   ...
   try:
    ...
    channel_tag.set('id',w(LogicM3U.channel_index))
    ...
    display_name_tag.text=w(LogicM3U.channel_index)
    ...
    for i in x(3):
     ...
     program_tag.set('channel',w(LogicM3U.channel_index))
