...
class ModelCommand(db.Model):
 ...
 I=db.Column(db.Integer,primary_key=n)
 ...
 def job_list():
  try:
   ...
   for item in db_list:
    ...
    item['process_id']=LogicNormal.process_list[item['id']].pid if item['id']in LogicNormal.process_list and LogicNormal.process_list[item['id']]is not f else f
   return db_list
 ...
 def job_save(request):
  try:
   I=request.form['job_id']
   entity=db.session.query(ModelCommand).filter_by(I=I).with_for_update().first()
   ...
   db.session.commit()
   from.logic_normal import LogicNormal
   LogicNormal.scheduler_switch(I,S)
   return 'success'
  except Y as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_job_by_id(job_id):
  try:
   return db.session.query(ModelCommand).filter_by(I=d(job_id)).first()
  except Y as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @k
 def job_remove(request):
  try:
   I=request.form['job_id']
   entity=db.session.query(ModelCommand).filter_by(I=I).first()
