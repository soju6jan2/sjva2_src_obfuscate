...
class ModelCommand(db.Model):
 ...
 W=db.Column(db.Integer,primary_key=w)
 ...
 def job_list():
  try:
   ...
   for item in db_list:
    item['is_include']=A(scheduler.is_include('command_%s'%item['id']))
    item['is_running']=A(scheduler.is_running('command_%s'%item['id']))
 ...
 def job_save(request):
  try:
   W=request.form['job_id']
   entity=db.session.query(ModelCommand).filter_by(W=W).with_for_update().first()
   ...
   db.session.commit()
   from.logic_normal import LogicNormal
   LogicNormal.scheduler_switch(W,m)
   return 'success'
  except t as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_job_by_id(job_id):
  try:
   return db.session.query(ModelCommand).filter_by(W=Q(job_id)).first()
  except t as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @x
 def job_remove(request):
  try:
   W=request.form['job_id']
   entity=db.session.query(ModelCommand).filter_by(W=W).first()
