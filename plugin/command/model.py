...
class ModelCommand(db.Model):
 ...
 U=db.Column(db.Integer,primary_key=W)
 ...
 def job_list():
  try:
   ...
   for item in db_list:
    item['is_include']=B(scheduler.is_include('command_%s'%item['id']))
    item['is_running']=B(scheduler.is_running('command_%s'%item['id']))
    item['process_id']=LogicNormal.process_list[item['id']].pid if item['id']in LogicNormal.process_list and LogicNormal.process_list[item['id']]is not w else w
   return db_list
  except z as e:
 ...
 def job_save(request):
  try:
   ...
   db.session.commit()
   from.logic_normal import LogicNormal
   LogicNormal.scheduler_switch(U,q)
   return 'success'
  except z as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 ...
 def get_job_by_id(job_id):
  try:
   return db.session.query(ModelCommand).filter_by(U=h(job_id)).first()
  except z as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 @a
