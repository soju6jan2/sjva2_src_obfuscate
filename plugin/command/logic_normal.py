...
class LogicNormal(G):
   ...
   try:
    ...
    for item in db_list:
     if '%s'%item.schedule_type=='1':
      ...
      LogicNormal.scheduler_switch(item.B,M)
   except V as e:
    logger.error('Exception:%s',e)
    logger.error(traceback.format_exc()) 
  try:
 ...
 def plugin_unload():
  try:
   ...
   logger.error(traceback.format_exc())
 @Q
 def foreground_command(command,job_id=v):
  try:
   ...
   if command[0]=='LOAD':
    ...
    if LogicNormal.foreground_process is not v:
     LogicNormal.foreground_command_close()
     time.sleep(0.5)
 ...
 def scheduler_switch(B,switch):
  try:
   ...
   s_id='command_%s'%B
   if switch:
    job_instance=Job(package_name,s_id,job.schedule_info,LogicNormal.execute_thread_function_by_scheduler,u"%s %s : %s"%(package_name,job.B,job.description),M,args=job.B)
 ...
 def execute_thread_function(command,command_id=-1):
  try:
   ...
   if command[0]=='LOAD':
    ...
    if command_id!=-1:
     command_logger=get_logger('%s_%s'%(package_name,command_id))
     if command_id in LogicNormal.process_list and LogicNormal.process_list[command_id]is not v:
 ...
 def execute_thread_function_by_scheduler(*args,**kwargs):
  try:
   ...
   job=db.session.query(ModelCommand).filter_by(B=z(args[0])).first()
   LogicNormal.execute_thread_function(job.command,command_id=job.B)
 ...
 def start_communicate2(process):
  ...
  sout=io.open(process.stdout.fileno(),'rb',closefd=h)
  def Pump(stream):
 ...
 def start_communicate_load():
  ...
  def func():
   position=0
   flag=M
   while LogicNormal.command_queue is not v:
 ...
 def send_queue_start():
  def send_queue_thread_function():
   ...
   while LogicNormal.command_queue:
    ...
    if line=='<END>':
     socketio.emit("end",v,namespace='/%s'%package_name,broadcast=M)
     break
    else:
     socketio.emit("add",line,namespace='/%s'%package_name,broadcast=M)
   LogicNormal.send_queue_thread=v
   LogicNormal.command_queue=v
   LogicNormal.foreground_process=v
   logger.debug('send_queue_thread_function END')
  if LogicNormal.send_queue_thread is v:
