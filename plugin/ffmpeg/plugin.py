...
def detail(sub):
 if sub=='setting':
  ...
  now=C(datetime.now(timezone('Asia/Seoul'))).replace(':','').replace('-','').replace(' ','-')
...
def api(sub):
 ...
 if sub=='download':
  ...
  try:
   ...
   f=Ffmpeg(url,filename,plugin_id=caller_id,listener=Y,max_pf_count=max_pf_count,call_plugin=package_name,save_path=save_path)
   f.start()
   ret['ret']='success'
   ret['data']=f.get_data()
  except A as exception:
