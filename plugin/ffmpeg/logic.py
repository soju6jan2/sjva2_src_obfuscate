...
class Status(x):
 ...
 def get_instance(value):
  ...
  return tmp[value]
class Logic(C):
 db_default={'temp_path':P.join(path_data,'download_tmp'),'save_path':P.join(path_data,'download'),'max_pf_count':'0','if_fail_remove_tmp_file':'True','timeout_minute':'60',}
 path_ffmpeg=A
 @j
 def db_init():
  try:
 ...
 def plugin_load():
  try:
   Logic.db_init()
   Logic.path_ffmpeg=P.join(path_app_root,'bin',S(),'ffmpeg')
   if S()=='Windows':
 ...
 def get_setting_value(key):
  try:
   return c.query(ModelSetting).filter_by(key=key).first().value
  except f as e:
   logger.error('Exception:%s',e)
   logger.error(N())
 @j
 def ffmpeg_listener(**args):
  ...
  if args['type']=='status_change':
   if args['status']==Status.DOWNLOADING:
    ...
    V("notify",data,namespace='/framework',broadcast=t)
    refresh_type='add' 
  elif args['type']=='last':
   ...
   if args['status']==Status.DOWNLOADING:
    refresh_type='status'
  if refresh_type is not A:
   V(refresh_type,args['data'],namespace='/%s'%package_name,broadcast=t)
