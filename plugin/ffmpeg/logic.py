 ...
 def ffmpeg_listener(**args):
  ...
  if args['type']=='status_change':
   if args['status']==Status.DOWNLOADING:
    ...
    socketio.emit("notify",data,namespace='/framework',broadcast=E)
    refresh_type='add' 
  elif args['type']=='last':
   ...
   if args['status']==Status.DOWNLOADING:
    refresh_type='status'
  if refresh_type is not N:
   socketio.emit(refresh_type,args['data'],namespace='/%s'%package_name,broadcast=E)
