...
class Status(m):
 ...
 def get_instance(value):
  ...
  return tmp[value]
class Logic(L):
 db_default={'temp_path':x.join(path_data,'download_tmp'),'save_path':x.join(path_data,'download'),'max_pf_count':'0','if_fail_remove_tmp_file':'True','timeout_minute':'60',}
 path_ffmpeg=E
 @w
 def db_init():
  try:
   for key,value in Logic.db_default.items():
    if M.query(ModelSetting).filter_by(key=key).count()==0:
     M.add(ModelSetting(key,value))
   M.commit()
  except q as e:
 ...
 def plugin_load():
  try:
   Logic.db_init()
   Logic.path_ffmpeg=x.join(path_app_root,'bin',p(),'ffmpeg')
   if p()=='Windows':
 ...
 def setting_save(req):
  try:
   for key,value in req.form.items():
    ...
    entity=M.query(ModelSetting).filter_by(key=key).with_for_update().first()
    entity.value=value
   M.commit()
   return k 
  except q as e:
 ...
 def get_setting_value(key):
  try:
   return M.query(ModelSetting).filter_by(key=key).first().value
  except q as e:
   logger.error('Exception:%s',e)
   logger.error(T())
 @w
 def ffmpeg_listener(**args):
  ...
  if args['type']=='status_change':
   if args['status']==Status.DOWNLOADING:
    ...
    S("notify",data,namespace='/framework',broadcast=k)
    refresh_type='add' 
  elif args['type']=='last':
   ...
   if args['status']==Status.DOWNLOADING:
    refresh_type='status'
  if refresh_type is not E:
   S(refresh_type,args['data'],namespace='/%s'%package_name,broadcast=k)
