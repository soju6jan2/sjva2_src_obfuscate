...
class Ffmpeg(f):
 instance_list=[]
 idx=1
 def __init__(self,url,filename,plugin_id=k,listener=k,max_pf_count=k,call_plugin=k,temp_path=k,save_path=k,proxy=k,headers=k):
  self.thread=k
  self.url=url
  self.filename=filename
  self.plugin_id=plugin_id
  self.listener=listener
  self.max_pf_count=V(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is k else max_pf_count)
  self.call_plugin=call_plugin
  self.process=k
  self.temp_path=ModelSetting.query.filter_by(key='temp_path').first().value if temp_path is k else temp_path
  self.save_path=ModelSetting.query.filter_by(key='save_path').first().value if save_path is k else save_path
  self.proxy=proxy
  ...
  logger.debug('Ffmpeg.instance_list LEN:%s',C(Ffmpeg.instance_list))
  if C(Ffmpeg.instance_list)>30:
   for f in Ffmpeg.instance_list:
    if f.thread is k and f.status!=Status.READY:
 ...
 def kill(self):
  try:
   if self.process is not k and self.process.poll()is k:
    import psutil
    process=psutil.Process(self.process.pid)
    for proc in process.children(recursive=R):
     proc.kill()
    process.kill()
  except I as exception:
   logger.error('Exception:%s',exception)
 ...
 def thread_fuction(self):
  try:
   ...
   try:
    ...
    pass
   self.process=subprocess.Popen(command,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=R,bufsize=1)
   self.status=Status.READY
 ...
 def log_thread_fuction(self):
  with self.process.stdout:
   for line in Y(self.process.stdout.readline,b''):
    try:
     if self.status==Status.READY:
  ...
  self.log_thread=k
 def get_data(self):
  data={'url':self.url,'filename':self.filename,'max_pf_count':self.max_pf_count,'call_plugin':self.call_plugin,'temp_path':self.temp_path,'save_path':self.save_path,'temp_fullpath':self.temp_fullpath,'save_fullpath':self.save_fullpath,'status':V(self.status),'status_str':self.status.name,'status_kor':W(self.status),'duration':self.duration,'duration_str':self.duration_str,'current_duration':self.current_duration,'percent':self.percent,'current_pf_count':self.current_pf_count,'idx':self.idx,'current_bitrate':self.current_bitrate,'current_speed':self.current_speed,'start_time':'' if self.start_time is k else W(self.start_time).split('.')[0][5:],'end_time':'' if self.end_time is k else W(self.end_time).split('.')[0][5:],'download_time':'' if self.download_time is k else '%02d:%02d'%(self.download_time.seconds/60,self.download_time.seconds%60),'exist':os.path.exists(self.save_fullpath),} 
  if self.status==Status.COMPLETED:
 ...
 def get_version():
  try:
   ...
   ret=[]
   with process.stdout:
    for line in Y(process.stdout.readline,b''):
     ret.append(line)
    process.wait()
   return ret
  except I as exception:
