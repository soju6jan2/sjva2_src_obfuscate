...
class Ffmpeg(m):
 instance_list=[]
 idx=1
 def __init__(self,url,filename,plugin_id=R,listener=R,max_pf_count=R,call_plugin=R,temp_path=R,save_path=R,proxy=R,headers=R):
  self.thread=R
  self.url=url
  self.filename=filename
  self.plugin_id=plugin_id
  self.listener=listener
  self.max_pf_count=s(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is R else max_pf_count)
  self.call_plugin=call_plugin
  self.process=R
  self.temp_path=ModelSetting.query.filter_by(key='temp_path').first().value if temp_path is R else temp_path
  self.save_path=ModelSetting.query.filter_by(key='save_path').first().value if save_path is R else save_path
  self.proxy=proxy
  ...
  self.idx=a(Ffmpeg.idx)
  Ffmpeg.idx+=1
  self.current_bitrate=''
  self.current_speed=''
  self.start_time=R
  self.end_time=R
  self.download_time=R
  ...
  logger.debug('Ffmpeg.instance_list LEN:%s',d(Ffmpeg.instance_list))
  if d(Ffmpeg.instance_list)>30:
   for f in Ffmpeg.instance_list:
    if f.thread is R and f.status!=Status.READY:
 ...
 def kill(self):
  try:
   if self.process is not R and self.process.poll()is R:
    import psutil
 ...
 def thread_fuction(self):
  try:
   import system
   user=a(system.ModelSetting.get('sjva_me_user_id'))
   try:
   ...
   if platform.system()=='Windows':
    now=a(datetime.now()).replace(':','').replace('-','').replace(' ','-')
    filename=('%s'%now)+'.mp4'
    self.temp_fullpath=os.path.join(self.temp_path,filename)
 ...
 def log_thread_fuction(self):
  ...
  self.log_thread=R
 def get_data(self):
  data={'url':self.url,'filename':self.filename,'max_pf_count':self.max_pf_count,'call_plugin':self.call_plugin,'temp_path':self.temp_path,'save_path':self.save_path,'temp_fullpath':self.temp_fullpath,'save_fullpath':self.save_fullpath,'status':s(self.status),'status_str':self.status.name,'status_kor':a(self.status),'duration':self.duration,'duration_str':self.duration_str,'current_duration':self.current_duration,'percent':self.percent,'current_pf_count':self.current_pf_count,'idx':self.idx,'current_bitrate':self.current_bitrate,'current_speed':self.current_speed,'start_time':'' if self.start_time is R else a(self.start_time).split('.')[0][5:],'end_time':'' if self.end_time is R else a(self.end_time).split('.')[0][5:],'download_time':'' if self.download_time is R else '%02d:%02d'%(self.download_time.seconds/60,self.download_time.seconds%60),'exist':os.path.exists(self.save_fullpath),} 
  if self.status==Status.COMPLETED:
