...
class Ffmpeg(W):
 ...
 def __init__(self,url,filename,plugin_id=Y,listener=Y,max_pf_count=Y,call_plugin=Y,temp_path=Y,save_path=Y,proxy=Y,headers=Y):
  ...
  self.max_pf_count=L(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is Y else max_pf_count)
  ...
  logger.debug('Ffmpeg.instance_list LEN:%s',X(Ffmpeg.instance_list))
  if X(Ffmpeg.instance_list)>30:
 ...
 def thread_fuction(self):
  try:
   import system
   user=C(system.ModelSetting.get('sjva_me_user_id'))
   try:
   ...
   if platform.system()=='Windows':
    now=C(datetime.now()).replace(':','').replace('-','').replace(' ','-')
    filename=('%s'%now)+'.mp4'
    self.temp_fullpath=os.path.join(self.temp_path,filename)
 ...
 def log_thread_fuction(self):
     ...
     if self.status==Status.READY:
      if line.find('Server returned 404 Not Found')!=-1 or line.find('Unknown error')!=-1:
       ...
       if match:
        ...
        self.duration=L(match.group(4))
        self.duration+=L(match.group(3))*100
        self.duration+=L(match.group(2))*100*60
        self.duration+=L(match.group(1))*100*60*60
      ...
      match=re.compile(r'time\=(\d{2})\:(\d{2})\:(\d{2})\.(\d{2})\sbitrate\=\s*(?P<bitrate>\d+).*?[$|\s](\s?speed\=\s*(?P<speed>.*?)x)?').search(line)
      if match:
       self.current_duration=L(match.group(4))
       self.current_duration+=L(match.group(3))*100
       self.current_duration+=L(match.group(2))*100*60
       self.current_duration+=L(match.group(1))*100*60*60
       self.percent=L(self.current_duration*100/self.duration)
  ...
  self.log_thread=Y
 def get_data(self):
  data={'url':self.url,'filename':self.filename,'max_pf_count':self.max_pf_count,'call_plugin':self.call_plugin,'temp_path':self.temp_path,'save_path':self.save_path,'temp_fullpath':self.temp_fullpath,'save_fullpath':self.save_fullpath,'status':L(self.status),'status_str':self.status.name,'status_kor':C(self.status),'duration':self.duration,'duration_str':self.duration_str,'current_duration':self.current_duration,'percent':self.percent,'current_pf_count':self.current_pf_count,'idx':self.idx,'current_bitrate':self.current_bitrate,'current_speed':self.current_speed,'start_time':'' if self.start_time is Y else C(self.start_time).split('.')[0][5:],'end_time':'' if self.end_time is Y else C(self.end_time).split('.')[0][5:],'download_time':'' if self.download_time is Y else '%02d:%02d'%(self.download_time.seconds/60,self.download_time.seconds%60),'exist':os.path.exists(self.save_fullpath),} 
  if self.status==Status.COMPLETED:
