...
class Ffmpeg(M):
 ...
 def __init__(self,url,filename,plugin_id=G,listener=G,max_pf_count=G,call_plugin=G,temp_path=G,save_path=G,proxy=G,headers=G):
  ...
  self.max_pf_count=g(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is G else max_pf_count)
 ...
 def kill(self):
  try:
   if self.process is not G and self.process.poll()is G:
    ...
    for proc in process.children(recursive=i):
     proc.kill()
    process.kill()
  except m as e:
   logger.error('Exception:%s',e)
 ...
 def thread_fuction(self):
  try:
   import system
   user=H(system.ModelSetting.get('sjva_me_user_id'))
   try:
   ...
   if platform.system()=='Windows':
    now=H(datetime.now()).replace(':','').replace('-','').replace(' ','-')
    filename=('%s'%now)+'.mp4'
    self.temp_fullpath=os.path.join(self.temp_path,filename)
   ...
   try:
    ...
    pass
   self.process=subprocess.Popen(command,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=i,bufsize=1)
   self.status=Status.READY
 ...
 def log_thread_fuction(self):
  ...
  self.log_thread=G
 def get_data(self):
  data={'url':self.url,'filename':self.filename,'max_pf_count':self.max_pf_count,'call_plugin':self.call_plugin,'temp_path':self.temp_path,'save_path':self.save_path,'temp_fullpath':self.temp_fullpath,'save_fullpath':self.save_fullpath,'status':g(self.status),'status_str':self.status.name,'status_kor':H(self.status),'duration':self.duration,'duration_str':self.duration_str,'current_duration':self.current_duration,'percent':self.percent,'current_pf_count':self.current_pf_count,'idx':self.idx,'current_bitrate':self.current_bitrate,'current_speed':self.current_speed,'start_time':'' if self.start_time is G else H(self.start_time).split('.')[0][5:],'end_time':'' if self.end_time is G else H(self.end_time).split('.')[0][5:],'download_time':'' if self.download_time is G else '%02d:%02d'%(self.download_time.seconds/60,self.download_time.seconds%60),'exist':os.path.exists(self.save_fullpath),} 
  if self.status==Status.COMPLETED:
