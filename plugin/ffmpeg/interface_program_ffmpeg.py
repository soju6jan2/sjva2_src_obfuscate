...
class Ffmpeg(B):
 instance_list=[]
 idx=1
 def __init__(self,url,filename,plugin_id=V,listener=V,max_pf_count=V,call_plugin=V,temp_path=V,save_path=V,proxy=V,headers=V):
  self.thread=V
  self.url=url
  self.filename=filename
  self.plugin_id=plugin_id
  self.listener=listener
  self.max_pf_count=D(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is V else max_pf_count)
  self.call_plugin=call_plugin
  self.process=V
  self.temp_path=ModelSetting.query.filter_by(key='temp_path').first().value if temp_path is V else temp_path
  self.save_path=ModelSetting.query.filter_by(key='save_path').first().value if save_path is V else save_path
  self.proxy=proxy
  ...
  self.idx=W(Ffmpeg.idx)
  Ffmpeg.idx+=1
  self.current_bitrate=''
  self.current_speed=''
  self.start_time=V
  self.end_time=V
  self.download_time=V
  ...
  logger.debug('Ffmpeg.instance_list LEN:%s',f(Ffmpeg.instance_list))
  if f(Ffmpeg.instance_list)>30:
   for f in Ffmpeg.instance_list:
    if f.thread is V and f.status!=Status.READY:
 ...
 def kill(self):
  try:
   if self.process is not V and self.process.poll()is V:
    import psutil
    process=psutil.Process(self.process.pid)
    for proc in process.children(recursive=G):
     proc.kill()
    process.kill()
  except E as e:
   logger.error('Exception:%s',e)
 ...
 def thread_fuction(self):
  try:
   import system
   user=W(system.ModelSetting.get('sjva_me_user_id'))
   try:
   ...
   if platform.system()=='Windows':
    now=W(datetime.now()).replace(':','').replace('-','').replace(' ','-')
    filename=('%s'%now)+'.mp4'
    self.temp_fullpath=os.path.join(self.temp_path,filename)
   ...
   try:
    ...
    pass
   self.process=subprocess.Popen(command,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=G,bufsize=1)
   self.status=Status.READY
 ...
 def log_thread_fuction(self):
  with self.process.stdout:
   for line in P(self.process.stdout.readline,b''):
    try:
     if self.status==Status.READY:
  ...
  self.log_thread=V
 def get_data(self):
  data={'url':self.url,'filename':self.filename,'max_pf_count':self.max_pf_count,'call_plugin':self.call_plugin,'temp_path':self.temp_path,'save_path':self.save_path,'temp_fullpath':self.temp_fullpath,'save_fullpath':self.save_fullpath,'status':D(self.status),'status_str':self.status.name,'status_kor':W(self.status),'duration':self.duration,'duration_str':self.duration_str,'current_duration':self.current_duration,'percent':self.percent,'current_pf_count':self.current_pf_count,'idx':self.idx,'current_bitrate':self.current_bitrate,'current_speed':self.current_speed,'start_time':'' if self.start_time is V else W(self.start_time).split('.')[0][5:],'end_time':'' if self.end_time is V else W(self.end_time).split('.')[0][5:],'download_time':'' if self.download_time is V else '%02d:%02d'%(self.download_time.seconds/60,self.download_time.seconds%60),'exist':os.path.exists(self.save_fullpath),} 
  if self.status==Status.COMPLETED:
 ...
 def get_version():
  try:
   ...
   ret=[]
   with process.stdout:
    for line in P(process.stdout.readline,b''):
     ret.append(line)
    process.wait()
   return ret
  except E as e:
