...
class Ffmpeg(l):
 instance_list=[]
 idx=1
 def __init__(self,url,filename,plugin_id=t,listener=t,max_pf_count=t,call_plugin=t,temp_path=t,save_path=t,proxy=t,headers=t):
  self.thread=t
  self.url=url
  self.filename=filename
  self.plugin_id=plugin_id
  self.listener=listener
  self.max_pf_count=g(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is t else max_pf_count)
  self.call_plugin=call_plugin
  self.process=t
  self.temp_path=ModelSetting.query.filter_by(key='temp_path').first().value if temp_path is t else temp_path
  self.save_path=ModelSetting.query.filter_by(key='save_path').first().value if save_path is t else save_path
  self.proxy=proxy
  ...
  if M(Ffmpeg.instance_list)>30:
   for f in Ffmpeg.instance_list:
    if f.thread is t and f.status!=Status.READY:
 ...
 def kill(self):
  try:
   if self.process is not t and self.process.poll()is t:
    import psutil
 ...
 def log_thread_fuction(self):
     ...
     if self.status==Status.READY:
      if line.find('Server returned 404 Not Found')!=-1 or line.find('Unknown error')!=-1:
       ...
       if match:
        ...
        self.duration=g(match.group(4))
        self.duration+=g(match.group(3))*100
        self.duration+=g(match.group(2))*100*60
        self.duration+=g(match.group(1))*100*60*60
      ...
      match=re.compile(r'time\=(\d{2})\:(\d{2})\:(\d{2})\.(\d{2})\sbitrate\=\s*(?P<bitrate>\d+).*?[$|\s](\s?speed\=\s*(?P<speed>.*?)x)?').search(line)
      if match:
       self.current_duration=g(match.group(4))
       self.current_duration+=g(match.group(3))*100
       self.current_duration+=g(match.group(2))*100*60
       self.current_duration+=g(match.group(1))*100*60*60
       self.percent=g(self.current_duration*100/self.duration)
  ...
  self.log_thread=t
 def get_data(self):
  data={'url':self.url,'filename':self.filename,'max_pf_count':self.max_pf_count,'call_plugin':self.call_plugin,'temp_path':self.temp_path,'save_path':self.save_path,'temp_fullpath':self.temp_fullpath,'save_fullpath':self.save_fullpath,'status':g(self.status),'status_str':self.status.name,'status_kor':F(self.status),'duration':self.duration,'duration_str':self.duration_str,'current_duration':self.current_duration,'percent':self.percent,'current_pf_count':self.current_pf_count,'idx':self.idx,'current_bitrate':self.current_bitrate,'current_speed':self.current_speed,'start_time':'' if self.start_time is t else F(self.start_time).split('.')[0][5:],'end_time':'' if self.end_time is t else F(self.end_time).split('.')[0][5:],'download_time':'' if self.download_time is t else '%02d:%02d'%(self.download_time.seconds/60,self.download_time.seconds%60),'exist':os.path.exists(self.save_fullpath),} 
  if self.status==Status.COMPLETED:
