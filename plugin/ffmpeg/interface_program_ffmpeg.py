...
class Ffmpeg(y):
 instance_list=[]
 idx=1
 def __init__(self,url,filename,plugin_id=m,listener=m,max_pf_count=m,call_plugin=m,temp_path=m,save_path=m,proxy=m,headers=m):
  self.thread=m
  self.url=url
  self.filename=filename
  self.plugin_id=plugin_id
  self.listener=listener
  self.max_pf_count=C(ModelSetting.query.filter_by(key='max_pf_count').first().value if max_pf_count is m else max_pf_count)
  self.call_plugin=call_plugin
  self.process=m
  self.temp_path=ModelSetting.query.filter_by(key='temp_path').first().value if temp_path is m else temp_path
  self.save_path=ModelSetting.query.filter_by(key='save_path').first().value if save_path is m else save_path
  self.proxy=proxy
  ...
  logger.debug('Ffmpeg.instance_list LEN:%s',q(Ffmpeg.instance_list))
  if q(Ffmpeg.instance_list)>30:
   for f in Ffmpeg.instance_list:
    if f.thread is m and f.status!=Status.READY:
 ...
 def kill(self):
  try:
   if self.process is not m and self.process.poll()is m:
    import psutil
    process=psutil.Process(self.process.pid)
    for proc in process.children(recursive=x):
     proc.kill()
    process.kill()
  except e as exception:
   logger.error('Exception:%s',exception)
 ...
 def thread_fuction(self):
  try:
   ...
   try:
    ...
    pass
   self.process=subprocess.Popen(command,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=x,bufsize=1)
   self.status=Status.READY
 ...
 def log_thread_fuction(self):
  with self.process.stdout:
   for line in L(self.process.stdout.readline,b''):
    try:
     if self.status==Status.READY:
 ...
 def get_version():
  try:
   ...
   process=subprocess.Popen(command,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,universal_newlines=x,bufsize=1)
   ret=[]
   with process.stdout:
