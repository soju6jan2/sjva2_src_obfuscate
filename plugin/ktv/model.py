...
class ModelKtvFile(db.Model):
 ...
 def __repr__(self):
  return i(self.as_dict())
 def as_dict(self):
  ret={x.name:h(self,x.name)for x in self.__table__.columns}
  ret['created_time']=self.created_time.strftime('%m-%d %H:%M:%S')if self.created_time is not o else ''
  ret['send_command_time']=self.send_command_time.strftime('%m-%d %H:%M:%S')if self.send_command_time is not o else ''
  ret['scan_time']=self.scan_time.strftime('%m-%d %H:%M:%S')if self.scan_time is not o else ''
  return ret
 @p
 def create(entity):
  try:
   ...
   f.move_type=q(entity.move_type)
   ...
   f.scan_status=q(entity.scan_status)
 ...
 def get_library_check_list():
  try:
   ...
   query=query.filter(or_(ModelKtvFile.send_command_time.is_(o),ModelKtvFile.send_command_time<datetime.datetime.now()+datetime.timedelta(hours=-1)))
 ...
 def get_image_empty_list():
  try:
   ...
   query=query.filter(ModelKtvFile.plex_image.is_(o))
