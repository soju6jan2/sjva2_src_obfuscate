...
class ModelKtvFile(db.Model):
 ...
 def create(entity):
  try:
   ...
   f.move_type=V(entity.move_type)
   ...
   f.scan_status=V(entity.scan_status)
 ...
 def get_library_check_list():
  try:
   ...
   query=query.filter(or_(ModelKtvFile.send_command_time.is_(d),ModelKtvFile.send_command_time<datetime.datetime.now()+datetime.timedelta(hours=-1)))
