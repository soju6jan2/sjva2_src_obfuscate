...
class ModelKtvFile(db.Model):
 ...
 def __repr__(self):
  return k(self.as_dict())
 def as_dict(self):
  ret={x.name:V(self,x.name)for x in self.__table__.columns}
  ret['created_time']=self.created_time.strftime('%m-%d %H:%M:%S')if self.created_time is not E else ''
  ret['send_command_time']=self.send_command_time.strftime('%m-%d %H:%M:%S')if self.send_command_time is not E else ''
  ret['scan_time']=self.scan_time.strftime('%m-%d %H:%M:%S')if self.scan_time is not E else ''
  return ret
 @R
 def create(entity):
  try:
 ...
 def get_library_check_list():
  try:
   ...
   query=query.filter(or_(ModelKtvFile.send_command_time.is_(E),ModelKtvFile.send_command_time<datetime.datetime.now()+datetime.timedelta(hours=-1)))
 ...
 def get_image_empty_list():
  try:
   ...
   query=query.filter(ModelKtvFile.plex_image.is_(E))
