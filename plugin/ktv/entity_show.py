...
class EntityLibraryPathRoot(b):
 ...
 depth=-1
 replace_for_plex=g
 def __init__(self,drive_type,mount_path,depth,rclone_path=g,sync_path=g,replace_for_plex=g):
...
class EntityShow(b):
 ...
 filename_more=''
 daum_info=g
 def __init__(self,filename,by=0,nd_download_path=g,daum_info=g,except_genre_remove_epi_number=g):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if X and self.daum_info is not g:
   ...
   self.change_filename_continous_episode(move=o)
   self.daum_info=daum_info
   self.change_filename_by_rule(move=o)
   ...
   if self.video_type==EntityShow.VideoType.KOREA_TV:
    self.daum_info=daum_tv.Logic.get_daum_tv_info(self.filename_name)
    if self.daum_info is not g:
     self.change_filename_by_rule(move=o)
   else:
    self.daum_info=g
 def analyze_filename(self):
  for idx,regex in p(self._REGEX_FILENAME):
   ...
   if match:
    ...
    if self.filename_no is not g and self.filename_no!='':
     self.filename_no=R(self.filename_no)
    else:
     self.filename_no=-1
    if idx==1:
     self.filename=EntityShow.make_filename(self)
    break
 def change_filename_continous_episode(self,move=X):
  ...
  if match:
   ...
   self.filename_no=R(match.group('no'))
   self.filename=EntityShow.make_filename(self)
   if move:
 ...
 def change_filename_by_rule(self,move=X):
  ...
  if self.daum_info.has_episode_info():
   ...
   if key in self.daum_info.episode_list:
    ...
    if self.filename_no!=-1:
     ...
     if flag:
      ...
      logger.debug(' - Daum 정보 Date:%s Count:%s No:%s',key,a(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
      self.log+=' - Daum 정보. 방송일:%s 회차:%s Count:%s\n'%(key,self.daum_info.episode_list[key][0],a(self.daum_info.episode_list[key]))
      logger.debug(' - episode_count_one_day : %s',self.daum_info.episode_count_one_day)
      if self.daum_info.episode_count_one_day==4:
       if self.filename_no*2 ==R(self.daum_info.episode_list[key][1]):
        self.filename_no=R(self.daum_info.episode_list[key][0])
       elif self.filename_no*2 ==R(self.daum_info.episode_list[key][3]):
        self.filename_no=R(self.daum_info.episode_list[key][2])
       flag_need_rename=X
      else:
       self.filename_no=R(self.daum_info.episode_list[key][0])
       flag_need_rename=X
     ...
     logger.debug(' - 파일명에 회차정보 삽입. Daum 정보 - date:%s count:%s %s',key,a(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
     self.filename_no=R(self.daum_info.episode_list[key][0])
     flag_need_rename=X
   else:
   ...
   if X and self.filename_no!=-1:
    if self.except_genre_remove_epi_number is not g and('all' in self.except_genre_remove_epi_number or self.daum_info.genre in self.except_genre_remove_epi_number):
 ...
 def make_filename(cls,_entity):
  ...
  if _entity.filename_release!='' and _entity.filename_release is not g:
 ...
 def set_find_library_path(self,nd_find_library_path):
  ...
  if nd_find_library_path.entity_library_root.drive_type==EntityLibraryPathRoot.DriveType.LOCAL:
   ...
   if nd_find_library_path.entity_library_root.replace_for_plex is not g:
