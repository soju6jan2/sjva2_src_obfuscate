...
class EntityShow(w):
 ...
 def __init__(self,filename,by=0,nd_download_path=X,daum_info=X,except_genre_remove_epi_number=X):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if c and self.daum_info is not X:
 ...
 def analyze_filename(self):
  for idx,regex in e(self._REGEX_FILENAME):
   ...
   if key in self.daum_info.episode_list:
    ...
    if self.filename_no!=-1:
     ...
     if flag:
      ...
      logger.debug(' - Daum 정보 Date:%s Count:%s No:%s',key,R(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
      self.log+=' - Daum 정보. 방송일:%s 회차:%s Count:%s\n'%(key,self.daum_info.episode_list[key][0],R(self.daum_info.episode_list[key]))
     ...
     logger.debug(' - 파일명에 회차정보 삽입. Daum 정보 - date:%s count:%s %s',key,R(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
     self.filename_no=j(self.daum_info.episode_list[key][0])
     flag_need_rename=c
   else:
