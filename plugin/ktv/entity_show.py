...
class EntityShow(X):
 ...
 def __init__(self,filename,by=0,nd_download_path=O,daum_info=O,except_genre_remove_epi_number=O):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if n and self.daum_info is not O:
  ...
  for idx,regex in W(self._REGEX_FILENAME):
   ...
   if match:
    ...
    if self.filename_no is not O and self.filename_no!='':
     self.filename_no=d(self.filename_no)
    else:
     self.filename_no=-1
    if idx==1:
 ...
 def change_filename_continous_episode(self,move=n):
  ...
  if match:
   ...
   self.filename_no=d(match.group('no'))
   self.filename=EntityShow.make_filename(self)
   if move:
    ...
    if self.filename_no!=-1:
     ...
     if flag:
      ...
      if self.daum_info.episode_count_one_day==4:
       if self.filename_no*2 ==d(self.daum_info.episode_list[key][1]):
        self.filename_no=d(self.daum_info.episode_list[key][0])
       elif self.filename_no*2 ==d(self.daum_info.episode_list[key][3]):
        self.filename_no=d(self.daum_info.episode_list[key][2])
       flag_need_rename=n
      else:
       self.filename_no=d(self.daum_info.episode_list[key][0])
       flag_need_rename=n
