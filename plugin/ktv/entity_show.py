...
class EntityLibraryPathRoot(c):
 ...
 depth=-1
 replace_for_plex=E
 def __init__(self,drive_type,mount_path,depth,rclone_path=E,sync_path=E,replace_for_plex=E):
...
class EntityShow(c):
 ...
 filename_more=''
 daum_info=E
 def __init__(self,filename,by=0,nd_download_path=E,daum_info=E,except_genre_remove_epi_number=E):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if f and self.daum_info is not E:
   ...
   self.change_filename_continous_episode(move=U)
   self.daum_info=daum_info
   self.change_filename_by_rule(move=U)
 ...
 def analyze_filename(self):
  for idx,regex in h(self._REGEX_FILENAME):
   ...
   if match:
    ...
    if self.filename_no is not E and self.filename_no!='':
     self.filename_no=o(self.filename_no)
    else:
     self.filename_no=-1
    if idx==1:
 ...
 def change_filename_continous_episode(self,move=f):
  ...
  if match:
   ...
   self.filename_no=o(match.group('no'))
   self.filename=EntityShow.make_filename(self)
   if move:
 ...
 def change_filename_by_rule(self,move=f):
  ...
  if self.daum_info.has_episode_info():
   ...
   if key in self.daum_info.episode_list:
    ...
    if self.filename_no!=-1:
     ...
     if flag:
      ...
      logger.debug(' - Daum 정보 Date:%s Count:%s No:%s',key,l(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
      self.log+=' - Daum 정보. 방송일:%s 회차:%s Count:%s\n'%(key,self.daum_info.episode_list[key][0],l(self.daum_info.episode_list[key]))
      logger.debug(' - episode_count_one_day : %s',self.daum_info.episode_count_one_day)
      if self.daum_info.episode_count_one_day==4:
       if self.filename_no*2 ==o(self.daum_info.episode_list[key][1]):
        self.filename_no=o(self.daum_info.episode_list[key][0])
       elif self.filename_no*2 ==o(self.daum_info.episode_list[key][3]):
        self.filename_no=o(self.daum_info.episode_list[key][2])
       flag_need_rename=f
      else:
       self.filename_no=o(self.daum_info.episode_list[key][0])
       flag_need_rename=f
     ...
     logger.debug(' - 파일명에 회차정보 삽입. Daum 정보 - date:%s count:%s %s',key,l(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
     self.filename_no=o(self.daum_info.episode_list[key][0])
     flag_need_rename=f
   else:
   ...
   if f and self.filename_no!=-1:
    if self.except_genre_remove_epi_number is not E and('all' in self.except_genre_remove_epi_number or self.daum_info.genre in self.except_genre_remove_epi_number):
 ...
 def make_filename(cls,_entity):
  ...
  if _entity.filename_release!='' and _entity.filename_release is not E:
 ...
 def set_find_library_path(self,nd_find_library_path):
  ...
  if nd_find_library_path.entity_library_root.drive_type==EntityLibraryPathRoot.DriveType.LOCAL:
   ...
   if nd_find_library_path.entity_library_root.replace_for_plex is not E:
