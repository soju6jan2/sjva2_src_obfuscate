...
class EntityShow(S):
 ...
 def __init__(self,filename,by=0,nd_download_path=v,daum_info=v,except_genre_remove_epi_number=v):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if K and self.daum_info is not v:
 ...
 def analyze_filename(self):
  for idx,regex in H(self._REGEX_FILENAME):
   ...
   if match:
    ...
    break
 def change_filename_continous_episode(self,move=K):
 ...
 def change_filename_by_rule(self,move=K):
