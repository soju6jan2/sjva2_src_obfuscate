...
class EntityLibraryPathRoot(K):
 ...
 depth=-1
 replace_for_plex=b
 def __init__(self,drive_type,mount_path,depth,rclone_path=b,sync_path=b,replace_for_plex=b):
...
class EntityLibraryPath(K):
 RENAME_REGEX=r'[\s\.\,\-\[\]\?\:\!\_\=\+]'
 entity_library_root=b
 basename='' 
 abspath='' 
 compare_name='' 
...
class EntityShow(K):
 ...
 filename_more=''
 daum_info=b
 def __init__(self,filename,by=0,nd_download_path=b,daum_info=b,except_genre_remove_epi_number=b):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if o and self.daum_info is not b:
   ...
   self.change_filename_continous_episode(move=c)
   self.daum_info=daum_info
   self.change_filename_by_rule(move=c)
   ...
   if self.video_type==EntityShow.VideoType.KOREA_TV:
    self.daum_info=daum_tv.Logic.get_daum_tv_info(self.filename_name)
    if self.daum_info is not b:
     self.change_filename_by_rule(move=c)
   else:
    self.daum_info=b
 def analyze_filename(self):
  for idx,regex in d(self._REGEX_FILENAME):
   ...
   if match:
    ...
    if self.filename_no is not b and self.filename_no!='':
    ...
    break
 def change_filename_continous_episode(self,move=o):
 ...
 def change_filename_by_rule(self,move=o):
  ...
  if self.daum_info.has_episode_info():
   ...
   if key in self.daum_info.episode_list:
    ...
    if self.filename_no!=-1:
     ...
     if flag:
      ...
      logger.debug(' - Daum 정보 Date:%s Count:%s No:%s',key,B(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
      self.log+=' - Daum 정보. 방송일:%s 회차:%s Count:%s\n'%(key,self.daum_info.episode_list[key][0],B(self.daum_info.episode_list[key]))
     ...
     logger.debug(' - 파일명에 회차정보 삽입. Daum 정보 - date:%s count:%s %s',key,B(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
     self.filename_no=Q(self.daum_info.episode_list[key][0])
     flag_need_rename=o
   else:
   ...
   if o and self.filename_no!=-1:
    if self.except_genre_remove_epi_number is not b and('all' in self.except_genre_remove_epi_number or self.daum_info.genre in self.except_genre_remove_epi_number):
 ...
 def make_filename(cls,_entity):
  ...
  if _entity.filename_release!='' and _entity.filename_release is not b:
 ...
 def set_find_library_path(self,nd_find_library_path):
  ...
  if nd_find_library_path.entity_library_root.drive_type==EntityLibraryPathRoot.DriveType.LOCAL:
   ...
   if nd_find_library_path.entity_library_root.replace_for_plex is not b:
