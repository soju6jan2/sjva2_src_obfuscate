...
class EntityLibraryPathRoot(C):
 ...
 depth=-1
 replace_for_plex=n
 def __init__(self,drive_type,mount_path,depth,rclone_path=n,sync_path=n,replace_for_plex=n):
...
class EntityShow(C):
 ...
 filename_more=''
 daum_info=n
 def __init__(self,filename,by=0,nd_download_path=n,daum_info=n,except_genre_remove_epi_number=n):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if q and self.daum_info is not n:
   ...
   self.change_filename_continous_episode(move=Y)
   self.daum_info=daum_info
   self.change_filename_by_rule(move=Y)
   ...
   if self.video_type==EntityShow.VideoType.KOREA_TV:
    self.daum_info=daum_tv.Logic.get_daum_tv_info(self.filename_name)
    if self.daum_info is not n:
     self.change_filename_by_rule(move=Y)
   else:
    self.daum_info=n
 def analyze_filename(self):
  for idx,regex in T(self._REGEX_FILENAME):
   ...
   if match:
    ...
    if self.filename_no is not n and self.filename_no!='':
 ...
 def change_filename_by_rule(self,move=q):
  ...
  if self.daum_info.has_episode_info():
   ...
   if q and self.filename_no!=-1:
    if self.except_genre_remove_epi_number is not n and('all' in self.except_genre_remove_epi_number or self.daum_info.genre in self.except_genre_remove_epi_number):
 ...
 def make_filename(cls,_entity):
  ...
  if _entity.filename_release!='' and _entity.filename_release is not n:
 ...
 def set_find_library_path(self,nd_find_library_path):
  ...
  if nd_find_library_path.entity_library_root.drive_type==EntityLibraryPathRoot.DriveType.LOCAL:
   ...
   if nd_find_library_path.entity_library_root.replace_for_plex is not n:
