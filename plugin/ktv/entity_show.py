...
class EntityLibraryPathRoot(I):
 ...
 depth=-1
 replace_for_plex=z
 def __init__(self,drive_type,mount_path,depth,rclone_path=z,sync_path=z,replace_for_plex=z):
...
class EntityShow(I):
 ...
 filename_more=''
 daum_info=z
 def __init__(self,filename,by=0,nd_download_path=z,daum_info=z,except_genre_remove_epi_number=z):
  ...
  if by==0:
   ...
   if self.daum_info:
    ...
    self.log+='Daum 정보 없음\n'
   if T and self.daum_info is not z:
 ...
 def analyze_filename(self):
  for idx,regex in d(self._REGEX_FILENAME):
   ...
   if match:
    ...
    if self.filename_no is not z and self.filename_no!='':
 ...
 def change_filename_by_rule(self,move=T):
  ...
  if self.daum_info.has_episode_info():
   ...
   if key in self.daum_info.episode_list:
    ...
    if self.filename_no!=-1:
     ...
     if flag:
      ...
      logger.debug(' - Daum 정보 Date:%s Count:%s No:%s',key,H(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
      self.log+=' - Daum 정보. 방송일:%s 회차:%s Count:%s\n'%(key,self.daum_info.episode_list[key][0],H(self.daum_info.episode_list[key]))
     ...
     logger.debug(' - 파일명에 회차정보 삽입. Daum 정보 - date:%s count:%s %s',key,H(self.daum_info.episode_list[key]),self.daum_info.episode_list[key][0])
     self.filename_no=J(self.daum_info.episode_list[key][0])
     flag_need_rename=T
   else:
   ...
   if T and self.filename_no!=-1:
    if self.except_genre_remove_epi_number is not z and('all' in self.except_genre_remove_epi_number or self.daum_info.genre in self.except_genre_remove_epi_number):
 ...
 def make_filename(cls,_entity):
  ...
  if _entity.filename_release!='' and _entity.filename_release is not z:
 ...
 def set_find_library_path(self,nd_find_library_path):
  ...
  if nd_find_library_path.entity_library_root.drive_type==EntityLibraryPathRoot.DriveType.LOCAL:
   ...
   if nd_find_library_path.entity_library_root.replace_for_plex is not z:
