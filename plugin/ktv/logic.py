...
class Logic(Q):
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,'ktv_process',interval,Logic.process_download_file0,[u'국내영상 파일 처리'],m)
 ...
 def process_download_file0():
  try:
   if app.config['config']['use_celery']:
    ...
    try:
     flag_rclone_start=result.get(on_message=Logic.for_synoindex,propagate=w)
   ...
   if Logic.plex_update_list:
    logger.debug('>> len plex_update_list : %s',z(Logic.plex_update_list))
     ...
     if os.path.isfile(abspath):
      ...
      if entity.video_type==EntityShow.VideoType.KOREA_TV:
       ...
       if z(_find_dir)==1:
        ...
        if flag_move==m and entity.daum_info:
         flag_search=m
 ...
 def receive_scan_result(p,filename):
  try:
   ...
   modelfile=db.session.query(ModelKtvFile).filter_by(p=R(p)).first()
   if modelfile is not y:
    modelfile.scan_status=3
    modelfile.scan_time=datetime.now()
    plex.Logic.get_section_id(modelfile,more=w)
 ...
 def check_library_completed():
  try:
   ...
   for entity in entity_list:
    logger.debug('filename:%s',entity.filename)
    plex.Logic.get_section_id(entity,more=w)
    db.session.add(entity)
   db.session.commit()
  except X as exception:
 ...
 def library_save(req):
  try:
   ...
   return m
 @j
 def call_rclone_plugin(item,remove=m):
  ...
  try:
   if w:
    ...
    if lib.library_type!=0:
     Logic.call_rclone_plugin(lib,remove=w)
 ...
 def send_to_listener(target_file):
  try:
   args=[]
   kargs={'plugin':'ktv','type':'add','filepath':target_file,'is_file':w}
