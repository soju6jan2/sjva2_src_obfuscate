...
class Logic(R):
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,'ktv_process',interval,Logic.process_download_file0,[u'국내영상 파일 처리'],B)
 ...
 def process_download_file0():
  try:
   ...
   if Logic.plex_update_list:
    logger.debug('>> len plex_update_list : %s',q(Logic.plex_update_list))
     ...
     if os.path.isfile(abspath):
      ...
      if entity.video_type==EntityShow.VideoType.KOREA_TV:
       ...
       if q(_find_dir)==1:
        ...
        if flag_move==B and entity.daum_info:
         flag_search=B
 ...
 def receive_scan_result(y,filename):
  try:
   ...
   modelfile=db.session.query(ModelKtvFile).filter_by(y=J(y)).first()
 ...
 def library_save(req):
  try:
   ...
   return B
 @W
 def call_rclone_plugin(item,remove=B):
