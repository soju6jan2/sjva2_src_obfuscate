...
class Logic(s):
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,'ktv_process',interval,Logic.process_download_file0,[u'국내영상 파일 처리'],j)
 ...
 def process_download_file0():
  try:
   if app.config['config']['use_celery']:
    ...
    try:
     flag_rclone_start=result.get(on_message=Logic.for_synoindex,propagate=Y)
 ...
 def process_download_file(self):
  ...
  for var in list_:
   try:
     ...
     if os.path.isfile(abspath):
      ...
      if entity.video_type==EntityShow.VideoType.KOREA_TV:
       ...
       if p(_find_dir)==1:
        ...
        if flag_move==j and entity.daum_info:
         flag_search=j
    ...
    try:
     if ModelSetting.query.filter_by(key='telegram').first().value=='True' and telegram_log is not X:
      img=X
      if entity is not X and entity.daum_info is not X and entity.daum_info.poster_url is not X:
 ...
 def _get_find_dir(dir_list,entity):
  ...
  for item in dir_list:
   if entity.filename.find(item.basename)!=-1:
    ...
    ret.append(item)
   elif entity.daum_info is not X and entity.daum_info.title==item.basename:
    ret.append(item)
 ...
 plex_update_list=[]
 @C
 def receive_scan_result(K,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',K,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(K=E(K)).first()
   if modelfile is not X:
    modelfile.scan_status=3
    modelfile.scan_time=datetime.now()
    plex.Logic.get_section_id(modelfile,more=Y)
 ...
 def check_library_completed():
  try:
   ...
   for entity in entity_list:
    logger.debug('filename:%s',entity.filename)
    plex.Logic.get_section_id(entity,more=Y)
    db.session.add(entity)
   db.session.commit()
  except m as exception:
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.K)).limit(page_size).offset((page-1)*page_size))
  ...
  try:
   if Y:
    ...
    if library_id==-1:
     ...
     item=db.session.query(ModelKtvLibrary).filter_by(K=library_id).with_for_update().first()
   ...
   return j
 @C
 def call_rclone_plugin(item,remove=j):
 ...
 def library_remove(req):
  try:
   if Y:
    library_id=E(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(K=library_id).first()
    if lib.library_type!=0:
     Logic.call_rclone_plugin(lib,remove=Y)
 ...
 def send_to_listener(target_file):
  try:
   args=[]
   kargs={'plugin':'ktv','type':'add','filepath':target_file,'is_file':Y}
