...
class Logic(s):
 ...
 def process_download_file0():
  try:
   if app.config['config']['use_celery']:
    ...
    try:
     flag_rclone_start=result.get(on_message=Logic.for_synoindex,propagate=K)
 ...
 plex_update_list=[]
 @R
 def receive_scan_result(o,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',o,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(o=M(o)).first()
   if modelfile is not Q:
    modelfile.scan_status=3
    modelfile.scan_time=datetime.now()
    plex.Logic.get_section_id(modelfile,more=K)
 ...
 def check_library_completed():
  try:
   ...
   for entity in entity_list:
    logger.debug('filename:%s',entity.filename)
    plex.Logic.get_section_id(entity,more=K)
    db.session.add(entity)
   db.session.commit()
  except m as e:
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.o)).limit(page_size).offset((page-1)*page_size))
  ...
  try:
   if K:
    ...
    if library_id==-1:
     ...
     item=db.session.query(ModelKtvLibrary).filter_by(o=library_id).with_for_update().first()
 ...
 def library_remove(req):
  try:
   if K:
    library_id=M(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(o=library_id).first()
    if lib.library_type!=0:
     Logic.call_rclone_plugin(lib,remove=K)
 ...
 def send_to_listener(target_file):
  try:
   args=[]
   kargs={'plugin':'ktv','type':'add','filepath':target_file,'is_file':K}
