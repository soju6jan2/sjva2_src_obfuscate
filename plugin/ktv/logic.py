...
class Logic(N):
 ...
 def process_download_file0():
  try:
   if app.config['config']['use_celery']:
    ...
    try:
     flag_rclone_start=result.get(on_message=Logic.for_synoindex,propagate=V)
 ...
 def process_download_file(self):
  ...
  for var in list_:
   try:
    ...
    try:
     if ModelSetting.query.filter_by(key='telegram').first().value=='True' and telegram_log is not f:
      img=f
      if entity is not f and entity.daum_info is not f and entity.daum_info.poster_url is not f:
 ...
 def _get_find_dir(dir_list,entity):
  ...
  for item in dir_list:
   if entity.filename.find(item.basename)!=-1:
    ...
    ret.append(item)
   elif entity.daum_info is not f and entity.daum_info.title==item.basename:
    ret.append(item)
 ...
 def receive_scan_result(A,filename):
  try:
   ...
   modelfile=db.session.query(ModelKtvFile).filter_by(A=M(A)).first()
   if modelfile is not f:
    modelfile.scan_status=3
    modelfile.scan_time=datetime.now()
    plex.Logic.get_section_id(modelfile,more=V)
 ...
 def check_library_completed():
  try:
   ...
   for entity in entity_list:
    logger.debug('filename:%s',entity.filename)
    plex.Logic.get_section_id(entity,more=V)
    db.session.add(entity)
   db.session.commit()
  except P as exception:
  ...
  try:
   if V:
    ...
    if lib.library_type!=0:
     Logic.call_rclone_plugin(lib,remove=V)
 ...
 def send_to_listener(target_file):
  try:
   args=[]
   kargs={'plugin':'ktv','type':'add','filepath':target_file,'is_file':V}
