...
class Logic(C):
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,'ktv_process',interval,Logic.process_download_file0,[u'국내영상 파일 처리'],Y)
 ...
 def process_download_file0():
  try:
   ...
   if Logic.plex_update_list:
    logger.debug('>> len plex_update_list : %s',b(Logic.plex_update_list))
 ...
 def process_download_file(self):
  ...
  for var in list_:
   try:
     ...
     if os.path.isfile(abspath):
      ...
      if entity.video_type==EntityShow.VideoType.KOREA_TV:
       ...
       if b(_find_dir)==1:
        ...
        if flag_move==Y and entity.daum_info:
         flag_search=Y
    ...
    try:
     if ModelSetting.query.filter_by(key='telegram').first().value=='True' and telegram_log is not n:
      img=n
      if entity is not n and entity.daum_info is not n and entity.daum_info.poster_url is not n:
 ...
 def _get_find_dir(dir_list,entity):
  ...
  for item in dir_list:
   if entity.filename.find(item.basename)!=-1:
    ...
    ret.append(item)
   elif entity.daum_info is not n and entity.daum_info.title==item.basename:
    ret.append(item)
 ...
 plex_update_list=[]
 @O
 def receive_scan_result(g,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',g,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(g=R(g)).first()
   if modelfile is not n:
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.g)).limit(page_size).offset((page-1)*page_size))
  ...
  try:
   if q:
    ...
    if library_id==-1:
     ...
     item=db.session.query(ModelKtvLibrary).filter_by(g=library_id).with_for_update().first()
   ...
   return Y
 @O
 def call_rclone_plugin(item,remove=Y):
 ...
 def library_remove(req):
  try:
   if q:
    library_id=R(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(g=library_id).first()
