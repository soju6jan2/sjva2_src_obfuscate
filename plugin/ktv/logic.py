...
class Logic(Y):
 ...
 def process_download_file(self):
  ...
  for var in list_:
   try:
    ...
    try:
     if ModelSetting.query.filter_by(key='telegram').first().value=='True' and telegram_log is not u:
      img=u
      if entity is not u and entity.daum_info is not u and entity.daum_info.poster_url is not u:
 ...
 def _get_find_dir(dir_list,entity):
  ...
  for item in dir_list:
   if entity.filename.find(item.basename)!=-1:
    ...
    ret.append(item)
   elif entity.daum_info is not u and entity.daum_info.title==item.basename:
    ret.append(item)
 ...
 plex_update_list=[]
 @G
 def receive_scan_result(y,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',y,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(y=M(y)).first()
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.y)).limit(page_size).offset((page-1)*page_size))
  ...
  try:
   if k:
    ...
    if library_id==-1:
     ...
     item=db.session.query(ModelKtvLibrary).filter_by(y=library_id).with_for_update().first()
 ...
 def library_remove(req):
  try:
   if k:
    library_id=M(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(y=library_id).first()
