...
class Logic(V):
 ...
 def process_download_file0():
  try:
   ...
   if Logic.plex_update_list:
    logger.debug('>> len plex_update_list : %s',d(Logic.plex_update_list))
 ...
 plex_update_list=[]
 @v
 def receive_scan_result(n,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',n,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(n=Y(n)).first()
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.n)).limit(page_size).offset((page-1)*page_size))
 ...
 def library_save(req):
  try:
   if X:
    library_id=Y(req.form['library_id'])
    if library_id==-1:
     item=ModelKtvLibrary()
    else:
     item=db.session.query(ModelKtvLibrary).filter_by(n=library_id).with_for_update().first()
    item.library_type=Y(req.form['library_type'])
 ...
 def library_remove(req):
  try:
   if X:
    library_id=Y(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(n=library_id).first()
