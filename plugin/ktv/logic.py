...
class Logic(D):
 ...
 def process_download_file0():
  try:
   if app.config['config']['use_celery']:
    ...
    try:
     flag_rclone_start=result.get(on_message=Logic.for_synoindex,propagate=e)
   ...
   if Logic.plex_update_list:
    logger.debug('>> len plex_update_list : %s',o(Logic.plex_update_list))
 ...
 def process_download_file(self):
  ...
  for var in list_:
   try:
    ...
    try:
     if ModelSetting.query.filter_by(key='telegram').first().value=='True' and telegram_log is not Y:
      img=Y
      if entity is not Y and entity.daum_info is not Y and entity.daum_info.poster_url is not Y:
 ...
 def _get_find_dir(dir_list,entity):
  ...
  for item in dir_list:
   if entity.filename.find(item.basename)!=-1:
    ...
    ret.append(item)
   elif entity.daum_info is not Y and entity.daum_info.title==item.basename:
    ret.append(item)
 ...
 plex_update_list=[]
 @O
 def receive_scan_result(R,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',R,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(R=g(R)).first()
   if modelfile is not Y:
    modelfile.scan_status=3
    modelfile.scan_time=datetime.now()
    plex.Logic.get_section_id(modelfile,more=e)
 ...
 def check_library_completed():
  try:
   ...
   for entity in entity_list:
    logger.debug('filename:%s',entity.filename)
    plex.Logic.get_section_id(entity,more=e)
    db.session.add(entity)
   db.session.commit()
  except s as exception:
 ...
 def filelist(req):
  try:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.R)).limit(page_size).offset((page-1)*page_size))
 ...
 def library_save(req):
  try:
   if e:
    library_id=g(req.form['library_id'])
    if library_id==-1:
     item=ModelKtvLibrary()
    else:
     item=db.session.query(ModelKtvLibrary).filter_by(R=library_id).with_for_update().first()
    item.library_type=g(req.form['library_type'])
 ...
 def library_remove(req):
  try:
   if e:
    library_id=g(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(R=library_id).first()
    if lib.library_type!=0:
     Logic.call_rclone_plugin(lib,remove=e)
 ...
 def send_to_listener(target_file):
  try:
   args=[]
   kargs={'plugin':'ktv','type':'add','filepath':target_file,'is_file':e}
