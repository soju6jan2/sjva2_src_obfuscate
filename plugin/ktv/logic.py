...
class Logic(D):
 ...
 def scheduler_start():
  try:
   ...
   job=Job(package_name,'ktv_process',interval,Logic.process_download_file0,[u'국내영상 파일 처리'],z)
     ...
     if os.path.isfile(abspath):
      ...
      if entity.video_type==EntityShow.VideoType.KOREA_TV:
       ...
       if n(_find_dir)==1:
        ...
        if flag_move==z and entity.daum_info:
         flag_search=z
 ...
 plex_update_list=[]
 @p
 def receive_scan_result(Q,filename):
  try:
   import plex
   logger.debug('Receive Scan Completed : %s-%s',Q,filename)
   modelfile=db.session.query(ModelKtvFile).filter_by(Q=q(Q)).first()
 ...
 def filelist(req):
  try:
   ret={}
   page=1
   page_size=q(db.session.query(ModelSetting).filter_by(key='web_page_size').first().value)
   job_id=''
   search=''
   if 'page' in req.form:
   ...
   count=query.count()
   query=(query.order_by(desc(ModelKtvFile.Q)).limit(page_size).offset((page-1)*page_size))
 ...
 def library_save(req):
  try:
   if P:
    library_id=q(req.form['library_id'])
    if library_id==-1:
     item=ModelKtvLibrary()
    else:
     item=db.session.query(ModelKtvLibrary).filter_by(Q=library_id).with_for_update().first()
    item.library_type=q(req.form['library_type'])
   ...
   return z
 @p
 def call_rclone_plugin(item,remove=z):
 ...
 def library_remove(req):
  try:
   if P:
    library_id=q(req.form['library_id'])
    lib=db.session.query(ModelKtvLibrary).filter_by(Q=library_id).first()
