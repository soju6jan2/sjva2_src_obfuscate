...
class LogicMovie(A):
   ...
   if not os.path.exists(dest):
    ...
    for t in os.listdir(src):
     try:
      ...
      logger.error('Exxception:%s',e)
      logger.error(traceback.format_exc())
    if os.path.exists(src)and d(os.listdir(src))==0:
     os.rmdir(src)
  except I as e:
   logger.error('Exxception:%s',e)
   logger.error(traceback.format_exc())
   ...
   for path in source_path:
    ...
    for f in lists:
     try:
      ...
      item['is_file']=K if os.path.isfile(item['fullpath'])else Q
 ...
 def start(source_path,target_path):
  if app.config['config']['use_celery']:
   ...
   try:
    result.get(on_message=LogicMovie.save_db,propagate=K)
   except:
    ...
    try:
     if not item['flag_move']and 'year' in item['guessit']:
      ...
      if movie and movie[0]['score']==100:
       ...
       if item['is_include_kor']==Q and item['search_name']is not D:
        movie=MovieSearch.search_imdb(item['search_name'].lower(),item['guessit']['year'])
        if movie is not D:
         logger.debug('IMDB TITLE:[%s][%s]',movie['title'],movie['year'])
