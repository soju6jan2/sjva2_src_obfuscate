...
class LogicMovie(l):
   ...
   if not os.path.exists(dest):
    ...
    for t in os.listdir(src):
     try:
      ...
      logger.error('Exxception:%s',exception)
      logger.error(traceback.format_exc())
    if os.path.exists(src)and u(os.listdir(src))==0:
     os.rmdir(src)
  except p as exception:
   logger.error('Exxception:%s',exception)
   logger.error(traceback.format_exc())
   ...
   for path in source_path:
    ...
    for f in lists:
     try:
      ...
      item['is_file']=h if os.path.isfile(item['fullpath'])else U
 ...
 def start(source_path,target_path):
  if app.config['config']['use_celery']:
   ...
   try:
    result.get(on_message=LogicMovie.save_db,propagate=h)
   except:
    ...
    try:
     if not item['flag_move']and 'year' in item['guessit']:
      ...
      if movie and movie[0]['score']==100:
       ...
       if item['is_include_kor']==U and item['search_name']is not t:
        movie=MovieSearch.search_imdb(item['search_name'].lower(),item['guessit']['year'])
        if movie is not t:
         logger.debug('IMDB TITLE:[%s][%s]',movie['title'],movie['year'])
