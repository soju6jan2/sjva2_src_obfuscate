 ...
 def execute_command(command):
  try:
   ...
   if app.config['config']['is_py2']:
    ...
    with p.stdout:
     for line in i(p.stdout.readline,b''):
      try:
    ...
    with p.stdout:
     for line in i(p.stdout.readline,''):
      ret.append(line.strip())
     p.wait()
...
class AlchemyEncoder(json.JSONEncoder):
 def default(self,obj):
  if g(obj.__class__,DeclarativeMeta):
   fields={}
   for field in[x for x in w(obj)if not x.startswith('_')and x!='metadata']:
