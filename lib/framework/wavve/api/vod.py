...
def vod_newcontents(page=1,limit=20,genre='all'):
 try:
  page=s(page)if v(page)!=s else page
...
def vod_contents(page=1,limit=20,content_type='newcontents',genre='all',orderby='new',is_cf=T):
 try:
  page=s(page)if v(page)!=s else page
  param=get_baseparameter()
  param['genre']=genre
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def vod_allprograms(page=1,limit=20,contenttype='program',genre='all',subgenre=C,orderby='new',is_cf=T):
 try:
  page=s(page)if v(page)!=s else page
  param=get_baseparameter()
  param['genre']=genre
