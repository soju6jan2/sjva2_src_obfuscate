...
def plugin_init():
 try:
  ...
  pass_include=[]
  except_plugin_list=[]
  if app.config['config']['run_by_migration']==Q:
  ...
  for plugin_name in plugins:
   ...
   try:
    ...
    try:
     mod_plugin_info=n(mod,'plugin_info') 
    ...
    mod_blue_print=n(mod,'blueprint')
    if mod_blue_print:
  ...
  for key,mod in plugin_instance_list.items():
   try:
    ...
    mod_plugin_load=n(mod,'plugin_load')
  ...
  logger.debug('Plugin Log completed.. : %s ',M(plugin_instance_list))
 except d as e:
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
def plugin_unload():
 for key,mod in plugin_instance_list.items():
  try:
   mod_plugin_unload=n(mod,'plugin_unload')
