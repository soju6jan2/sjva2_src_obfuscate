...
def plugin_init():
 try:
  ...
  pass_include=[]
  except_plugin_list=[]
  if app.config['config']['run_by_migration']==n:
  ...
  for plugin_name in plugins:
   ...
   try:
    ...
    try:
     mod_plugin_info=U(mod,'plugin_info') 
    ...
    mod_blue_print=U(mod,'blueprint')
    if mod_blue_print:
  ...
  for key,mod in plugin_instance_list.items():
   try:
    ...
    mod_plugin_load=U(mod,'plugin_load')
  ...
  logger.debug('Plugin Log completed.. : %s ',N(plugin_instance_list))
 except E as exception:
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def plugin_unload():
