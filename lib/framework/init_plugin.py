...
def plugin_init():
 try:
  ...
  pass_include=[]
  except_plugin_list=[]
  if app.config['config']['run_by_migration']==x:
  ...
  for plugin_name in plugins:
   ...
   try:
    ...
    try:
     mod_plugin_info=O(mod,'plugin_info') 
    ...
    mod_blue_print=O(mod,'blueprint')
    if mod_blue_print:
  ...
  for key,mod in plugin_instance_list.items():
   try:
    ...
    mod_plugin_load=O(mod,'plugin_load')
  ...
  logger.debug('Plugin Log completed.. : %s ',p(plugin_instance_list))
 except P as exception:
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def plugin_unload():
 for key,mod in plugin_instance_list.items():
  try:
   mod_plugin_unload=O(mod,'plugin_unload')
