...
try:
 ...
 app.config['SQLALCHEMY_TRACK_MODIFICATIONS']=u
 app.config['config']={}
 logger.debug('======================================')
 logger.debug(sys.argv)
 app.config['config']['run_by_real']=M if sys.argv[0]=='sjva.py' else u
 app.config['config']['run_by_migration']=M if sys.argv[-2]=='db' else u
 app.config['config']['run_by_worker']=M if sys.argv[0].find('celery')!=-1 else u
 app.config['config']['run_by_init_db']=M if sys.argv[-1]=='init_db' else u
 ...
 pip_install()
 db=SQLAlchemy(app,session_options={"autoflush":u})
 scheduler=Scheduler()
