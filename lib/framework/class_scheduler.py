...
job_defaults={'coalesce':g,'max_instances':1}
...
class Scheduler(k):
 ...
 """
    ...
    """ 
 def add_job_instance(self,job_instance,run=z):
  from framework import app
  if app.config['config']['run_by_real']and app.config['config']['auth_status']:
   if not self.is_include(job_instance.job_id):
    ...
    if job_instance.is_interval:
     ...
     self.sched.add_job(job_instance.job_function,CronTrigger.from_crontab(job_instance.interval),id=job_instance.job_id,args=(E))
    job=self.sched.get_job(job_instance.job_id)
