...
def get_episode_json_default(episode_code,quality,token,proxy=b):
 ...
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=b,inc_quality=D):
 ts='%d'%time.time()
 if token is b:
  token=config['token']
 try:
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=b,is_live=H):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  if episode_data["body"]["stream"]["quality"]is b:
   ...
   qualityCount=x(episode_data["body"]["stream"]["quality"])
   ...
   try:
    ...
    if D:
     ...
     if item['schedule']['episode']is not b:
      ...
      if info['title'].startswith('CH.')and x(item['schedule']['episode']['image'])>0:
