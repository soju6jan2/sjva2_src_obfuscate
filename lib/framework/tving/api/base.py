...
config={'token':s,'param':"&free=all&lastFrequency=y&order=broadDate",'program_param':'&free=all&order=frequencyDesc&programCode=%s','default_param':'&screenCode=CSSD0100&networkCode=CSND0900&osCode=CSOD0900&teleCode=CSCD0900&apiKey=1e7952d0917d6aab1f0293a063697610'}
...
def get_vod_list(p=s,page=1):
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default(episode_code,quality,token,proxy=s):
 ts='%d'%time.time()
 if token is s:
  token=config['token']
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=s,inc_quality=x):
 ts='%d'%time.time()
 if token is s:
  token=config['token']
 try:
  ...
  if decrypted_url.find('Policy=')==-1:
   data,ret=get_episode_json_default_live(episode_code,quality,token,proxy=proxy,inc_quality=U)
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=s,is_live=U):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  airdate=h(episode_data["body"]["content"]["info"]["episode"]["broadcast_date"])[2:]
  currentQuality=s
  if episode_data["body"]["stream"]["quality"]is s:
   currentQuality="stream40"
  else:
   qualityCount=N(episode_data["body"]["stream"]["quality"])
   for i in l(qualityCount):
    if episode_data["body"]["stream"]["quality"][i]["selected"]=="Y":
     ...
     break
  if currentQuality is s:
   return
  qualityRes=get_quality_to_res(currentQuality)
  episodeno_str=h(episodeno)
  if episodeno<10:
  ...
  for item in data["body"]["result"]:
   try:
    ...
    if x:
     ...
     if item['schedule']['episode']is not s:
      info['episode_title']=item['schedule']['episode']['name']['ko']
      if info['title'].startswith('CH.')and N(item['schedule']['episode']['image'])>0:
...
def get_movie_json(code,deviceid,token,proxy=s):
 ts='%d'%time.time()
 if token is s:
  token=config['token']
 try:
  ...
  if data['body']['result']['code']=="000":
   if '4k_nondrm_url' in data['body']['stream']['broadcast']:
    ...
    if decrypted_url.find('5000k_PC.mp4')!=-1:
     ...
     data['ret']['filename']=Util.change_text_for_use_filename('%s.%s.%s.2160p-ST.mp4'%(data['body']['content']['info']['movie']['name']['ko'],h(data['body']['content']['info']['movie']['release_date'])[:4],data['body']['content']['info']['movie']['name']['en']))
    else:
     data['ret']['ret']='no_4k'
     data['ret']['decrypted_url']=decrypted_url
  else:
...
def get_prefer_url(url):
 try:
  ...
  last_url=s
  for t in V(data.split('\n')):
   if t.strip().find('chunklist.m3u8')!=-1:
    last_url=t
    break
  if last_url is not s and last_url!='':
  ...
  logger.error(traceback.format_exc())
 return url
def get_vod_list2(param=s,page=1,genre='all'):
 try:
...
"""
def get_movie_json2(code,deviceid,token,proxy=s,quality='stream50'):
 ts='%d'%time.time()
 if token is s:
  token=config['token']
 try:
