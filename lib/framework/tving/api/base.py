...
def get_episode_json_default(episode_code,quality,token,proxy=e):
 ...
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=e,inc_quality=p):
 ts='%d'%time.time()
 if token is e:
  token=config['token']
 try:
  ...
  if decrypted_url.find('Policy=')==-1:
   data,ret=get_episode_json_default_live(episode_code,quality,token,proxy=proxy,inc_quality=x)
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=e,is_live=x):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  if episode_data["body"]["stream"]["quality"]is e:
   ...
   qualityCount=l(episode_data["body"]["stream"]["quality"])
   for i in n(qualityCount):
   ...
   try:
    ...
    if p:
     ...
     if item['schedule']['episode']is not e:
      ...
      if info['title'].startswith('CH.')and l(item['schedule']['episode']['image'])>0:
