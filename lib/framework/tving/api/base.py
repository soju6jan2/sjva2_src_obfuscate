...
def get_episode_json_default(episode_code,quality,token,proxy=c):
 ...
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=c,inc_quality=Q):
 ts='%d'%time.time()
 if token is c:
  token=config['token']
 try:
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=c,is_live=v):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  if episode_data["body"]["stream"]["quality"]is c:
   ...
   qualityCount=r(episode_data["body"]["stream"]["quality"])
   ...
   try:
    ...
    if Q:
     ...
     if item['schedule']['episode']is not c:
      ...
      if info['title'].startswith('CH.')and r(item['schedule']['episode']['image'])>0:
