...
config={'token':B,'param':"&free=all&lastFrequency=y&order=broadDate",'program_param':'&free=all&order=frequencyDesc&programCode=%s','default_param':'&screenCode=CSSD0100&networkCode=CSND0900&osCode=CSOD0900&teleCode=CSCD0900&apiKey=1e7952d0917d6aab1f0293a063697610'}
...
def get_vod_list(p=B,page=1):
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default(episode_code,quality,token,proxy=B):
 ts='%d'%time.time()
 if token is B:
  token=config['token']
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=B,inc_quality=V):
 ts='%d'%time.time()
 if token is B:
  token=config['token']
 try:
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=B,is_live=K):
 try:
  if is_live:
  ...
  for item in data["body"]["result"]:
   try:
    ...
    if V:
     ...
     if item['schedule']['episode']is not B:
...
def get_movie_json(code,deviceid,token,proxy=B):
 ts='%d'%time.time()
 if token is B:
  token=config['token']
 try:
...
def get_prefer_url(url):
 try:
  ...
  last_url=B
  for t in d(data.split('\n')):
   if t.strip().find('chunklist.m3u8')!=-1:
    last_url=t
    break
  if last_url is not B and last_url!='':
  ...
  logger.error(traceback.format_exc())
 return url
def get_vod_list2(param=B,page=1,genre='all'):
 try:
...
"""
def get_movie_json2(code,deviceid,token,proxy=B,quality='stream50'):
 ts='%d'%time.time()
 if token is B:
  token=config['token']
 try:
