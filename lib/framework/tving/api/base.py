...
def get_episode_json_default(episode_code,quality,token,proxy=e):
 ...
 try:
  ...
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=e,inc_quality=L):
 ts='%d'%time.time()
 if token is e:
  token=config['token']
 try:
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=e,is_live=G):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  if episode_data["body"]["stream"]["quality"]is e:
   ...
   qualityCount=F(episode_data["body"]["stream"]["quality"])
   ...
   try:
    ...
    if L:
     ...
     if item['schedule']['episode']is not e:
      ...
      if info['title'].startswith('CH.')and F(item['schedule']['episode']['image'])>0:
