...
def get_episode_json_default(episode_code,quality,token,proxy=r):
 ...
 try:
  ...
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=r,inc_quality=T):
 ts='%d'%time.time()
 if token is r:
  token=config['token']
 try:
  ...
  if decrypted_url.find('Policy=')==-1:
   data,ret=get_episode_json_default_live(episode_code,quality,token,proxy=proxy,inc_quality=n)
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=r,is_live=n):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  if episode_data["body"]["stream"]["quality"]is r:
   ...
   qualityCount=e(episode_data["body"]["stream"]["quality"])
   for i in f(qualityCount):
   ...
   try:
    ...
    if T:
     ...
     if item['schedule']['episode']is not r:
      ...
      if info['title'].startswith('CH.')and e(item['schedule']['episode']['image'])>0:
