...
def get_episode_json_default(episode_code,quality,token,proxy=M):
 ...
 try:
  ...
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=M,inc_quality=Q):
 ts='%d'%time.time()
 if token is M:
  token=config['token']
 try:
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=M,is_live=q):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  if episode_data["body"]["stream"]["quality"]is M:
   ...
   qualityCount=P(episode_data["body"]["stream"]["quality"])
   ...
   try:
    ...
    if Q:
     ...
     if item['schedule']['episode']is not M:
      ...
      if info['title'].startswith('CH.')and P(item['schedule']['episode']['image'])>0:
