...
def get_episode_json_default(episode_code,quality,token,proxy=k):
 ...
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def get_episode_json_default_live(episode_code,quality,token,proxy=k,inc_quality=P):
 ts='%d'%time.time()
 if token is k:
  token=config['token']
 try:
  ...
  if decrypted_url.find('Policy=')==-1:
   data,ret=get_episode_json_default_live(episode_code,quality,token,proxy=proxy,inc_quality=m)
...
def get_episode_json_proxy(episode_code, quality, proxy_url, token):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
"""
def get_episode_json(episode_code,quality,token,proxy=k,is_live=m):
 try:
  if is_live:
...
def get_filename(episode_data):
 try:
  ...
  airdate=f(episode_data["body"]["content"]["info"]["episode"]["broadcast_date"])[2:]
  currentQuality=k
  if episode_data["body"]["stream"]["quality"]is k:
   currentQuality="stream40"
  else:
   qualityCount=C(episode_data["body"]["stream"]["quality"])
   for i in B(qualityCount):
   ...
   try:
    ...
    if P:
     ...
     if item['schedule']['episode']is not k:
      ...
      if info['title'].startswith('CH.')and C(item['schedule']['episode']['image'])>0:
  ...
  if data['body']['result']['code']=="000":
   if '4k_nondrm_url' in data['body']['stream']['broadcast']:
    ...
    if decrypted_url.find('5000k_PC.mp4')!=-1:
     ...
     data['ret']['filename']=Util.change_text_for_use_filename('%s.%s.%s.2160p-ST.mp4'%(data['body']['content']['info']['movie']['name']['ko'],f(data['body']['content']['info']['movie']['release_date'])[:4],data['body']['content']['info']['movie']['name']['en']))
    else:
     data['ret']['ret']='no_4k'
     data['ret']['decrypted_url']=decrypted_url
  else:
