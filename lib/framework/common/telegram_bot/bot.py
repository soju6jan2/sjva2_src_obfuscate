...
class TelegramBot(n):
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=z(mod,'process_telegram_data')
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      name=''
     try:
      TelegramBot.SUPER_BOT.promoteChatMember(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID[-1],user_id,can_post_messages=f,can_invite_users=f,can_promote_members=f)
     ...
     try:
      ...
      if K(tmp)==2:
       ...
       data=V
       text='입장한 방이 없습니다.'
       for idx,c in d(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID):
        try:
   ...
   logger.error(traceback.format_exc())
 @P
 def super_send_message(text,encryped=f,only_last=Q):
  try:
