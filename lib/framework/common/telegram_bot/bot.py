...
class TelegramBot(e):
 ...
 def start(bot_token):
  try:
   if TelegramBot.message_loop is z:
    ...
    if SystemModelSetting.get('ddns')=='https://sjva-server.soju6jan.com':
     ...
     pass
    while TelegramBot.message_loop is not z:
     time.sleep(60*60)
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      name=''
     try:
      TelegramBot.SUPER_BOT.promoteChatMember(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID[-1],user_id,can_post_messages=y,can_invite_users=y,can_promote_members=y)
     ...
     try:
      ...
      if F(tmp)==2:
       ...
       data=z
       text='입장한 방이 없습니다.'
       for idx,c in O(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID):
        try:
      ...
      logger.error('Exception:%s',exception)
      logger.error(traceback.format_exc())
      TelegramBot.SUPER_BOT.sendMessage(chat_id,G(e)) 
    else:
   ...
   logger.error(traceback.format_exc())
 @B
 def super_send_message(text,encryped=y,only_last=d):
  try:
   if TelegramBot.SUPER_BOT is z:
    TelegramBot.SUPER_BOT=Bot(TelegramBot.SUPER_TOKEN)
   if encryped:
