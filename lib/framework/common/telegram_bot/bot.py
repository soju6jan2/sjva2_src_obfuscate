...
class TelegramBot(s):
 ...
 def start(bot_token):
  try:
   if TelegramBot.message_loop is O:
    ...
    if SystemModelSetting.get('ddns')=='https://sjva-server.soju6jan.com':
     ...
     pass
    while TelegramBot.message_loop is not O:
     time.sleep(60*60)
 ...
 def super_receive_callback(msg):
  try:
   ...
   logger.error(traceback.format_exc())
 @R
 def super_send_message(text,encryped=o,only_last=Q):
  try:
   if TelegramBot.SUPER_BOT is O:
    TelegramBot.SUPER_BOT=Bot(TelegramBot.SUPER_TOKEN)
   if encryped:
