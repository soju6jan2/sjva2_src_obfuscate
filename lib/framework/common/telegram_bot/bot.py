...
class TelegramBot(M):
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=Y(mod,'process_telegram_data')
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      name=''
     try:
      TelegramBot.SUPER_BOT.promoteChatMember(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID[-1],user_id,can_post_messages=E,can_invite_users=E,can_promote_members=E)
        ...
        try:
         ...
         if data is not D:
          if data['status']=='administrator':
           ...
           text=json.dumps(data,indent=2)+'\n'+'%s번 방에 있습니다. 32번=%s, 33번=%s'%((idx+1),w(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID)-1,w(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID))
           break
        except P as exception:
         logger.error('Exception:%s',exception)
         logger.error(traceback.format_exc())
   ...
   logger.error(traceback.format_exc())
 @f
 def super_send_message(text,encryped=E,only_last=W):
  try:
