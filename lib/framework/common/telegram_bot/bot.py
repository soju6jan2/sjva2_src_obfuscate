...
class TelegramBot(g):
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=e(mod,'process_telegram_data')
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      logger.error('Exception:%s',exception)
      logger.error(traceback.format_exc())
      TelegramBot.SUPER_BOT.sendMessage(chat_id,W(e)) 
    else:
   ...
   logger.error(traceback.format_exc())
 @c
 def super_send_message(text,encryped=I,only_last=b):
  try:
