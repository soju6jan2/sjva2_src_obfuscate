...
class TelegramBot(I):
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=d(mod,'process_telegram_data')
 ...
 def super_receive_callback(msg):
  try:
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      if u(tmp)==2:
       ...
       data=s
       text='입장한 방이 없습니다.'
       for idx,c in a(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID):
        try:
   ...
   logger.error(traceback.format_exc())
 @Y
 def super_send_message(text,encryped=q,only_last=t):
  try:
