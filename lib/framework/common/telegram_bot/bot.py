...
class TelegramBot(B):
 ...
 def start(bot_token):
  try:
   if TelegramBot.message_loop is r:
    ...
    if SystemModelSetting.get('ddns')=='https://sjva-server.soju6jan.com':
     ...
     pass
    while TelegramBot.message_loop is not r:
     time.sleep(60*60)
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=b(mod,'process_telegram_data')
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      name=''
     try:
      TelegramBot.SUPER_BOT.promoteChatMember(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID[-1],user_id,can_post_messages=I,can_invite_users=I,can_promote_members=I)
     ...
     try:
        ...
        try:
         ...
         if data is not r:
          if data['status']=='administrator':
           ...
           text=json.dumps(data,indent=2)+'\n'+'%s번 방에 있습니다. 32번=%s, 33번=%s'%((idx+1),k(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID)-1,k(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID))
           break
        except U as exception:
         logger.error('Exception:%s',exception)
         logger.error(traceback.format_exc())
      ...
      logger.error('Exception:%s',exception)
      logger.error(traceback.format_exc())
      TelegramBot.SUPER_BOT.sendMessage(chat_id,M(e)) 
    else:
   ...
   logger.error(traceback.format_exc())
 @u
 def super_send_message(text,encryped=I,only_last=J):
  try:
   if TelegramBot.SUPER_BOT is r:
    TelegramBot.SUPER_BOT=Bot(TelegramBot.SUPER_TOKEN)
   if encryped:
