...
class TelegramBot(J):
 ...
 def start(bot_token):
  try:
   if TelegramBot.message_loop is c:
    ...
    if SystemModelSetting.get('ddns')=='https://sjva-server.soju6jan.com':
     ...
     pass
    while TelegramBot.message_loop is not c:
     time.sleep(60*60)
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=f(mod,'process_telegram_data')
