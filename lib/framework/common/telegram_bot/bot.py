...
class TelegramBot(u):
 ...
 def start(bot_token):
  try:
   if TelegramBot.message_loop is D:
    ...
    if SystemModelSetting.get('ddns')=='https://sjva-server.soju6jan.com':
     ...
     pass
    while TelegramBot.message_loop is not D:
     time.sleep(60*60)
  ...
  try:
   ...
   if 'plugin' in data:
    try:
     ...
     mod_process_telegram_data=V(mod,'process_telegram_data')
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      name=''
     try:
      TelegramBot.SUPER_BOT.promoteChatMember(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID[-1],user_id,can_post_messages=x,can_invite_users=x,can_promote_members=x)
        ...
        try:
         ...
         if data is not D:
          if data['status']=='administrator':
           ...
           text=json.dumps(data,indent=2)+'\n'+'%s번 방에 있습니다. 32번=%s, 33번=%s'%((idx+1),r(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID)-1,r(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID))
           break
        except F as exception:
         logger.error('Exception:%s',exception)
         logger.error(traceback.format_exc())
   ...
   logger.error(traceback.format_exc())
 @z
 def super_send_message(text,encryped=x,only_last=j):
  try:
   if TelegramBot.SUPER_BOT is D:
    TelegramBot.SUPER_BOT=Bot(TelegramBot.SUPER_TOKEN)
   if encryped:
