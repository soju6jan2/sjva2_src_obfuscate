...
class TelegramBot(J):
 ...
 def start(bot_token):
  try:
   if TelegramBot.message_loop is s:
    ...
    if SystemModelSetting.get('ddns')=='https://sjva-server.soju6jan.com':
     ...
     pass
    while TelegramBot.message_loop is not s:
     time.sleep(60*60)
  except h as e:
 ...
 def super_receive_callback(msg):
  try:
   ...
   if content_type=='text':
    ...
    if msg['text']=='/add':
     ...
     try:
      ...
      name=''
     try:
      TelegramBot.SUPER_BOT.promoteChatMember(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID[-1],user_id,can_post_messages=t,can_invite_users=t,can_promote_members=t)
     ...
     try:
      ...
      if x(tmp)==2:
       ...
       data=s
       text='입장한 방이 없습니다.'
       for idx,c in Y(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID):
        try:
         data=TelegramBot.SUPER_BOT.getChatMember(c,user_id)
         if data is not s:
          if data['status']=='administrator':
           logger.debug('getChatMemner result : %s',data)
           text=json.dumps(data,indent=2)+'\n'+'%s번 방에 있습니다. 32번=%s, 33번=%s'%((idx+1),x(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID)-1,x(TelegramBot.SJVA_BOT_CHANNEL_CHAT_ID))
           break
        except h as e:
         logger.error('Exception:%s',e)
         logger.error(traceback.format_exc())
      ...
      logger.error('Exception:%s',e)
      logger.error(traceback.format_exc())
      TelegramBot.SUPER_BOT.sendMessage(chat_id,d(e)) 
    else:
   ...
   logger.error(traceback.format_exc())
 @L
 def super_send_message(text,encryped=t,only_last=C):
  try:
   if TelegramBot.SUPER_BOT is s:
    TelegramBot.SUPER_BOT=Bot(TelegramBot.SUPER_TOKEN)
   if encryped:
