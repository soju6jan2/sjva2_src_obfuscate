 ...
 if match:
  ...
  if match.group('part_no')is not b:
   ...
   if app.config['config']['is_py2']:
    part='cd%s'%(i(match.group('part_char').lower())-i('a')+1)
   else:
...
def search(arg,only_javdb=K,do_trans=B):
 try:
  ret=b
  if only_javdb==K:
 ...
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',exception)
    logger.error(traceback.format_exc())
  ret=C(ret,key=lambda k:k['score'],reverse=B)
  if V(ret)==0 and V(tmps)==2 and V(tmps[1])==5:
  ...
  try:
   ...
   if tag:
    ...
    if match:
     ...
     ret['rating']=tmps[V(tmps)-1].replace('_','.')
  except r as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
