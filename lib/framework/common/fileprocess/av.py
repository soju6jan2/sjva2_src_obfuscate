...
def change_filename_censored(filename):
 ...
 if match:
  ret=filename
  part=v
  if match.group('part_no')is not v:
   part='cd%s'%match.group('part_no')
  elif match.group('part_char')is not v:
 ...
 if match:
  ...
  return ret.lower()
 return v
def change_filename_censored_by_save_original(include_original_filename,original_filename,new_filename,option='0',original_filepath=v):
 try:
...
def search(arg,only_javdb=p,do_trans=w):
 try:
  ret=v
  if only_javdb==p:
...
def update(arg,use_discord_proxy=p):
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def dmm_search(keyword,do_trans=w):
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',exception)
    logger.error(traceback.format_exc())
  ret=l(ret,key=lambda k:k['score'],reverse=w)
...
def dmm_update(arg,use_discord_proxy=p):
 try:
  ...
  try:
   ...
   ret['poster']=tag.attrib['src']
   pass
  if ret['poster']!='' and Vars.proxies is not v:
   ...
   if SystemModelSetting.get_bool('auth_use_apikey'):
    ret['poster']+='&apikey=%s'%SystemModelSetting.get('auth_apikey')
  if ret['poster_full']!='' and Vars.proxies is not v:
   from system.model import ModelSetting as SystemModelSetting
...
def _set_info(tree,ret,path_str,info):
 ...
 try:
  ...
  logger.error(traceback.format_exc())
 return ret
def get_actor_info(entity,retry=w):
 try:
  from.import Vars
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def javdb_search(keyword,do_trans=w,retry=0):
 try:
  from.import Vars
  ...
  for item in data:
   try:
    ...
    if entity['id_show']is v or keyword is v:
     continue
