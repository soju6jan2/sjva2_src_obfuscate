...
def search(arg,only_javdb=S,do_trans=A):
 try:
  ret=T
  if only_javdb==S:
...
def update(arg,use_discord_proxy=S):
 try:
  if f(arg)<=5:
 ...
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',exception)
    logger.error(traceback.format_exc())
  ret=r(ret,key=lambda k:k['score'],reverse=A)
  if f(ret)==0 and f(tmps)==2 and f(tmps[1])==5:
  ...
  logger.error(traceback.format_exc())
  return S
def dmm_update(arg,use_discord_proxy=S):
 try:
  ...
  try:
   ...
   if tag:
    ...
    if match:
     ...
     ret['rating']=tmps[f(tmps)-1].replace('_','.')
  except K as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
...
def get_actor_info(entity,retry=A):
 try:
  ...
  if retry:
   ...
   time.sleep(1)
   return get_actor_info(entity,retry=S)
  else:
