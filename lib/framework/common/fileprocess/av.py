...
v=SystemLogicTrans.trans
EXTENSION='mp4|avi|mkv|ts|wmv|m2ts|smi|srt|ass|m4v|flv|asf|mpg|ogm'
def change_filename_censored(H):
 original_filename=H
 H=H.lower()
 H=H.replace('-h264','')
 H=H.replace('-264','')
 H=H.replace('z_1080p','').replace('z_720p','')
 H=H.replace('z_','')
 H=H.replace('-c','')
 regex=r'^(?P<code>.*?)\.1080p\.(?P<ext>%s)$'%EXTENSION
 match=e(regex).match(H)
 if match:
  H='%s.%s'%(match.group('code'),match.group('ext'))
 ...
 if match:
  ret=H
  part=d
  if match.group('part_no')is not d:
   part='cd%s'%match.group('part_no')
  elif match.group('part_char')is not d:
   if n['config']['is_py2']:
 ...
 if match:
  ...
  return ret.lower()
 regex=r'^(?P<name>[a-zA-Z]{3,})\-?(?P<no>\d+).*?\.(?P<ext>%s)$'%EXTENSION
 match=e(regex).match(original_filename)
 if match:
  ret='%s-%s.%s'%(match.group('name'),match.group('no'),match.group('ext'))
  return ret.lower()
 regex=r'(?P<name>[a-zA-Z]+)\-(?P<no>\d+).*?\.(?P<ext>%s)$'%EXTENSION
 match=e(regex).search(H)
 if match:
  ret='%s-%s.%s'%(match.group('name'),match.group('no'),match.group('ext'))
  return ret.lower()
 regex=r'(?P<name>[a-zA-Z]+)\-(?P<no>\d+).*?\.(?P<ext>%s)$'%EXTENSION
 match=e(regex).search(original_filename)
 if match:
  ret='%s-%s.%s'%(match.group('name'),match.group('no'),match.group('ext'))
  return ret.lower()
 ...
 if match:
  ...
  return ret.lower()
 return d
def change_filename_censored_by_save_original(include_original_filename,original_filename,new_filename,option='0',original_filepath=d):
 try:
  if include_original_filename:
   new_name,new_ext=L.splitext(new_filename)
   part=d
   match=i(r'(?P<part>cd\d+)$',new_name)
   if match:
    return new_filename
    part=match.group('part')
    new_name=new_name.replace(part,'')
   ori_name,ori_ext=L.splitext(original_filename)
   ori_name=ori_name.replace('[','(').replace(']',')').strip()
   if part is d:
    if option=='0':
     return '%s [%s]%s'%(new_name,ori_name,new_ext)
    elif option=='1':
     return '%s [%s(%s)]%s'%(new_name,ori_name,X(original_filepath).st_size,new_ext)
    elif option=='2':
     from framework.util import Util
     return '%s [%s(%s)]%s'%(new_name,ori_name,Util.sizeof_fmt(X(original_filepath).st_size,suffix='B'),new_ext)
    return '%s [%s]%s'%(new_name,ori_name,new_ext)
   else:
...
_session=U()
def search(arg,only_javdb=r,do_trans=b):
 try:
  ret=d
  if only_javdb==r:
  ...
  z('Exception:%s',e)
  z(O())
def update(arg,use_discord_proxy=r):
 try:
  if D(arg)<=5:
  ...
  z('Exception:%s',e)
  z(O())
def dmm_search(keyword,do_trans=b):
 try:
  ...
  if D(tmps)==2:
   ...
   title=keyword
  C('keyword %s -> %s',keyword,title)
  url='https://www.dmm.co.jp/digital/videoa/-/list/search/=/?searchstr=%s'%title
  page=_session.get(url,headers=_headers,proxies=G)
  data=page.text
  tree=P(data)
  ...
  for node in lists:
   try:
    ...
    match=e(r'\/cid=(?P<code>.*?)\/').search(href)
    if match:
    ...
    entity['title_ko']=v(entity['title'])if do_trans else entity['title']
    match=e(r'(h_)?\d*(?P<real>[a-zA-Z]+)(?P<no>\d+)([a-zA-Z]+)?$').search(entity['id'])
    if match:
    ...
    C('score :%s %s ',entity['score'],entity['id_show'])
    ret.append(entity)
   except u as e:
    z('Exception:%s',e)
    z(O())
  ret=p(ret,key=lambda k:k['score'],reverse=b)
  if D(ret)==0 and D(tmps)==2 and D(tmps[1])==5:
  ...
  z(O())
  return r
def dmm_update(arg,use_discord_proxy=r):
 try:
  ...
  if ret['poster']!='' and G is not d:
   ...
   if SystemModelSetting.get_bool('auth_use_apikey'):
    ret['poster']+='&apikey=%s'%SystemModelSetting.get('auth_apikey')
  if ret['poster_full']!='' and G is not d:
   from system.model import ModelSetting as SystemModelSetting
  ...
  match=e(r'^(?P<time>\d+)').search(tag[0].text_content())
  if match:
  ...
  for node in nodes: 
   entity={}
   match=e(r'\/id=(?P<id>.*?)\/').search(node.attrib['href'])
   if match:
  ...
  match=e(r'(h_)?\d*(?P<real>[a-zA-Z]+)(?P<no>\d+)([a-zA-Z]+)?$').match(ret['code'])
  if match:
  ...
  if tag:
   ...
   if info=='studio':
    if ret[info]in _studio:
     ...
     return ret
   ret['%s_ko'%info]=v(ret[info])
 except u as e:
  z('Exception:%s',e)
  z(O())
 return ret
def get_actor_info(entity,retry=b):
 try:
  from.import Vars
   G=Vars.proxies
  ...
  if retry:
   C(u'단시간 많은 요청으로 재요청')
   B(1)
   return get_actor_info(entity,retry=r)
  else:
   C(u'잠시후 다시 요청')
   entity['img']='xxxx'
   entity['name_kor']=''
  ...
  z('Exception:%s',e)
  z(O())
def javdb_search(keyword,do_trans=b,retry=0):
 try:
  from.import Vars
   G=Vars.proxies
  keyword=keyword.strip().replace(' ','-')
  C('javdb keyword:%s',keyword)
  keyword=_javdb_prefer_keyword(keyword)
  C('javdb prefer keyword:%s',keyword)
  C('Keyword :%s',keyword)
  ...
  for item in data:
   try:
    ...
    entity['title_ko']=v(tmp)if do_trans else tmp
    entity['poster']=item['cover_url']
    if entity['poster'].startswith('//'):
     entity['poster']='https:'+entity['poster']
    C('javdb search entity[id_show]:[%s] keyword:[%s]',entity['id_show'],keyword)
    if entity['id_show']is d or keyword is d:
     continue
...
def javdb_update(arg,retry=0):
 try:
  ...
  if D(tags)==0:
   if retry<5:
    C('JAVDB UPDATE RETRY : %s',arg)
    return javdb_update(arg,retry+1)
   else:
    C('JAVDB UPDATE CRITICAL : %s',arg)
    return
  for tag in tags:
   ...
   if label=='番號:':
    ...
    if value.replace(' ','')!='N/A':
     ret['director']=value
     ret['director_ko']=v(ret['director'])
   elif label=='片商:':
    if value.replace(' ','')!='N/A':
     ret['studio']=value
     if ret['studio']in _studio:
      ...
      ret['studio_ko']=v(ret['studio'])
   elif label=='發行:':
    if value.replace(' ','')!='N/A':
     ret['label']=value
     ret['label_ko']=v(ret['label'])
   elif label=='類別:':
  ...
  ret['release']=''
  match=e(r'(?P<real>[a-zA-Z]+)-(?P<no>\d+)').match(ret['code'])
  if match:
   ret['release']=match.group('real')
  if ret['release']=='':
...
def uncensored_filename_analyze(filename):
 try:
  ...
  if filename.find('ssdv')!=-1:
   ...
   return['ssdv',d]
  match=i('(?P<code2>n\d{4})',filename,Q)
  if match:
...
def _javdb_prefer_keyword(keyword):
 try:
  ...
  if tmp is not d:
   return tmp[1]
  match=l(r'(?P<code>\w+\.\d{2}\.\d{2}.\d{2})\.',keyword,Q)
  if match:
...
def is_uncensored(filename):
 ...
 if filename.startswith('heydouga'):
  match=i(r'(?P<code>\d{4}\-\d{3,5})',filename)
  if match:
   return['heydouga',d]
 if filename.startswith('kb'):
  return['kb',d]
