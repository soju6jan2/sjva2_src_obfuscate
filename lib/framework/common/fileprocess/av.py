...
def change_filename_censored(C):
 original_filename=C
 C=C.lower()
 C=C.replace('-h264','')
 C=C.replace('-264','')
 C=C.replace('z_1080p','').replace('z_720p','')
 C=C.replace('z_','')
 C=C.replace('-c','')
 regex=r'^(?P<code>.*?)\.1080p\.(?P<ext>%s)$'%EXTENSION
 match=re.compile(regex).match(C)
 if match:
  C='%s.%s'%(match.group('code'),match.group('ext'))
 ...
 if match:
  ret=C
  part=B
  if match.group('part_no')is not B:
   part='cd%s'%match.group('part_no')
  elif match.group('part_char')is not B:
   if app.config['config']['is_py2']:
    part='cd%s'%(v(match.group('part_char').lower())-v('a')+1)
   else:
 ...
 match=re.compile(regex).search(C)
 if match:
  ret='%s-%s.%s'%(match.group('name'),match.group('no'),match.group('ext'))
  return ret.lower()
 regex=r'(?P<name>[a-zA-Z]+)\-(?P<no>\d+).*?\.(?P<ext>%s)$'%EXTENSION
 ...
 if match:
  ...
  return ret.lower()
 return B
def change_filename_censored_by_save_original(include_original_filename,original_filename,new_filename,option='0',original_filepath=B):
 try:
...
def search(arg,only_javdb=R,do_trans=Q):
 try:
  ret=B
  if only_javdb==R:
...
def update(arg,use_discord_proxy=R):
 try:
  ...
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
def dmm_search(keyword,do_trans=Q):
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',e)
    logger.error(traceback.format_exc())
  ret=u(ret,key=lambda k:k['score'],reverse=Q)
  if G(ret)==0 and G(tmps)==2 and G(tmps[1])==5:
...
def dmm_update(arg,use_discord_proxy=R):
 try:
  ...
  try:
   ...
   ret['poster']=tag.attrib['src']
   pass
  if ret['poster']!='' and Vars.proxies is not B:
   ...
   if SystemModelSetting.get_bool('auth_use_apikey'):
    ret['poster']+='&apikey=%s'%SystemModelSetting.get('auth_apikey')
  if ret['poster_full']!='' and Vars.proxies is not B:
   from system.model import ModelSetting as SystemModelSetting
  ...
  try:
   ...
   if tag:
    ...
    if match:
     ...
     ret['rating']=tmps[G(tmps)-1].replace('_','.')
  except p as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
...
def _set_info(tree,ret,path_str,info):
 ...
 try:
  ...
  logger.error(traceback.format_exc())
 return ret
def get_actor_info(entity,retry=Q):
 try:
  from.import Vars
  ...
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
def javdb_search(keyword,do_trans=Q,retry=0):
 try:
  from.import Vars
  ...
  for item in data:
   try:
    ...
    if entity['id_show']is B or keyword is B:
     continue
