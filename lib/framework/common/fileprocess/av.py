 ...
 if match:
  ...
  if match.group('part_no')is not R:
   ...
   if app.config['config']['is_py2']:
    part='cd%s'%(p(match.group('part_char').lower())-p('a')+1)
   else:
...
def search(arg,only_javdb=c,do_trans=a):
 try:
  ret=R
  if only_javdb==c:
...
def update(arg,use_discord_proxy=c):
 try:
  if w(arg)<=5:
 ...
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',exception)
    logger.error(traceback.format_exc())
  ret=i(ret,key=lambda k:k['score'],reverse=a)
  if w(ret)==0 and w(tmps)==2 and w(tmps[1])==5:
  ...
  logger.error(traceback.format_exc())
  return c
def dmm_update(arg,use_discord_proxy=c):
 try:
  ...
  try:
   ...
   if tag:
    ...
    if match:
     ...
     ret['rating']=tmps[w(tmps)-1].replace('_','.')
  except t as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
...
def get_actor_info(entity,retry=a):
 try:
  ...
  if retry:
   ...
   time.sleep(1)
   return get_actor_info(entity,retry=c)
  else:
