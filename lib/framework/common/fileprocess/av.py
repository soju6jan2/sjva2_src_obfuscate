...
def search(arg,only_javdb=n,do_trans=y):
 try:
  ret=G
  if only_javdb==n:
...
def update(arg,use_discord_proxy=n):
 try:
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def dmm_search(keyword,do_trans=y):
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',exception)
    logger.error(traceback.format_exc())
  ret=x(ret,key=lambda k:k['score'],reverse=y)
...
def _set_info(tree,ret,path_str,info):
 ...
 try:
  ...
  logger.error(traceback.format_exc())
 return ret
def get_actor_info(entity,retry=y):
 try:
  from.import Vars
  ...
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
def javdb_search(keyword,do_trans=y,retry=0):
 try:
  from.import Vars
