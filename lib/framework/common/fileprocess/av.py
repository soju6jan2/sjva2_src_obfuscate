...
def search(arg,only_javdb=M,do_trans=x):
 try:
  ret=h
  if only_javdb==M:
 ...
 try:
  ...
  for node in lists:
   try:
    ...
    logger.error('Exception:%s',exception)
    logger.error(traceback.format_exc())
  ret=a(ret,key=lambda k:k['score'],reverse=x)
  if A(ret)==0 and A(tmps)==2 and A(tmps[1])==5:
  ...
  try:
   ...
   if tag:
    ...
    if match:
     ...
     ret['rating']=tmps[A(tmps)-1].replace('_','.')
  except n as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
