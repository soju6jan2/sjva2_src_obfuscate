...
class FfmpegQueueEntity(abc.ABCMeta('ABC',(c,),{'__slots__':()})):
 static_index=1
 entity_list=[]
...
class FfmpegQueue(c):
 ...
 def ffmpeg_listener(self,**arg):
  ...
  entity.ffmpeg_status=m(arg['status'])
  entity.ffmpeg_status_kor=D(arg['status'])
  entity.ffmpeg_percent=arg['data']['percent']
  entity.ffmpeg_arg['status']= D(arg['status'])
  entity.refresh_status()
 ...
 def command(self,cmd,entity_id):
  ...
  try:
   if cmd=='cancel':
    ...
    if self.download_queue is not e:
