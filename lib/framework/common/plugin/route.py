...
def default_route(P):
 @P.blueprint.route('/')
 def home():
  if P.ModelSetting is not s:
   tmp=P.ModelSetting.get('recent_menu_plugin')
   if tmp is not s and tmp!='':
    tmps=tmp.split('|')
...
def default_route_socketio(P,instance):
 if instance.socketio_list is s:
 ...
 def disconnect():
  try:
   ...
   P.logger.error('Exception:%s',exception)
   P.logger.error(traceback.format_exc())
 def socketio_callback(cmd,data,encoding=c):
  if instance.socketio_list:
   if encoding:
    data=json.dumps(data,cls=AlchemyEncoder)
    data=json.loads(data)
   socketio.emit(cmd,data,namespace='/{package_name}/{sub}'.format(package_name=P.package_name,sub=instance.name),broadcast=c)
