...
class Logic(l):
 ...
 def plugin_load(self):
  try:
   ...
   for module in self.P.module_list:
    module.plugin_load()
   if self.P.ModelSetting is not H:
    for module in self.P.module_list:
 ...
 def scheduler_start(self,sub):
  try:
   ...
   job=Job(self.P.package_name,job_id,module.get_scheduler_interval(),self.scheduler_function,module.get_scheduler_desc(),v,args=sub)
 ...
 def get_module(self,sub):
  try:
   ...
   self.P.logger.error('Exception:%s',exception)
   self.P.logger.error(traceback.format_exc())
 def process_telegram_data(self,data,target=H):
  try:
   for module in self.P.module_list:
    if target is H or target.startswith(module.name):
