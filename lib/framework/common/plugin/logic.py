...
class Logic(b):
 ...
 def plugin_load(self):
  try:
   ...
   for module in self.P.module_list:
    module.plugin_load()
   if self.P.ModelSetting is not c:
    for module in self.P.module_list:
 ...
 def scheduler_start(self,sub):
  try:
   ...
   job=Job(self.P.package_name,job_id,module.get_scheduler_interval(),self.scheduler_function,module.get_scheduler_desc(),B,args=sub)
 ...
 def process_telegram_data(self,data,target=c):
  try:
   for module in self.P.module_list:
    if target is c or target.startswith(module.name):
