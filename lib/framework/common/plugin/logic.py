...
class Logic(U):
 ...
 def plugin_load(self):
  try:
   ...
   for module in self.P.module_list:
    module.plugin_load()
   if self.P.ModelSetting is not X:
    for module in self.P.module_list:
 ...
 def get_module(self,sub):
  try:
   ...
   self.P.logger.error('Exception:%s',exception)
   self.P.logger.error(traceback.format_exc())
 def process_telegram_data(self,data,target=X):
  try:
   for module in self.P.module_list:
    if target is X or target.startswith(module.name):
