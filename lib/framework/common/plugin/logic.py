...
class Logic(c):
 ...
 def plugin_load(self):
  try:
   ...
   for module in self.P.module_list:
    module.plugin_load()
   if self.P.ModelSetting is not e:
    for module in self.P.module_list:
 ...
 def scheduler_start(self,sub):
  try:
   ...
   job=Job(self.P.package_name,job_id,module.get_scheduler_interval(),self.scheduler_function,module.get_scheduler_desc(),C,args=sub)
 ...
 def process_telegram_data(self,data,target=e):
  try:
   for module in self.P.module_list:
    if target is e or target.startswith(module.name):
