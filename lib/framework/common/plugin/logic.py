...
class Logic(t):
 ...
 def plugin_load(self):
  try:
   ...
   for module in self.P.module_list:
    module.plugin_load()
   if self.P.ModelSetting is not N:
    for module in self.P.module_list:
 ...
 def scheduler_start(self,sub):
  try:
   ...
   job=Job(self.P.package_name,job_id,module.get_scheduler_interval(),self.scheduler_function,module.get_scheduler_desc(),J,args=sub)
 ...
 def process_telegram_data(self,data,target=N):
  try:
   for module in self.P.module_list:
    if target is N or target.startswith(module.name):
