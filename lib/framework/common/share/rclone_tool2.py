...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=s)
class RcloneTool2(C):
 @r
 def lsjson(rclone_path,config_path,remote_path,option=H):
  try:
   ...
   if ret is not H:
    ret=P(q(ret,key=lambda k:k['Path']))
   return ret
 ...
 def purge(rclone_path,config_path,remote_path,option=H):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=s)
   logger.debug(ret)
   return ret
  except V as exception:
   logger.error('Exception:%s',exception)
 ...
 def getid(rclone_path,config_path,remote_path,option=H):
  try:
   ...
   if ret is not H and(z(ret.split(' '))>1 or ret==''):
    ret=H
   return ret
  except V as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return R
 @r
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=s,is_move=R):
  try:
   ret={'completed':R,'folderid':'','lsjson':H}
   ...
   socketio.emit("command_modal_clear",H,namespace='/framework',broadcast=s)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=s)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=s,show_modal=show_modal,clear=R).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==s:
    for i in L(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,I(f(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
