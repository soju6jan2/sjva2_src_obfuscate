...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=N)
class RcloneTool2(i):
 @E
 def lsjson(rclone_path,config_path,remote_path,option=Y):
  try:
   ...
   if ret is not Y:
    ret=H(L(ret,key=lambda k:k['Path']))
   return ret
  except z as e:
 ...
 def purge(rclone_path,config_path,remote_path,option=Y):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=N)
   logger.debug(ret)
   return ret
  except z as e:
   logger.error('Exception:%s',e)
 ...
 def getid(rclone_path,config_path,remote_path,option=Y):
  try:
   ...
   if ret is not Y and(e(ret.split(' '))>1 or ret==''):
    ret=Y
   return ret
  except z as e:
   logger.error('Exception:%s',e)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return m
 @E
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=N,is_move=m):
  try:
   ret={'completed':m,'folderid':'','lsjson':Y}
   ...
   socketio.emit("command_modal_clear",Y,namespace='/framework',broadcast=N)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=N)
   socketio.emit("command_modal_add_text",'잠시만 기다리세요.\n\n',namespace='/framework',broadcast=N)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=N,show_modal=show_modal,clear=m).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==N:
    for i in V(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,u(q(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
