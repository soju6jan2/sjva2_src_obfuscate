...
class RcloneTool2(U):
 ...
 def getid(rclone_path,config_path,remote_path,option=B):
  try:
   ...
   if ret is not B and(I(ret.split(' '))>1 or ret==''):
    ret=B
   return ret
  except w as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return E
 @c
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=r,is_move=E):
  try:
   ...
   socketio.emit("command_modal_clear",B,namespace='/framework',broadcast=r)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=r,show_modal=show_modal,clear=E).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==r:
    for i in F(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,m(f(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
