...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=D)
class RcloneTool2(P):
 @N
 def lsjson(rclone_path,config_path,remote_path,option=Y):
  try:
   ...
   if ret is not Y:
    ret=g(b(ret,key=lambda k:k['Path']))
   return ret
 ...
 def purge(rclone_path,config_path,remote_path,option=Y):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=D)
   logger.debug(ret)
   return ret
  except J as exception:
   logger.error('Exception:%s',exception)
 ...
 def getid(rclone_path,config_path,remote_path,option=Y):
  try:
   ...
   if ret is not Y and(n(ret.split(' '))>1 or ret==''):
    ret=Y
   return ret
  except J as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return m
 @N
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=D,is_move=m):
  try:
   ...
   socketio.emit("command_modal_clear",Y,namespace='/framework',broadcast=D)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=D)
   socketio.emit("command_modal_add_text",'잠시만 기다리세요.\n\n',namespace='/framework',broadcast=D)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=D,show_modal=show_modal,clear=m).start()
  ...
  try:
   ...
   if is_correct==D:
    for i in r(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,A(B(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
 ...
 def fileid_copy(rclone_path,config_path,fileid,remote_path):
  try:
   ...
   fileid=AESCipher.decrypt(B(fileid),Vars.key)
 ...
 def __make_rclone_conf(config_path,rclone_upload_remote,folder_id):
  try:
   ...
   import time
   filename='%s.conf'%(B(time.time()).split('.')[0])
 ...
 def folderid_decrypt(folderid):
  from framework.common.util import AESCipher
  folderid=AESCipher.decrypt(B(folderid),Vars.key)
  return folderid
 """
    @staticmethod
