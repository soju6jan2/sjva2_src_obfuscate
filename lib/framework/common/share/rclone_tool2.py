...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=A)
class RcloneTool2(r):
 @N
 def lsjson(rclone_path,config_path,remote_path,option=c):
  try:
   ...
   if ret is not c:
    ret=q(H(ret,key=lambda k:k['Path']))
   return ret
 ...
 def rmdir(rclone_path,config_path,remote_path,option=c):
  try:
   ...
   logger.error(traceback.format_exc())
 @N
 def purge(rclone_path,config_path,remote_path,option=c):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=A)
   logger.debug(ret)
   return ret
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @N
 def mkdir(rclone_path,config_path,remote_path,option=c):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @N
 def getid(rclone_path,config_path,remote_path,option=c):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not c and(k(ret.split(' '))>1 or ret==''):
    ret=c
   return ret
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @N
 def moveto(rclone_path,config_path,remote_path,remote_path2,option=c):
  try:
   from system.logic_command import SystemLogicCommand
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return h
 @N
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=A,is_move=h):
  try:
   ret={'completed':h,'folderid':'','lsjson':c}
   ...
   socketio.emit("command_modal_clear",c,namespace='/framework',broadcast=A)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=A)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=A,show_modal=show_modal,clear=h).start()
  ...
  try:
   ...
   try:
    ...
    for i in X(10):
     ...
     if user_size_data is not c and content_source_size_data is not c and user_size_data['bytes']==content_source_size_data['bytes']:
   ...
   if is_correct==A:
    for i in X(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,T(f(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
