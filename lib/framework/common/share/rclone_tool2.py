...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=I)
class RcloneTool2(q):
 @u
 def lsjson(rclone_path,config_path,remote_path,option=X):
  try:
   ...
   if ret is not X:
    ret=z(h(ret,key=lambda k:k['Path']))
   return ret
 ...
 def purge(rclone_path,config_path,remote_path,option=X):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=I)
   logger.debug(ret)
   return ret
  except O as exception:
   logger.error('Exception:%s',exception)
 ...
 def getid(rclone_path,config_path,remote_path,option=X):
  try:
   ...
   if ret is not X and(J(ret.split(' '))>1 or ret==''):
    ret=X
   return ret
  except O as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return B
 @u
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=I,is_move=B):
  try:
   ret={'completed':B,'folderid':'','lsjson':X}
   ...
   socketio.emit("command_modal_clear",X,namespace='/framework',broadcast=I)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=I)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=I,show_modal=show_modal,clear=B).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==I:
    for i in W(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,n(C(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
