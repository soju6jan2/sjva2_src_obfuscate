...
class RcloneTool2(j):
 ...
 def lsjson(rclone_path,config_path,remote_path,option=P):
  try:
   ...
   if ret is not P:
    ret=u(x(ret,key=lambda k:k['Path']))
   return ret
 ...
 def getid(rclone_path,config_path,remote_path,option=P):
  try:
   ...
   if ret is not P and(D(ret.split(' '))>1 or ret==''):
    ret=P
   return ret
  except G as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return l
 @c
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=o,is_move=l):
  try:
   ret={'completed':l,'folderid':'','lsjson':P}
   ...
   socketio.emit("command_modal_clear",P,namespace='/framework',broadcast=o)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=o,show_modal=show_modal,clear=l).start()
  ...
  try:
   ...
   if is_correct==o:
    for i in R(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,y(s(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
 ...
 def fileid_copy(rclone_path,config_path,fileid,remote_path):
  try:
   ...
   fileid=AESCipher.decrypt(s(fileid),Vars.key)
 ...
 def __make_rclone_conf(config_path,rclone_upload_remote,folder_id):
  try:
   ...
   import time
   filename='%s.conf'%(s(time.time()).split('.')[0])
 ...
 def folderid_decrypt(folderid):
  from framework.common.util import AESCipher
  folderid=AESCipher.decrypt(s(folderid),Vars.key)
  return folderid
 """
    @staticmethod
