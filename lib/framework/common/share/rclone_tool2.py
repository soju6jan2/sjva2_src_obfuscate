...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=z)
class RcloneTool2(C):
 @K
 def lsjson(rclone_path,config_path,remote_path,option=y):
  try:
   ...
   if ret is not y:
    ret=G(v(ret,key=lambda k:k['Path']))
   return ret
  except e as e:
 ...
 def purge(rclone_path,config_path,remote_path,option=y):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=z)
   logger.debug(ret)
   return ret
  except e as e:
   logger.error('Exception:%s',e)
 ...
 def getid(rclone_path,config_path,remote_path,option=y):
  try:
   ...
   if ret is not y and(h(ret.split(' '))>1 or ret==''):
    ret=y
   return ret
  except e as e:
   logger.error('Exception:%s',e)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return J
 @K
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=z,is_move=J):
  try:
   ret={'completed':J,'folderid':'','lsjson':y}
   ...
   socketio.emit("command_modal_clear",y,namespace='/framework',broadcast=z)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=z)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=z,show_modal=show_modal,clear=J).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==z:
    for i in P(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,f(k(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
