...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=J)
class RcloneTool2(G):
 @N
 def lsjson(rclone_path,config_path,remote_path,option=E):
  try:
   ...
   if ret is not E:
    ret=e(i(ret,key=lambda k:k['Path']))
   return ret
 ...
 def rmdir(rclone_path,config_path,remote_path,option=E):
  try:
   ...
   logger.error(traceback.format_exc())
 @N
 def purge(rclone_path,config_path,remote_path,option=E):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=J)
   logger.debug(ret)
   return ret
  except U as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @N
 def mkdir(rclone_path,config_path,remote_path,option=E):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @N
 def getid(rclone_path,config_path,remote_path,option=E):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not E and(a(ret.split(' '))>1 or ret==''):
    ret=E
   return ret
  except U as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @N
 def moveto(rclone_path,config_path,remote_path,remote_path2,option=E):
  try:
   from system.logic_command import SystemLogicCommand
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return x
 @N
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=J,is_move=x):
  try:
   ret={'completed':x,'folderid':'','lsjson':E}
   ...
   socketio.emit("command_modal_clear",E,namespace='/framework',broadcast=J)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=J)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=J,show_modal=show_modal,clear=x).start()
  ...
  try:
   ...
   try:
    ...
    for i in X(10):
     ...
     if user_size_data is not E and content_source_size_data is not E and user_size_data['bytes']==content_source_size_data['bytes']:
   ...
   if is_correct==J:
    for i in X(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,A(u(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
