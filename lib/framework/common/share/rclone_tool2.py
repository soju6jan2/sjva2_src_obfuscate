...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=f)
class RcloneTool2(j):
 @Y
 def lsjson(rclone_path,config_path,remote_path,option=I):
  try:
   ...
   if ret is not I:
    ret=a(r(ret,key=lambda k:k['Path']))
   return ret
 ...
 def rmdir(rclone_path,config_path,remote_path,option=I):
  try:
   ...
   logger.error(traceback.format_exc())
 @Y
 def purge(rclone_path,config_path,remote_path,option=I):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=f)
   logger.debug(ret)
   return ret
  except N as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @Y
 def mkdir(rclone_path,config_path,remote_path,option=I):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @Y
 def getid(rclone_path,config_path,remote_path,option=I):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not I and(D(ret.split(' '))>1 or ret==''):
    ret=I
   return ret
  except N as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @Y
 def moveto(rclone_path,config_path,remote_path,remote_path2,option=I):
  try:
   from system.logic_command import SystemLogicCommand
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return T
 @Y
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=f,is_move=T):
  try:
   ret={'completed':T,'folderid':'','lsjson':I}
   ...
   socketio.emit("command_modal_clear",I,namespace='/framework',broadcast=f)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=f)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=f,show_modal=show_modal,clear=T).start()
  ...
  try:
   ...
   try:
    ...
    for i in J(10):
     ...
     if user_size_data is not I and content_source_size_data is not I and user_size_data['bytes']==content_source_size_data['bytes']:
   ...
   if is_correct==f:
    for i in J(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,c(M(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
