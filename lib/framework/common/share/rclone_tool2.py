...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=l)
class RcloneTool2(O):
 @S
 def lsjson(rclone_path,config_path,remote_path,option=x):
  try:
   ...
   if ret is not x:
    ret=a(m(ret,key=lambda k:k['Path']))
   return ret
 ...
 def purge(rclone_path,config_path,remote_path,option=x):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=l)
   logger.debug(ret)
   return ret
  except b as exception:
   logger.error('Exception:%s',exception)
 ...
 def getid(rclone_path,config_path,remote_path,option=x):
  try:
   ...
   if ret is not x and(G(ret.split(' '))>1 or ret==''):
    ret=x
   return ret
  except b as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return w
 @S
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=l,is_move=w):
  try:
   ret={'completed':w,'folderid':'','lsjson':x}
   ...
   socketio.emit("command_modal_clear",x,namespace='/framework',broadcast=l)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=l)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=l,show_modal=show_modal,clear=w).start()
  ...
  try:
   ...
   if is_correct==l:
    for i in N(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,I(V(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
 ...
 def fileid_copy(rclone_path,config_path,fileid,remote_path):
  try:
   ...
   fileid=AESCipher.decrypt(V(fileid),Vars.key)
 ...
 def __make_rclone_conf(config_path,rclone_upload_remote,folder_id):
  try:
   ...
   import time
   filename='%s.conf'%(V(time.time()).split('.')[0])
 ...
 def folderid_decrypt(folderid):
  from framework.common.util import AESCipher
  folderid=AESCipher.decrypt(V(folderid),Vars.key)
  return folderid
 """
    @staticmethod
