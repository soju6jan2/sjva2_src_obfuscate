...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=V)
class RcloneTool2(U):
 @N
 def lsjson(rclone_path,config_path,remote_path,option=z):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not z:
    ret=X(n(ret,key=lambda k:k['Path']))
   return ret
  except d as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 @N
 def size(rclone_path,config_path,remote_path,option=z):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @N
 def rmdir(rclone_path,config_path,remote_path,option=z):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @N
 def purge(rclone_path,config_path,remote_path,option=z):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=V)
   logger.debug(ret)
   return ret
  except d as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 @N
 def mkdir(rclone_path,config_path,remote_path,option=z):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @N
 def getid(rclone_path,config_path,remote_path,option=z):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not z and(W(ret.split(' '))>1 or ret==''):
    ret=z
   return ret
  except d as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
 @N
 def moveto(rclone_path,config_path,remote_path,remote_path2,option=z):
  try:
   from system.logic_command import SystemLogicCommand
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return P
 @N
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=V,is_move=P):
  try:
   ret={'completed':P,'folderid':'','lsjson':z}
   ...
   socketio.emit("command_modal_clear",z,namespace='/framework',broadcast=V)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=V)
   socketio.emit("command_modal_add_text",'잠시만 기다리세요.\n\n',namespace='/framework',broadcast=V)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=V,show_modal=show_modal,clear=P).start()
  ...
  try:
   ...
   try:
    ...
    for i in J(10):
     ...
     if user_size_data is not z and content_source_size_data is not z and user_size_data['bytes']==content_source_size_data['bytes']:
    ...
    logger.error(traceback.format_exc())
   if is_correct==V:
    for i in J(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,b(E(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
 ...
 def fileid_copy(rclone_path,config_path,fileid,remote_path):
  try:
   ...
   fileid=AESCipher.decrypt(E(fileid),Vars.key)
 ...
 def __make_rclone_conf(config_path,rclone_upload_remote,folder_id):
  try:
   ...
   import time
   filename='%s.conf'%(E(time.time()).split('.')[0])
   ...
   while V:
    ...
    if first_rclone_info is z and dest_remote.find('access_token')!=-1:
    ...
    if match.group('remote_name')==rclone_upload_remote:
     ...
     dest_remote=z
     match=z
   if rclone_upload_remote is not z:
    if dest_remote is z:
     raise d('cannot find remote_name')
    else:
     if dest_remote.find('type = drive')==-1:
      if first_rclone_info is not z:
       src_remote_ready=first_rclone_info
      else:
       raise d('cannot find google remote_name')
     else:
      pass
 ...
 def folderid_decrypt(folderid):
  from framework.common.util import AESCipher
  folderid=AESCipher.decrypt(E(folderid),Vars.key)
  return folderid
 """
    @staticmethod
