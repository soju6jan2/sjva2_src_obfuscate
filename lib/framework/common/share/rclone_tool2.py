...
class RcloneTool2(F):
 @n
 def lsjson(rclone_path,config_path,remote_path,option=j):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not j:
    ret=m(q(ret,key=lambda k:k['Path']))
   return ret
  except I as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @n
 def size(rclone_path,config_path,remote_path,option=j):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @n
 def rmdir(rclone_path,config_path,remote_path,option=j):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @n
 def purge(rclone_path,config_path,remote_path,option=j):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @n
 def mkdir(rclone_path,config_path,remote_path,option=j):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @n
 def getid(rclone_path,config_path,remote_path,option=j):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not j and(L(ret.split(' '))>1 or ret==''):
    ret=j
   return ret
  except I as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @n
 def moveto(rclone_path,config_path,remote_path,remote_path2,option=j):
  try:
   from system.logic_command import SystemLogicCommand
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return a
 @n
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=c,is_move=a):
  try:
   ret={'completed':a,'folderid':'','lsjson':j}
   ...
   socketio.emit("command_modal_clear",j,namespace='/framework',broadcast=c)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=c,show_modal=show_modal,clear=a).start()
  ...
  try:
   ...
   try:
    ...
    for i in B(10):
     ...
     if user_size_data is not j and content_source_size_data is not j and user_size_data['bytes']==content_source_size_data['bytes']:
    ...
    logger.error(traceback.format_exc())
   if is_correct==c:
    for i in B(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,r(p(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
 ...
 def __make_rclone_conf(config_path,rclone_upload_remote,folder_id):
  try:
   ...
   while c:
    ...
    if first_rclone_info is j and dest_remote.find('access_token')!=-1:
