...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=E)
class RcloneTool2(l):
 @S
 def lsjson(rclone_path,config_path,remote_path,option=P):
  try:
   ...
   if ret is not P:
    ret=H(q(ret,key=lambda k:k['Path']))
   return ret
 ...
 def purge(rclone_path,config_path,remote_path,option=P):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=E)
   logger.debug(ret)
   return ret
  except f as exception:
   logger.error('Exception:%s',exception)
 ...
 def getid(rclone_path,config_path,remote_path,option=P):
  try:
   ...
   if ret is not P and(B(ret.split(' '))>1 or ret==''):
    ret=P
   return ret
  except f as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return W
 @S
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=E,is_move=W):
  try:
   ret={'completed':W,'folderid':'','lsjson':P}
   ...
   socketio.emit("command_modal_clear",P,namespace='/framework',broadcast=E)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=E)
   socketio.emit("command_modal_add_text",'잠시만 기다리세요.\n\n',namespace='/framework',broadcast=E)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=E,show_modal=show_modal,clear=W).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==E:
    for i in U(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,c(i(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
