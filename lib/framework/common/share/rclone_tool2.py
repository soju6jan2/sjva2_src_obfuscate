...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=R)
class RcloneTool2(N):
 @j
 def lsjson(rclone_path,config_path,remote_path,option=q):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not q:
    ret=B(A(ret,key=lambda k:k['Path']))
   return ret
  except H as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @j
 def size(rclone_path,config_path,remote_path,option=q):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @j
 def rmdir(rclone_path,config_path,remote_path,option=q):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @j
 def purge(rclone_path,config_path,remote_path,option=q):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=R)
   logger.debug(ret)
   return ret
  except H as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @j
 def mkdir(rclone_path,config_path,remote_path,option=q):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   logger.error(traceback.format_exc())
 @j
 def getid(rclone_path,config_path,remote_path,option=q):
  try:
   from system.logic_command import SystemLogicCommand
   ...
   if ret is not q and(b(ret.split(' '))>1 or ret==''):
    ret=q
   return ret
  except H as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 @j
 def moveto(rclone_path,config_path,remote_path,remote_path2,option=q):
  try:
   from system.logic_command import SystemLogicCommand
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return k
 @j
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=R,is_move=k):
  try:
   ret={'completed':k,'folderid':'','lsjson':q}
   ...
   socketio.emit("command_modal_clear",q,namespace='/framework',broadcast=R)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=R)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=R,show_modal=show_modal,clear=k).start()
  ...
  try:
   ...
   try:
    ...
    for i in t(10):
     ...
     if user_size_data is not q and content_source_size_data is not q and user_size_data['bytes']==content_source_size_data['bytes']:
   ...
   if is_correct==R:
    for i in t(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,l(F(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
 ...
 def __make_rclone_conf(config_path,rclone_upload_remote,folder_id):
  try:
   ...
   while R:
    ...
    if first_rclone_info is q and dest_remote.find('access_token')!=-1:
