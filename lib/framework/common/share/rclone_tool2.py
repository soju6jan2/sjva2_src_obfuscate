...
class RcloneTool2(t):
 ...
 def lsjson(rclone_path,config_path,remote_path,option=h):
  try:
   ...
   if ret is not h:
    ret=R(T(ret,key=lambda k:k['Path']))
   return ret
 ...
 def getid(rclone_path,config_path,remote_path,option=h):
  try:
   ...
   if ret is not h and(d(ret.split(' '))>1 or ret==''):
    ret=h
   return ret
  except I as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return E
 @x
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=v,is_move=E):
  try:
   ret={'completed':E,'folderid':'','lsjson':h}
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=v,show_modal=show_modal,clear=E).start()
  ...
  try:
   ...
   if is_correct==v:
    for i in D(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,j(C(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
