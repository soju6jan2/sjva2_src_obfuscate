...
def emit(msg):
 socketio.emit("command_modal_add_text",msg,namespace='/framework',broadcast=O)
class RcloneTool2(t):
 @X
 def lsjson(rclone_path,config_path,remote_path,option=r):
  try:
   ...
   if ret is not r:
    ret=v(y(ret,key=lambda k:k['Path']))
   return ret
 ...
 def purge(rclone_path,config_path,remote_path,option=r):
  try:
   ...
   ret=SystemLogicCommand.execute_command_return(command,force_log=O)
   logger.debug(ret)
   return ret
  except e as exception:
   logger.error('Exception:%s',exception)
 ...
 def getid(rclone_path,config_path,remote_path,option=r):
  try:
   ...
   if ret is not r and(h(ret.split(' '))>1 or ret==''):
    ret=r
   return ret
  except e as exception:
   logger.error('Exception:%s',exception)
 ...
 def can_use_relay(rclone_path,config_path,remote_path):
  ...
  return i
 @X
 def do_user_upload(rclone_path,config_path,remote_path,folder_name,upload_folderid,board_type,category_type,show_modal=O,is_move=i):
  try:
   ret={'completed':i,'folderid':'','lsjson':r}
   ...
   socketio.emit("command_modal_clear",r,namespace='/framework',broadcast=O)
   socketio.emit("command_modal_show",'업로드',namespace='/framework',broadcast=O)
   ...
   return_log=SystemLogicCommand2('업로드',[['msg','5. Rclone 명령'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=O,show_modal=show_modal,clear=i).start()
 ...
 def do_relay_copy(rclone_path,sourceid,targetid):
  try:
   ...
   try:
    ...
    logger.error(traceback.format_exc())
   if is_correct==O:
    for i in E(0,for_range):
     tmp='%s/copy_%s_%s'%(target_remote,A(F(time.time()).split('.')[0]),SystemModelSetting.get('sjva_me_user_id'))
