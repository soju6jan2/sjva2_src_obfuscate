...
class RcloneTool(y):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=N):
  try:
   ...
   logger.error(traceback.format_exc())
 @V
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=X,listener=N,dry=X,option=N,remove_config=w,show_modal=w,force_remote_name=N):
  try:
   ...
   return_folder_id=X
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is N or force_remote_name=='' else force_remote_name
   else:
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=w,show_modal=show_modal).start()
