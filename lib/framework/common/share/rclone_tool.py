...
class RcloneTool(O):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=x):
  try:
   ...
   logger.error(traceback.format_exc())
 @S
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=w,listener=x,dry=w,option=x,remove_config=l,show_modal=l,force_remote_name=x):
  try:
   ...
   if folder_id_encrypted:
    ...
    folder_id=AESCipher.decrypt(V(folder_id),Vars.key)
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=l,show_modal=show_modal).start()
    ...
    if ret['percent']==100:
     ...
     for i in N(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        from framework.common.util import AESCipher
        ret['folder_id']=AESCipher.encrypt(V(item['ID']),Vars.key)
