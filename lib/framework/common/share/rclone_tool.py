...
class RcloneTool(b):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=v):
  try:
   ...
   logger.error(traceback.format_exc())
 @L
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=w,listener=v,dry=w,option=v,remove_config=X,show_modal=X,force_remote_name=v):
  try:
   ...
   return_folder_id=w
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is v or force_remote_name=='' else force_remote_name
   else:
