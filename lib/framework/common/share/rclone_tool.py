...
class RcloneTool(u):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=U):
  try:
   ...
   logger.error(traceback.format_exc())
 @f
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=b,listener=U,dry=b,option=U,remove_config=m,show_modal=m,force_remote_name=U):
  try:
   ...
   if folder_id_encrypted:
    ...
    folder_id=AESCipher.decrypt(x(folder_id),Vars.key)
   rclone_upload_remote=remote_path.split(':')[0]
   return_folder_id=b
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is U or force_remote_name=='' else force_remote_name
   else:
    ...
    if ret['percent']==100:
     ...
     for i in M(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        from framework.common.util import AESCipher
        ret['folder_id']=AESCipher.encrypt(x(item['ID']),Vars.key)
        command=[rclone_path,'--config',config_path,'lsjson',parent_remote+'/'+item['Name'],'-R','--files-only']
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=E(ret['lsjson'],key=lambda k:k['Path'])
        break
