...
class RcloneTool(e):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=Q):
  try:
   ...
   logger.error(traceback.format_exc())
 @H
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=P,listener=Q,dry=P,option=Q,remove_config=p,show_modal=p,force_remote_name=Q):
  try:
   ...
   return_folder_id=P
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is Q or force_remote_name=='' else force_remote_name
   else:
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=p,show_modal=show_modal).start()
