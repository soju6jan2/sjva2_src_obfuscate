...
class RcloneTool(o):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=s):
  try:
   ...
   logger.error(traceback.format_exc())
 @l
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=S,listener=s,dry=S,option=s,remove_config=M,show_modal=M,force_remote_name=s):
  try:
   ...
   if folder_id_encrypted:
    ...
    folder_id=AESCipher.decrypt(A(folder_id),Vars.key)
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=M,show_modal=show_modal).start()
    ...
    if ret['percent']==100:
     ...
     for i in e(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        from framework.common.util import AESCipher
        ret['folder_id']=AESCipher.encrypt(A(item['ID']),Vars.key)
        command=[rclone_path,'--config',config_path,'lsjson',parent_remote+'/'+item['Name'],'-R','--files-only']
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=x(ret['lsjson'],key=lambda k:k['Path'])
        break
