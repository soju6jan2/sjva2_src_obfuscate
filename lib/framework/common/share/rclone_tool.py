...
class RcloneTool(t):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=r):
  try:
   ...
   logger.error(traceback.format_exc())
 @X
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=i,listener=r,dry=i,option=r,remove_config=O,show_modal=O,force_remote_name=r):
  try:
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=O,show_modal=show_modal).start()
     ...
     for i in E(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        ...
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=y(ret['lsjson'],key=lambda k:k['Path'])
        break
