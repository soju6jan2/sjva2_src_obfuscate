...
class RcloneTool(F):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=j):
  try:
   ...
   logger.error(traceback.format_exc())
 @n
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=a,listener=j,dry=a,option=j,remove_config=c,show_modal=c,force_remote_name=j):
  try:
   ...
   if folder_id_encrypted:
    ...
    folder_id=AESCipher.decrypt(p(folder_id),Vars.key)
   rclone_upload_remote=remote_path.split(':')[0]
   return_folder_id=a
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is j or force_remote_name=='' else force_remote_name
   else:
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=c,show_modal=show_modal).start()
    ...
    if ret['percent']==100:
     ...
     for i in B(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        from framework.common.util import AESCipher
        ret['folder_id']=AESCipher.encrypt(p(item['ID']),Vars.key)
        command=[rclone_path,'--config',config_path,'lsjson',parent_remote+'/'+item['Name'],'-R','--files-only']
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=q(ret['lsjson'],key=lambda k:k['Path'])
        break
