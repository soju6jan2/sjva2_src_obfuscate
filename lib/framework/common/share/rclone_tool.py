...
class RcloneTool(C):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=y):
  try:
   ...
   logger.error(traceback.format_exc())
 @K
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=J,listener=y,dry=J,option=y,remove_config=z,show_modal=z,force_remote_name=y):
  try:
   ...
   if folder_id_encrypted:
    ...
    folder_id=AESCipher.decrypt(k(folder_id),Vars.key)
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=z,show_modal=show_modal).start()
    ...
    if ret['percent']==100:
     ...
     for i in P(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        from framework.common.util import AESCipher
        ret['folder_id']=AESCipher.encrypt(k(item['ID']),Vars.key)
        command=[rclone_path,'--config',config_path,'lsjson',parent_remote+'/'+item['Name'],'-R','--files-only']
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=v(ret['lsjson'],key=lambda k:k['Path'])
        break
