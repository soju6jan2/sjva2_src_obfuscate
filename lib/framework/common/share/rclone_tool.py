...
class RcloneTool(l):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=a):
  try:
   ...
   logger.error(traceback.format_exc())
 @M
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=p,listener=a,dry=p,option=a,remove_config=u,show_modal=u,force_remote_name=a):
  try:
   ...
   if folder_id_encrypted:
    ...
    folder_id=AESCipher.decrypt(Q(folder_id),Vars.key)
   rclone_upload_remote=remote_path.split(':')[0]
   return_folder_id=p
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is a or force_remote_name=='' else force_remote_name
   else:
   ...
   return_log=SystemLogicCommand2('공유',[['msg','잠시만 기다리세요'],command,['msg','Rclone 명령을 완료하였습니다.'],],wait=u,show_modal=show_modal).start()
    ...
    if ret['percent']==100:
     ...
     for i in z(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        from framework.common.util import AESCipher
        ret['folder_id']=AESCipher.encrypt(Q(item['ID']),Vars.key)
        command=[rclone_path,'--config',config_path,'lsjson',parent_remote+'/'+item['Name'],'-R','--files-only']
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=m(ret['lsjson'],key=lambda k:k['Path'])
        break
