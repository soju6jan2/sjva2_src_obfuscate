...
class RcloneTool(X):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=O):
  try:
   ...
   logger.error(traceback.format_exc())
 @k
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=B,listener=O,dry=B,option=O,remove_config=U,show_modal=U,force_remote_name=O):
  try:
   ...
   return_folder_id=B
   if mode=='download':
    src_remote=remote_path.split(':')[0]if force_remote_name is O or force_remote_name=='' else force_remote_name
   else:
