...
class RcloneTool(N):
 ...
 def rmdir(rclone_path,config_path,remote_path,option=q):
  try:
   ...
   logger.error(traceback.format_exc())
 @j
 def do_action(rclone_path,config_path,mode,server_type,folder_id,folder_name,server_filename,remote_path,action,folder_id_encrypted=k,listener=q,dry=k,option=q,remove_config=R,show_modal=R,force_remote_name=q):
  try:
     ...
     for i in t(20):
      ...
      for item in ret['lsjson']:
       if item['Name']==tmp:
        ...
        logger.debug(command)
        ret['lsjson']=SystemLogicCommand.execute_command_return(command,format='json')
        ret['lsjson']=A(ret['lsjson'],key=lambda k:k['Path'])
        break
