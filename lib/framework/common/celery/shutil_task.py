...
def move(source_path,target_path,run_in_celery=m):
 try:
  if app.config['config']['use_celery']and run_in_celery==m:
...
def _move_task(source_path,target_path):
 try:
  ...
  logger.error(traceback.format_exc())
  return m
def move_exist_remove(source_path,target_path,run_in_celery=m):
 try:
  if app.config['config']['use_celery']and run_in_celery==m:
