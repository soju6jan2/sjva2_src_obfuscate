...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return P
 @N
 def server_process(save_list,category=i):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not i:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':u(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=i
        info['video_count']+=1
 ...
 def append(E,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%E,data={'data':json.dumps(data.as_dict())})
  except z as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
