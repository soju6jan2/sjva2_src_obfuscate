...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return z
 @O
 def server_process(save_list,category=W):
 ...
 def append(R,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%R,data={'data':json.dumps(data.as_dict())})
  except v as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
