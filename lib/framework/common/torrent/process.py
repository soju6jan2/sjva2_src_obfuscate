...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return K
 @e
 def server_process(save_list,category=t):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not t:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':v(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=t
        info['video_count']+=1
