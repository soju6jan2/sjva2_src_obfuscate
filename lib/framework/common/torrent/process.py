...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return J
 @u
 def server_process(save_list,category=z):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not z:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':X(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=z
        info['video_count']+=1
 ...
 def append(I,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%I,data={'data':json.dumps(data.as_dict())})
  except L as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
