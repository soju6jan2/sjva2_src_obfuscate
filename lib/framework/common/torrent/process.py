...
class TorrentProcess:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not a:
         ...
         f['daum']={'daum_id':C(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=a
        info['video_count']+=1
 ...
 def append(K,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%K,data={'data':json.dumps(data.as_dict())})
  except e as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
