...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return M
 @o
 def server_process(save_list,category=y):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not y:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':j(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=y
        info['video_count']+=1
