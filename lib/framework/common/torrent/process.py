...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return I
 @y
 def server_process(save_list,category=F):
 ...
 def append(q,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%q,data={'data':json.dumps(data.as_dict())})
  except B as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
