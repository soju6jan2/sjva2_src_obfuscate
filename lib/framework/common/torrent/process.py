...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return I
 @P
 def server_process(save_list,category=B):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not B:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':o(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=B
        info['video_count']+=1
 ...
 def append(V,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%V,data={'data':json.dumps(data.as_dict())})
  except p as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
