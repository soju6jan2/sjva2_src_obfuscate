...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return N
 @O
 def server_process(save_list,category=g):
 ...
 def append(T,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%T,data={'data':json.dumps(data.as_dict())})
  except q as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
