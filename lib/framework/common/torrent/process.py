...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return U
 @q
 def server_process(save_list,category=L):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not L:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':F(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=L
        info['video_count']+=1
 ...
 def append(j,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%j,data={'data':json.dumps(data.as_dict())})
  except z as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
