...
class TorrentProcess:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not Q:
         ...
         f['daum']={'daum_id':r(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=Q
        info['video_count']+=1
 ...
 def append(F,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%F,data={'data':json.dumps(data.as_dict())})
  except g as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
