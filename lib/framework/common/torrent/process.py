...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return X
 @R
 def server_process(save_list,category=r):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not r:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':y(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=r
        info['video_count']+=1
 ...
 def append(a,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%a,data={'data':json.dumps(data.as_dict())})
  except o as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
