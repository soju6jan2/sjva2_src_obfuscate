...
class TorrentProcess:
 ...
 def is_broadcast_member():
  ...
  return y
 @R
 def server_process(save_list,category=D):
     ...
     for info in item['torrent_info']:
      ...
      for f in info['files']:
       ...
       if f['type']=='video':
        ...
        if entity.daum_info is not D:
         daum=entity.daum_info.as_dict()
         f['daum']={'daum_id':b(daum['daum_id']),'poster_url':daum['poster_url'],'genre':daum['genre'],'title':daum['title'],}
        else:
         f['daum']=D
        info['video_count']+=1
 ...
 def append(M,data):
  try:
   import requests
   import json
   response=requests.post("https://sjva.me/sjva/torrent_%s.php"%M,data={'data':json.dumps(data.as_dict())})
  except m as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
