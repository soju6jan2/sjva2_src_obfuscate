...
class MovieSearch(t):
 ...
 def search_movie(movie_name,movie_year):
  try:
   ...
   split_index=-1
   is_include_kor=o
   for index,c in B(movie_name):
    if app.config['config']['is_py2']:
     if N(u'가')<=N(c)<=N(u'힣'):
      is_include_kor=S
      split_index=-1
     elif N('a')<=N(c.lower())<=N('z'):
      is_include_eng=S
      if split_index==-1:
       split_index=index
     elif N('0')<=N(c.lower())<=N('9')or N(' '):
      pass
     else:
      split_index=-1
    else:
   ...
   if kor is not C:
    ...
    for i in m(A(tmps)):
     if app.config['config']['is_py2']:
      if N(u'가')<=N(tmps[i][0])<=N(u'힣')or N('0')<=N(tmps[i][0])<=N('9'):
       pass
      else:
       index=i
       break
 ...
 def search_movie_web(movie_list,movie_name,movie_year):
  try:
   ...
   res=session.get(url,headers=headers,cookies=SystemLogicSite.get_daum_cookies())
   data=res.json()
   for index,item in B(data['items']['movie']):
    tmps=item.split('|')
    score=85-(index*5)
  ...
  try:
    ...
    if need_another_search:
     ...
     if tmp is not C:
      ...
      for tag in tag_list:
       ...
       if match:
        ...
        elif match.group(2)==movie_year and first_url is not C:
         first_url='https://search.daum.net/search?%s'%tag.attrib['href']
      ...
      if need_another_search and first_url is not C:
     ...
     if tmp is not C:
      ...
      for tag in tag_list:
       ...
       elif year==movie_year and first_url is not C:
        first_url='https://search.daum.net/search?%s'%tag.attrib['href']
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
  movie_list=O(K(H(movie_list,key=lambda k:k['score'])))
  return movie_list
 @u
