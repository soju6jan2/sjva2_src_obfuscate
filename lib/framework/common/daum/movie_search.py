...
class MovieSearch(T):
 ...
 def search_movie(movie_name,movie_year):
  try:
   ...
   split_index=-1
   is_include_kor=q
   for index,c in h(movie_name):
    if app.config['config']['is_py2']:
     if E(u'가')<=E(c)<=E(u'힣'):
      is_include_kor=M
      split_index=-1
     elif E('a')<=E(c.lower())<=E('z'):
      is_include_eng=M
      if split_index==-1:
       split_index=index
     elif E('0')<=E(c.lower())<=E('9')or E(' '):
      pass
     else:
      split_index=-1
    else:
   ...
   if kor is not V:
    ...
    for i in y(l(tmps)):
     if app.config['config']['is_py2']:
      if E(u'가')<=E(tmps[i][0])<=E(u'힣')or E('0')<=E(tmps[i][0])<=E('9'):
       pass
      else:
       index=i
       break
 ...
 def search_movie_web(movie_list,movie_name,movie_year):
  try:
   ...
   res=session.get(url,headers=headers,cookies=SystemLogicSite.get_daum_cookies())
   data=res.json()
   for index,item in h(data['items']['movie']):
    tmps=item.split('|')
    score=85-(index*5)
  ...
  try:
    ...
    if need_another_search:
     ...
     if tmp is not V:
      ...
      for tag in tag_list:
       ...
       if match:
        ...
        elif match.group(2)==movie_year and first_url is not V:
         first_url='https://search.daum.net/search?%s'%tag.attrib['href']
      ...
      if need_another_search and first_url is not V:
     ...
     if tmp is not V:
      ...
      for tag in tag_list:
       ...
       elif year==movie_year and first_url is not V:
        first_url='https://search.daum.net/search?%s'%tag.attrib['href']
   ...
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
  movie_list=Q(A(j(movie_list,key=lambda k:k['score'])))
  return movie_list
 @r
