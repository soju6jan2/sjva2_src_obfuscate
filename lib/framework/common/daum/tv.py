...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @A
 def get_daum_tv_info(search_name,daum_id=I,on_home=y):
  try:
   entity={}
      ...
      if N(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=v(F(entity['episode_list'][item.attrib['data-clip']][idx])-F(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if N(entity['episode_list']):
     entity['episode_count_one_day']=F(f(C(N(items))/N(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=W(entity['series'],key=lambda k:F(k['id']))
