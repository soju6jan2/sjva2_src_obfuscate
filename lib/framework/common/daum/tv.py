...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @F
 def get_daum_tv_info(search_name,daum_id=u,on_home=q):
  try:
   entity={}
   ...
   entity['episode_list']={}
   if C(items)>300:
    items=items[C(items)-300:]
   today=Y(datetime.now().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if C(a_tag)==1:
      ...
      if C(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=C(entity['episode_list'][item.attrib['data-clip']])-1
         _=J(Y(entity['episode_list'][item.attrib['data-clip']][idx])-Y(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
          if item.attrib['data-clip']!='' and today>=Y(item.attrib['data-clip']):
     ...
     logger.error(traceback.format_exc())
   try:
    if C(entity['episode_list']):
     entity['episode_count_one_day']=Y(x(t(C(items))/C(entity['episode_list'])))
    ...
    entity['episode_count_one_day']=1
   logger.debug('daum tv len(entity.episode_list) : %s %s %s',C(items),C(entity['episode_list']),entity['episode_count_one_day'])
   return entity 
  except B as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @F
 def get_show_info(title,no=u,date=u):
  try:
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=a(entity['series'],key=lambda k:Y(k['id']))
   logger.debug('SERIES : %s',C(entity['series']))
   entity['equal_name']=[]
   ...
   logger.debug(traceback.format_exc())
 @F
 def get_show_info_on_home_title(title,daum_id=u):
  try:
