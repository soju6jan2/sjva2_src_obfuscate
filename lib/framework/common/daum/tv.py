...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @G
 def get_daum_tv_info(search_name,daum_id=F,on_home=o):
  try:
   entity={}
      ...
      if h(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=E(T(entity['episode_list'][item.attrib['data-clip']][idx])-T(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if h(entity['episode_list']):
     entity['episode_count_one_day']=T(n(c(h(items))/h(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=B(entity['series'],key=lambda k:T(k['id']))
   logger.debug('SERIES : %s',h(entity['series']))
   entity['equal_name']=[]
