...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @h
 def get_daum_tv_info(search_name,daum_id=w,on_home=x):
  try:
   entity={}
      ...
      if m(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=f(c(entity['episode_list'][item.attrib['data-clip']][idx])-c(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if m(entity['episode_list']):
     entity['episode_count_one_day']=c(b(s(m(items))/m(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=O(entity['series'],key=lambda k:c(k['id']))
   logger.debug('SERIES : %s',m(entity['series']))
   entity['equal_name']=[]
