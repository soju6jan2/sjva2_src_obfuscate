...
class DaumTV:
 ...
 def get_daum_tv_info(search_name,daum_id=Y,on_home=X):
  try:
   ...
   today=g(datetime.now().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if B(a_tag)==1:
      ...
      if B(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=f(g(entity['episode_list'][item.attrib['data-clip']][idx])-g(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if B(entity['episode_list']):
     entity['episode_count_one_day']=g(F(D(B(items))/B(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=C(entity['series'],key=lambda k:g(k['id']))
