...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @E
 def get_daum_tv_info(search_name,daum_id=S,on_home=y):
  try:
   entity={}
      ...
      if k(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=K(b(entity['episode_list'][item.attrib['data-clip']][idx])-b(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if k(entity['episode_list']):
     entity['episode_count_one_day']=b(A(x(k(items))/k(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=O(entity['series'],key=lambda k:b(k['id']))
