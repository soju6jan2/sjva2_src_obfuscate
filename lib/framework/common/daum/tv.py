...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @f
 def get_daum_tv_info(search_name,daum_id=z,on_home=K):
  try:
   entity={}
      ...
      if T(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=e(Q(entity['episode_list'][item.attrib['data-clip']][idx])-Q(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if T(entity['episode_list']):
     entity['episode_count_one_day']=Q(o(G(T(items))/T(entity['episode_list'])))
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @f
 def get_show_info(title,no=z,date=z):
  try:
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=j(entity['series'],key=lambda k:Q(k['id']))
   ...
   logger.debug(traceback.format_exc())
 @f
 def get_show_info_on_home_title(title,daum_id=z):
  try:
