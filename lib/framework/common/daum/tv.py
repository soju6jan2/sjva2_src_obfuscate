...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @L
 def get_daum_tv_info(search_name,daum_id=k,on_home=z):
  try:
   entity={}
   ...
   today=o(datetime.now().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if g(a_tag)==1:
      ...
      if g(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=f(o(entity['episode_list'][item.attrib['data-clip']][idx])-o(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
          if item.attrib['data-clip']!='' and today>=o(item.attrib['data-clip']):
   ...
   try:
    if g(entity['episode_list']):
     entity['episode_count_one_day']=o(m(Q(g(items))/g(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=G(entity['series'],key=lambda k:o(k['id']))
