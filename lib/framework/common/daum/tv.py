...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @J
 def get_daum_tv_info(search_name,daum_id=W,on_home=v):
  try:
   entity={}
      ...
      if N(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         _=p(K(entity['episode_list'][item.attrib['data-clip']][idx])-K(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
   ...
   try:
    if N(entity['episode_list']):
     entity['episode_count_one_day']=K(j(q(N(items))/N(entity['episode_list'])))
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=T(entity['series'],key=lambda k:K(k['id']))
