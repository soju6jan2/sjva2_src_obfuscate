...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @q
 def get_daum_tv_info(search_name,daum_id=F,on_home=M):
  try:
   entity={}
   ...
   for item in items:
    try:
      ...
      if n(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=n(entity['episode_list'][item.attrib['data-clip']])-1
         _=Q(E(entity['episode_list'][item.attrib['data-clip']][idx])-E(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
     ...
     logger.error(traceback.format_exc())
   try:
    if n(entity['episode_list']):
     entity['episode_count_one_day']=E(Y(U(n(items))/n(entity['episode_list'])))
    ...
    entity['episode_count_one_day']=1
   logger.debug('daum tv len(entity.episode_list) : %s %s %s',n(items),n(entity['episode_list']),entity['episode_count_one_day'])
   return entity 
  except L as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=B(entity['series'],key=lambda k:E(k['id']))
   logger.debug('SERIES : %s',n(entity['series']))
   entity['equal_name']=[]
