...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @F
 def get_daum_tv_info(search_name,daum_id=D,on_home=L):
  try:
   entity={}
   ...
   entity['episode_list']={}
   if K(items)>300:
    items=items[K(items)-300:]
   today=w(datetime.now().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if K(a_tag)==1:
      ...
      if K(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=K(entity['episode_list'][item.attrib['data-clip']])-1
         _=y(w(entity['episode_list'][item.attrib['data-clip']][idx])-w(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
          if item.attrib['data-clip']!='' and today>=w(item.attrib['data-clip']):
     ...
     logger.error(traceback.format_exc())
   try:
    if K(entity['episode_list']):
     entity['episode_count_one_day']=w(i(o(K(items))/K(entity['episode_list'])))
    ...
    entity['episode_count_one_day']=1
   logger.debug('daum tv len(entity.episode_list) : %s %s %s',K(items),K(entity['episode_list']),entity['episode_count_one_day'])
   return entity 
  except g as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @F
 def get_show_info(title,no=D,date=D):
  try:
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=R(entity['series'],key=lambda k:w(k['id']))
   logger.debug('SERIES : %s',K(entity['series']))
   entity['equal_name']=[]
   ...
   logger.debug(traceback.format_exc())
 @F
 def get_show_info_on_home_title(title,daum_id=D):
  try:
