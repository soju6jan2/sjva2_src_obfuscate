...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @l
 def get_daum_tv_info(search_name,daum_id=F,on_home=q):
  try:
   entity={}
   ...
   entity['episode_list']={}
   if t(items)>300:
    items=items[t(items)-300:]
   today=e(datetime.now().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if t(a_tag)==1:
      ...
      if t(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=t(entity['episode_list'][item.attrib['data-clip']])-1
         _=v(e(entity['episode_list'][item.attrib['data-clip']][idx])-e(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
          if item.attrib['data-clip']!='' and today>=e(item.attrib['data-clip']):
     ...
     logger.error(traceback.format_exc())
   try:
    if t(entity['episode_list']):
     entity['episode_count_one_day']=e(u(w(t(items))/t(entity['episode_list'])))
    ...
    entity['episode_count_one_day']=1
   logger.debug('daum tv len(entity.episode_list) : %s %s %s',t(items),t(entity['episode_list']),entity['episode_count_one_day'])
   return entity 
  except E as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @l
 def get_show_info(title,no=F,date=F):
  try:
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=x(entity['series'],key=lambda k:e(k['id']))
   logger.debug('SERIES : %s',t(entity['series']))
   entity['equal_name']=[]
   ...
   logger.debug(traceback.format_exc())
 @l
 def get_show_info_on_home_title(title,daum_id=F):
  try:
