...
class DaumTV:
 ...
 def check_filename(filename):
  o('check_filename filename : %s',filename)
  try:
   ret=A
 ...
 def change_filename_continous_episode(ret):
  try:
   ...
   if match:
    o(u'합본 : %s',ret['filename'])
    ret['title']=match.group('title').strip()
    if ret['no']==-1:
     ret['no']=m(match.group('no'))
   return ret
  except x as e:
   X('Exception:%s',e)
   X(v())
 ...
 def get_html(url):
  try:
   ...
   X(v())
 @I
 def get_daum_tv_info(search_name,daum_id=A,on_home=V):
  try:
   entity={}
   search_name=DaumTV.get_search_name_from_original(search_name)
   if daum_id is not A:
    url='https://search.daum.net/search?w=tv&q=%s&irk=%s&irt=tv-program&DA=TVP'%(q(search_name.encode('utf8')),daum_id)
   else:
    url='https://search.daum.net/search?w=tv&q=%s'%(q(search_name.encode('utf8')))
   ...
   entity['episode_list']={}
   if p(items)>300:
    items=items[p(items)-300:]
   today=m(b().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if p(a_tag)==1:
      ...
      if p(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=p(entity['episode_list'][item.attrib['data-clip']])-1
         _=t(m(entity['episode_list'][item.attrib['data-clip']][idx])-m(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
     ...
     X(v())
   try:
    if p(entity['episode_list']):
     entity['episode_count_one_day']=m(O(e(p(items))/p(entity['episode_list'])))
    ...
    entity['episode_count_one_day']=1
   o('daum tv len(entity.episode_list) : %s %s %s',p(items),p(entity['episode_list']),entity['episode_count_one_day'])
   return entity 
  except x as e:
   X('Exception:%s',e)
   X(v())
 ...
 def get_search_name_from_original(search_name):
  ...
  search_name=c(r'\[.*?\]','',search_name).strip()
  channel_list=['채널 A','채널A']
  for tmp in channel_list:
   if search_name.startswith(tmp):
    search_name=search_name.replace(tmp,'').strip()
  search_name=c(r'^.{2,3}드라마','',search_name).strip()
  search_name=c(r'^.{1,3}특집','',search_name).strip()
  return search_name
 @I
 def get_show_info(title,no=A,date=A):
  try:
   title=DaumTV.get_search_name_from_original(title)
   url='https://search.daum.net/search?q=%s'%(q(title.encode('utf8')))
   data=DaumTV.get_html(url)
   root=lxml.html.fromstring(data)
 ...
 def get_show_info_on_home(root):
  try:
   ...
   o('22222get_show_info_on_home title: %s',entity['title'])
   match=M(r'q\=(?P<title>.*?)&').search(tags[tag_index].attrib['href'])
   if match:
    entity['title']=H(match.group('title'))
   ...
   if p(tags)==1:
    if tags[0].text==u'방송종료':
     ...
     entity['status']=0
   o('get_show_info_on_home status: %s',entity['status'])
   tags=root.xpath('//*[@id="tvpColl"]/div[2]/div/div[1]/div')
   entity['extra_info']=tags[0].text_content().strip()
   o('get_show_info_on_home extra_info: %s',entity['extra_info'])
   entity['studio']=''
   ...
   if p(tags)==1:
    ...
    if p(tags)==1:
     entity['studio']=tags[0].text
   o('get_show_info_on_home studio: %s',entity['studio'])
   ...
   o('get_show_info_on_home 1: %s',entity['status'])
   entity['series']=[]
   ...
   if tags:
    ...
    entity['series']=R(entity['series'],key=lambda k:m(k['id']))
   o('SERIES : %s',p(entity['series']))
   entity['equal_name']=[]
   ...
   o('Exception get_show_info_by_html : %s',e)
   o(v())
 @I
 def get_show_info_on_home_title(title,daum_id=A):
  try:
   title=title.replace(u'[종영]','')
   if daum_id is A:
    url='https://search.daum.net/search?q=%s'%(q(title.encode('utf8')))
   else:
    url='https://search.daum.net/search?q=%s&irk=%s&irt=tv-program&DA=TVP'%(q(title.encode('utf8')),daum_id)
