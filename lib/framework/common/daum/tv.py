...
class DaumTV:
 ...
 def get_html(url):
  try:
   ...
   logger.error(traceback.format_exc())
 @V
 def get_daum_tv_info(search_name,daum_id=E,on_home=T):
  try:
   entity={}
   ...
   entity['episode_list']={}
   if z(items)>300:
    items=items[z(items)-300:]
   today=X(datetime.now().strftime('%Y%m%d'))
   for item in items:
    try:
     a_tag=item.xpath('a')
     if z(a_tag)==1:
      ...
      if z(span_tag)==1:
       if item.attrib['data-clip']in entity['episode_list']:
        if entity['episode_list'][item.attrib['data-clip']][0]==span_tag[0].text.strip().replace(u'회',''):
         ...
         idx=z(entity['episode_list'][item.attrib['data-clip']])-1
         _=d(X(entity['episode_list'][item.attrib['data-clip']][idx])-X(span_tag[0].text.strip().replace(u'회','')))
         if _<=4:
          if item.attrib['data-clip']!='' and today>=X(item.attrib['data-clip']):
     ...
     logger.error(traceback.format_exc())
   try:
    if z(entity['episode_list']):
     entity['episode_count_one_day']=X(N(l(z(items))/z(entity['episode_list'])))
    ...
    entity['episode_count_one_day']=1
   logger.debug('daum tv len(entity.episode_list) : %s %s %s',z(items),z(entity['episode_list']),entity['episode_count_one_day'])
   return entity 
  except k as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def get_search_name_from_original(search_name):
  ...
  return search_name
 @V
 def get_show_info(title,no=E,date=E):
  try:
 ...
 def get_show_info_on_home(root):
  try:
   ...
   if tags:
    ...
    entity['series']=m(entity['series'],key=lambda k:X(k['id']))
   logger.debug('SERIES : %s',z(entity['series']))
   entity['equal_name']=[]
   ...
   logger.debug(traceback.format_exc())
 @V
 def get_show_info_on_home_title(title,daum_id=E):
  try:
