...
def login():
 if request.method=='POST':
  ...
  if username not in USERS:
   ...
   USERS[username].authenticated=B
  ...
  if db.session.query(system.ModelSetting).filter_by(key='use_login').first().value=='False':
   ...
   USERS[username].authenticated=B
   login_user(USERS[username],remember=B)
...
def logout():
 user=current_user
 user.authenticated=W
 json_res={'ok':B,'msg':'user <%s> logout'%user.user_id}
 logout_user()
...
def download_file(path):
 ...
 return send_from_directory('',path,as_attachment=B)
@app.route("/hls")
def hls_play():
