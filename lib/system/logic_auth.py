...
class SystemLogicAuth(V):
 ...
 def get_auth_status(retry=B):
  try:
   ...
   ret={'ret':U,'desc':'','level':0,'point':0}
   if value=='':
    ret['desc']='미인증'
   elif value=='wrong_id':
    ...
    if status is not X and status['ret']:
     ret['ret']=status['ret']
     ...
     if retry:
      ...
      return SystemLogicAuth.get_auth_status(retry=U)
     else:
 ...
 def make_auth_status(level,point):
  try:
   from framework.common.util import AESCipher
   if app.config['config']['is_py2']:
    ret=AESCipher.encrypt(o('%s_%s_%s'%(ModelSetting.get('sjva_id'),level,point)),mykey=(codecs.encode(SystemLogicAuth.get_ip().encode(),'hex')+codecs.encode(ModelSetting.get('auth_apikey').encode(),'hex')).zfill(32)[:32])
   else:
    mykey=(codecs.encode(SystemLogicAuth.get_ip().encode(),'hex').decode()+codecs.encode(ModelSetting.get('auth_apikey').encode(),'hex').decode()).zfill(32)[:32].encode()
    ret= AESCipher.encrypt(o('%s_%s_%s'%(ModelSetting.get('sjva_id'),level,point)),mykey=mykey)
   logger.debug(ret)
   return ret
  except S as e:
   logger.error('Exception:%s',e)
 ...
 def do_auth():
  try:
   ...
   if data['result']=='success':
    ...
    ret['level']=u(data['level'])
    ret['point']=u(data['point'])
    ...
    tmp=SystemLogicAuth.get_auth_status(retry=U)
    ret['ret']=tmp['ret']
    ret['msg']=tmp['desc']
   return ret
  except S as e:
