...
class SystemLogic(y):
 ...
 def init():
  try:
   ...
   item.value=B(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,B(timedelta).split('.')[0],app.config['config']['repeat'])
  except A as exception:
   info['time']=B(exception)
  return info
 @v
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(q(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return c 
  except A as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if G(interval.split(' '))==1:
    interval='%s'%(q(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",c)
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",c)
   scheduler.add_job_instance(job_instance,run=T)
  except A as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
