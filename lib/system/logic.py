...
class SystemLogic(c):
 ...
 def init():
  try:
   ...
   item.value=M(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,M(timedelta).split('.')[0],app.config['config']['repeat'])
  except H as e:
   info['time']=M(e)
  return info
 @V
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(B(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return F 
  except H as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
