...
class SystemLogic(B):
 ...
 def init():
  try:
   ...
   item.value=C(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,C(timedelta).split('.')[0],app.config['config']['repeat'])
  except l as exception:
   info['time']=C(exception)
  return info
 @R
