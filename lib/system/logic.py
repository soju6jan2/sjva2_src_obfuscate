...
class SystemLogic(h):
   ...
   for key,value in SystemLogic.db_default.items():
    if db.session.query(ModelSetting).filter_by(key=key).count()==0:
     ...
     if key=='sjva_id' or key=='auth_apikey':
      value=''.join(random.choice(string.ascii_uppercase+string.digits)for _ in O(10))
 ...
 def init():
  try:
   ...
   item.value=E(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,E(timedelta).split('.')[0],app.config['config']['repeat'])
  except d as exception:
   info['time']=E(exception)
  return info
 @B
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(m(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return F 
  except d as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if p(interval.split(' '))==1:
    interval='%s'%(m(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",F)
   scheduler.add_job_instance(job_instance,run=T)
 """    
    @staticmethod
