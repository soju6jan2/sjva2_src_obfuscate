...
class SystemLogic(h):
 ...
 def init():
  try:
   ...
   item.value=z(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,z(timedelta).split('.')[0],app.config['config']['repeat'])
  except W as exception:
   info['time']=z(exception)
  return info
 @N
 ...
 def setting_save_after():
  try:
   ...
   SystemLogicSite.get_daum_cookies(force=A)
   return A 
  except W as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   ...
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",A)
   scheduler.add_job_instance(job_instance,run=i)
 """    
    @staticmethod
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",A)
   scheduler.add_job_instance(job_instance,run=i)
  except W as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
