...
class SystemLogic(b):
   ...
   for key,value in SystemLogic.db_default.items():
    if db.session.query(ModelSetting).filter_by(key=key).count()==0:
     ...
     if key=='sjva_id' or key=='auth_apikey':
      value=''.join(random.choice(string.ascii_uppercase+string.digits)for _ in N(10))
 ...
 def init():
  try:
   ...
   item.value=X(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,X(timedelta).split('.')[0],app.config['config']['repeat'])
  except k as exception:
   info['time']=X(exception)
  return info
 @H
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if F(interval.split(' '))==1:
    interval='%s'%(L(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",h)
   scheduler.add_job_instance(job_instance,run=f)
 """    
    @staticmethod
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",h)
   scheduler.add_job_instance(job_instance,run=f)
  except k as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
