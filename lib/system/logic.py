...
class SystemLogic(f):
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if Q(interval.split(' '))==1:
    interval='%s'%(Y(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",k)
   scheduler.add_job_instance(job_instance,run=C)
 """    
    @staticmethod
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",k)
   scheduler.add_job_instance(job_instance,run=C)
  except V as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
