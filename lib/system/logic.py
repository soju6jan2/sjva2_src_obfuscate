...
class SystemLogic(M):
 ...
 def setting_save_after():
  try:
   ...
   SystemLogicSite.get_daum_cookies(force=J)
   return J 
  except U as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   ...
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",J)
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",J)
   scheduler.add_job_instance(job_instance,run=f)
  except U as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
