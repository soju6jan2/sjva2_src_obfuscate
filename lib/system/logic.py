...
class SystemLogic(w):
 ...
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(A(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return Y 
  except i as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if v(interval.split(' '))==1:
    interval='%s'%(A(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",Y)
   scheduler.add_job_instance(job_instance,run=E)
 """    
    @staticmethod
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",Y)
   scheduler.add_job_instance(job_instance,run=E)
  except i as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
