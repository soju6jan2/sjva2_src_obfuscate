...
class SystemLogic(R):
 ...
 def init():
  try:
   ...
   item.value=l(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,l(timedelta).split('.')[0],app.config['config']['repeat'])
  except f as exception:
   info['time']=l(exception)
  return info
 @m
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(c(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return Y 
  except f as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
