...
class SystemLogic(q):
 ...
 def init():
  try:
   ...
   item.value=S(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,S(timedelta).split('.')[0],app.config['config']['repeat'])
  except y as exception:
   info['time']=S(exception)
  return info
 @F
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   ...
   scheduler.add_job_instance(job_instance,run=s)
 """    
    @staticmethod
