...
class SystemLogic(E):
 ...
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(w(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return r 
  except A as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def setting_save_after():
  try:
   ...
   SystemLogicSite.get_daum_cookies(force=r)
   return r 
  except A as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if W(interval.split(' '))==1:
    interval='%s'%(w(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",r)
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",r)
   scheduler.add_job_instance(job_instance,run=f)
  except A as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
