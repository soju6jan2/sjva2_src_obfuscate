...
class SystemLogic(v):
 ...
 def init():
  try:
   ...
   item.value=K(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,K(timedelta).split('.')[0],app.config['config']['repeat'])
  except V as e:
   info['time']=K(e)
  return info
 @j
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",i)
   scheduler.add_job_instance(job_instance,run=b)
  except V as e:
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
