...
class SystemLogic(a):
 ...
 def init():
  try:
   ...
   item.value=y(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,y(timedelta).split('.')[0],app.config['config']['repeat'])
  except B as exception:
   info['time']=y(exception)
  return info
 @q
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(E(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return t 
  except B as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def setting_save_after():
  try:
   ...
   SystemLogicSite.get_daum_cookies(force=t)
   return t 
  except B as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   ...
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",t)
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",t)
   scheduler.add_job_instance(job_instance,run=C)
  except B as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
