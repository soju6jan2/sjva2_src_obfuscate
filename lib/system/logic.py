...
class SystemLogic(p):
 ...
 def init():
  try:
   ...
   item.value=r(app.config['config']['repeat'])
   db.session.commit()
 ...
 def get_info():
  ...
  try:
   ...
   info['time']=u'시작 : %s   경과 : %s   재시작 : %s'%(tmp,r(timedelta).split('.')[0],app.config['config']['repeat'])
  except P as exception:
   info['time']=r(exception)
  return info
 @H
 def setting_save_system(req):
  try:
   ...
   SystemLogic.set_restart_scheduler()
   set_level(L(db.session.query(ModelSetting).filter_by(key='log_level').first().value))
   return G 
  except P as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
 ...
 def set_restart_scheduler():
  ...
  if interval!='0':
   if K(interval.split(' '))==1:
    interval='%s'%(L(interval)*60)
   job_instance=Job(package_name,name,interval,SystemLogic.restart,u"자동 재시작",G)
   scheduler.add_job_instance(job_instance,run=V)
 """    
    @staticmethod
 ...
 def set_scheduler_check_scheduler():
  try:
   ...
   if scheduler.is_include(name):
    scheduler.remove_job(name)
   job_instance=Job(package_name,name,2,scheduler.first_run_check_thread_function,u"Scheduler Check",G)
   scheduler.add_job_instance(job_instance,run=V)
  except P as exception:
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
