...
class SystemLogicSelenium(h):
 ...
 @N
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=A):
  try:
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=f
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=i)
 @N
 def get_driver(chrome_options=f):
  try:
   if SystemLogicSelenium.chrome_driver is f:
   ...
   logger.error(traceback.format_exc())
 @N
 def create_driver(chrome_options=f):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not f:
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=d([img_frag.size[1]for img_frag in img_li])
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @N
 def __get_downloaded_files(driver=f):
  if driver is f:
   driver=SystemLogicSelenium.get_driver()
 ...
 @N
 def get_file_content(path,driver=f):
  if driver is f:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise W("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @N
 def get_downloaded_files(driver=f):
  if driver is f:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @N
 def waitUntilDownloadCompleted(maxTime=600,driver=f):
  if driver is f:
   driver=SystemLogicSelenium.get_driver()
