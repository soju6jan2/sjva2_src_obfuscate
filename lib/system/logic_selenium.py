...
class SystemLogicSelenium(M):
 ...
 @c
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=J):
  try:
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=N
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=f)
 @c
 def get_driver(chrome_options=N):
  try:
   if SystemLogicSelenium.chrome_driver is N:
   ...
   logger.error(traceback.format_exc())
 @c
 def create_driver(chrome_options=N):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not N:
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @c
 def __get_downloaded_files(driver=N):
  if driver is N:
   driver=SystemLogicSelenium.get_driver()
 ...
 @c
 def get_file_content(path,driver=N):
  if driver is N:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise U("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @c
 def get_downloaded_files(driver=N):
  if driver is N:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @c
 def waitUntilDownloadCompleted(maxTime=600,driver=N):
  if driver is N:
   driver=SystemLogicSelenium.get_driver()
