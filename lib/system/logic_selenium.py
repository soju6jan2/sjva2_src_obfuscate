...
class SystemLogicSelenium(z):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=f):
  try:
   ...
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=b)
 @H
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=Y([img_frag.size[1]for img_frag in img_li])
