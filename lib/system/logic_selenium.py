...
class SystemLogicSelenium(C):
 ...
 @D
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=p):
  try:
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=M
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=W)
 @D
 def get_driver(chrome_options=M):
  try:
   if SystemLogicSelenium.chrome_driver is M:
   ...
   logger.error(traceback.format_exc())
 @D
 def create_driver(chrome_options=M):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not M:
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=L([img_frag.size[1]for img_frag in img_li])
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @D
 def __get_downloaded_files(driver=M):
  if driver is M:
   driver=SystemLogicSelenium.get_driver()
 ...
 @D
 def get_file_content(path,driver=M):
  if driver is M:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise x("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @D
 def get_downloaded_files(driver=M):
  if driver is M:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @D
 def waitUntilDownloadCompleted(maxTime=600,driver=M):
  if driver is M:
   driver=SystemLogicSelenium.get_driver()
