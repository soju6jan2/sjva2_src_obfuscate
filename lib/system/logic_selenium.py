...
class SystemLogicSelenium(w):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=Y):
  try:
   ...
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=E)
 @y
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=K([img_frag.size[1]for img_frag in img_li])
 ...
 def get_file_content(path,driver=f):
  ...
  if not result.startswith('data:'):
   raise i("Failed to get file content: %s"%result)
