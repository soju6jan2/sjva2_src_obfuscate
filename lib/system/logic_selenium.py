...
class SystemLogicSelenium(s):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=h):
  try:
   ...
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=z
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=x)
 @J
 def get_driver(chrome_options=z):
  try:
   if SystemLogicSelenium.chrome_driver is z:
   ...
   logger.error(traceback.format_exc())
 @J
 def create_driver(chrome_options=z):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not z:
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @J
 def __get_downloaded_files(driver=z):
  if driver is z:
   driver=SystemLogicSelenium.get_driver()
 ...
 @J
 def get_file_content(path,driver=z):
  if driver is z:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise Q("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @J
 def get_downloaded_files(driver=z):
  if driver is z:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @J
 def waitUntilDownloadCompleted(maxTime=600,driver=z):
  if driver is z:
   driver=SystemLogicSelenium.get_driver()
