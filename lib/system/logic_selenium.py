...
class SystemLogicSelenium(y):
 ...
 @v
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=c):
  try:
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=w
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=T)
 @v
 def get_driver(chrome_options=w):
  try:
   if SystemLogicSelenium.chrome_driver is w:
   ...
   logger.error(traceback.format_exc())
 @v
 def create_driver(chrome_options=w):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not w:
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=S([img_frag.size[1]for img_frag in img_li])
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @v
 def __get_downloaded_files(driver=w):
  if driver is w:
   driver=SystemLogicSelenium.get_driver()
 ...
 @v
 def get_file_content(path,driver=w):
  if driver is w:
   driver=SystemLogicSelenium.get_driver()
 ...
 @v
 def get_downloaded_files(driver=w):
  if driver is w:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @v
 def waitUntilDownloadCompleted(maxTime=600,driver=w):
  if driver is w:
   driver=SystemLogicSelenium.get_driver()
