...
class SystemLogicSelenium(P):
 ...
 @e
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=W):
  try:
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=s
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=U)
 @e
 def get_driver(chrome_options=s):
  try:
   if SystemLogicSelenium.chrome_driver is s:
   ...
   logger.error(traceback.format_exc())
 @e
 def create_driver(chrome_options=s):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not s:
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @e
 def __get_downloaded_files(driver=s):
  if driver is s:
   driver=SystemLogicSelenium.get_driver()
 ...
 @e
 def get_file_content(path,driver=s):
  if driver is s:
   driver=SystemLogicSelenium.get_driver()
 ...
 @e
 def get_downloaded_files(driver=s):
  if driver is s:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @e
 def waitUntilDownloadCompleted(maxTime=600,driver=s):
  if driver is s:
   driver=SystemLogicSelenium.get_driver()
