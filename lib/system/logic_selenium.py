...
class SystemLogicSelenium(w):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=Q):
  try:
   ...
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=a)
 @K
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=f([img_frag.size[1]for img_frag in img_li])
 ...
 def get_file_content(path,driver=m):
  ...
  if not result.startswith('data:'):
   raise A("Failed to get file content: %s"%result)
