...
class SystemLogicSelenium(j):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=i):
  try:
   ...
   logger.error('Exception:%s',exception)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=E
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=L)
 @x
 def get_driver(chrome_options=E):
  try:
   if SystemLogicSelenium.chrome_driver is E:
   ...
   logger.error(traceback.format_exc())
 @x
 def create_driver(chrome_options=E):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not E:
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @x
 def __get_downloaded_files(driver=E):
  if driver is E:
   driver=SystemLogicSelenium.get_driver()
 ...
 @x
 def get_file_content(path,driver=E):
  if driver is E:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise l("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @x
 def get_downloaded_files(driver=E):
  if driver is E:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @x
 def waitUntilDownloadCompleted(maxTime=600,driver=E):
  if driver is E:
   driver=SystemLogicSelenium.get_driver()
