...
class SystemLogicSelenium(V):
 ...
 @s
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=B):
  try:
   ...
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=X
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=U)
 @s
 def get_driver(chrome_options=X):
  try:
   if SystemLogicSelenium.chrome_driver is X:
   ...
   logger.error(traceback.format_exc())
 @s
 def create_driver(chrome_options=X):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not X:
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @s
 def __get_downloaded_files(driver=X):
  if driver is X:
   driver=SystemLogicSelenium.get_driver()
 ...
 @s
 def get_file_content(path,driver=X):
  if driver is X:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise S("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @s
 def get_downloaded_files(driver=X):
  if driver is X:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @s
 def waitUntilDownloadCompleted(maxTime=600,driver=X):
  if driver is X:
   driver=SystemLogicSelenium.get_driver()
