...
class SystemLogicSelenium(n):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=P):
  try:
   ...
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=t
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=O)
 @X
 def get_driver(chrome_options=t):
  try:
   if SystemLogicSelenium.chrome_driver is t:
   ...
   logger.error(traceback.format_exc())
 @X
 def create_driver(chrome_options=t):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not t:
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @X
 def __get_downloaded_files(driver=t):
  if driver is t:
   driver=SystemLogicSelenium.get_driver()
 ...
 @X
 def get_file_content(path,driver=t):
  if driver is t:
   driver=SystemLogicSelenium.get_driver()
  ...
  if not result.startswith('data:'):
   raise H("Failed to get file content: %s"%result)
  return base64.b64decode(result[result.find('base64,')+7:])
 @X
 def get_downloaded_files(driver=t):
  if driver is t:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @X
 def waitUntilDownloadCompleted(maxTime=600,driver=t):
  if driver is t:
   driver=SystemLogicSelenium.get_driver()
