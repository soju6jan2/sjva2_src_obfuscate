...
class SystemLogicSelenium(b):
 ...
 @H
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=h):
  try:
   ...
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=f)
 @H
 ...
 def get_file_content(path,driver=M):
  ...
  if not result.startswith('data:'):
   raise k("Failed to get file content: %s"%result)
