...
class SystemLogicSelenium(v):
 ...
 def get_pagesoruce_by_selenium(url,wait_xpath,retry=i):
  try:
   ...
   logger.error('Exception:%s',e)
   logger.error(traceback.format_exc())
   SystemLogicSelenium.chrome_driver=D
   if retry:
    return SystemLogicSelenium.get_pagesoruce_by_selenium(url,wait_xpath,retry=b)
 @j
 def get_driver(chrome_options=D):
  try:
   if SystemLogicSelenium.chrome_driver is D:
   ...
   logger.error(traceback.format_exc())
 @j
 def create_driver(chrome_options=D):
  try:
 ...
 def close_driver():
  try:
   if SystemLogicSelenium.chrome_driver is not D:
 ...
 def full_screenshot(driver,low_offset=0):
  try:
   ...
   img_frame_height=N([img_frag.size[1]for img_frag in img_li])
 ...
 def remove_element(driver,element):
  driver.execute_script("""
        ...
        """  , element)
 @j
 def __get_downloaded_files(driver=D):
  if driver is D:
   driver=SystemLogicSelenium.get_driver()
 ...
 @j
 def get_file_content(path,driver=D):
  if driver is D:
   driver=SystemLogicSelenium.get_driver()
 ...
 @j
 def get_downloaded_files(driver=D):
  if driver is D:
   driver=SystemLogicSelenium.get_driver()
  files=SystemLogicSelenium.__get_downloaded_files()
  return files
 @j
 def waitUntilDownloadCompleted(maxTime=600,driver=D):
  if driver is D:
   driver=SystemLogicSelenium.get_driver()
