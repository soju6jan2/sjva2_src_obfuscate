...
@login_required
def home():
 return render_template('info.html',arg=O)
@blueprint.route('/<sub>',methods=['GET','POST'])
@login_required
def first_menu(sub):
 arg=O
 if sub=='home':
  return render_template('%s_%s.html'%(package_name,sub),arg=O)
 elif sub=='setting':
 ...
 try:
  if sub=='setting':
   ...
   if sub2=='basic':
    ...
    arg['scheduler']=u(scheduler.is_include('%s_%s'%(package_name,sub2)))
    arg['is_running']=u(scheduler.is_running('%s_%s'%(package_name,sub2)))
    return render_template('%s_%s_%s.html'%(package_name,sub,sub2),arg=arg)
   elif sub2=='env':
