...
@login_required
def home():
 return render_template('info.html',arg=L)
@blueprint.route('/<sub>',methods=['GET','POST'])
@login_required
def first_menu(sub):
 arg=L
 if sub=='home':
  return render_template('%s_%s.html'%(package_name,sub),arg=L)
 elif sub=='setting':
 ...
 try:
  if sub=='setting':
   ...
   if sub2=='basic':
    ...
    arg['scheduler']=y(scheduler.is_include('%s_%s'%(package_name,sub2)))
    arg['is_running']=y(scheduler.is_running('%s_%s'%(package_name,sub2)))
    return render_template('%s_%s_%s.html'%(package_name,sub,sub2),arg=arg)
   elif sub2=='env':
...
def connect_system_restart():
 try:
  socketio.emit("on_connect",'restart',namespace='/system_restart',broadcast=t)
 except B as exception:
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
...
class InfoThread(threading.Thread):
 ...
 def run(self):
  while not self.stop_flag:
   ...
   ret['system']=SystemLogic.get_info()
   ret['scheduler']=scheduler.get_job_list_info()
   socketio.emit("status",ret,namespace='/system',broadcast=t)
   time.sleep(1)
