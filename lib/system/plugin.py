 ...
 try:
  if sub=='setting':
   ...
   if sub2=='basic':
    ...
    arg['scheduler']=I(scheduler.is_include('%s_%s'%(package_name,sub2)))
    arg['is_running']=I(scheduler.is_running('%s_%s'%(package_name,sub2)))
    return render_template('%s_%s_%s.html'%(package_name,sub,sub2),arg=arg)
   elif sub2=='env':
...
def connect_system_restart():
 try:
  socketio.emit("on_connect",'restart',namespace='/system_restart',broadcast=s)
 except G as e:
  logger.error('Exception:%s',e)
  logger.error(traceback.format_exc())
...
class InfoThread(threading.Thread):
 ...
 def run(self):
  while not self.stop_flag:
   ...
   ret['system']=SystemLogic.get_info()
   ret['scheduler']=scheduler.get_job_list_info()
   socketio.emit("status",ret,namespace='/system',broadcast=s)
   time.sleep(1)
