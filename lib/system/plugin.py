...
@login_required
def home():
 return render_template('info.html',arg=N)
@blueprint.route('/<sub>',methods=['GET','POST'])
@login_required
def first_menu(sub):
 arg=N
 if sub=='home':
  return render_template('%s_%s.html'%(package_name,sub),arg=N)
 elif sub=='setting':
...
def connect_system_restart():
 try:
  socketio.emit("on_connect",'restart',namespace='/system_restart',broadcast=J)
 except U as exception:
  logger.error('Exception:%s',exception)
  logger.error(traceback.format_exc())
...
class InfoThread(threading.Thread):
 ...
 def run(self):
  while not self.stop_flag:
   ...
   ret['system']=SystemLogic.get_info()
   ret['scheduler']=scheduler.get_job_list_info()
   socketio.emit("status",ret,namespace='/system',broadcast=J)
   time.sleep(1)
