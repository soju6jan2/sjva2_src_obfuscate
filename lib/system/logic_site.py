...
class SystemLogicSite(n):
 ...
 def process_ajax(sub,req):
  try:
   ...
   ret['ret']=O
   ret['log']=D(traceback.format_exc())
  return jsonify(ret)
 @X
 ...
 @X
 def scheduler_start():
  job=Job(package_name,'%s_site'%package_name,ModelSetting.get('site_daum_interval'),SystemLogicSite.scheduler_function,u"Daum cookie refresh",O)
 ...
 def scheduler_function():
  try:
   ...
   logger.error(traceback.format_exc())
 @X 
 def get_daum_cookie_by_selenium(notify=O):
  try:
   ret={}
   ret['ret']=O
   ...
   if notify:
    ...
    socketio.emit("notify",data,namespace='/framework',broadcast=P) 
  ...
  return ret
 @X
 def get_daum_cookies(force=O):
  try:
   if SystemLogicSite.daum_cookie is t or force:
    ret={}
