...
class SystemLogicSite(z):
 ...
 def process_ajax(sub,req):
  try:
   ...
   if sub=='site_daum_test':
    ...
    ret=SystemLogicSite.get_daum_cookie_by_selenium(notify=f)
    return jsonify(ret)
   elif sub=='scheduler':
 ...
 @H
 def scheduler_start():
  job=Job(package_name,'%s_site'%package_name,ModelSetting.get('site_daum_interval'),SystemLogicSite.scheduler_function,u"Daum cookie refresh",b)
 ...
 def scheduler_function():
  try:
   ...
   logger.error(traceback.format_exc())
 @H 
 def get_daum_cookie_by_selenium(notify=b):
  try:
   ret={}
   ret['ret']=b
   ...
   if notify:
    ...
    socketio.emit("notify",data,namespace='/framework',broadcast=f) 
