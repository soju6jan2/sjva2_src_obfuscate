...
class SystemLogicSite(r):
 ...
 def process_ajax(sub,req):
  try:
   ...
   ret['ret']=a
   ret['log']=I(traceback.format_exc())
  return jsonify(ret)
 @q
 ...
 @q
 def scheduler_start():
  job=Job(package_name,'%s_site'%package_name,ModelSetting.get('site_daum_interval'),SystemLogicSite.scheduler_function,u"Daum cookie refresh",a)
 ...
 def scheduler_function():
  try:
   ...
   logger.error(traceback.format_exc())
 @q 
 def get_daum_cookie_by_selenium(notify=a):
  try:
   ret={}
   ret['ret']=a
   ...
   if notify:
    ...
    socketio.emit("notify",data,namespace='/framework',broadcast=s) 
  ...
  return ret
 @q
 def get_daum_cookies(force=a):
  try:
   if SystemLogicSite.daum_cookie is y or force:
    ret={}
