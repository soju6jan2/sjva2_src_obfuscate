...
class SystemLogicSite(h):
 ...
 def process_ajax(sub,req):
  try:
   ...
   if sub=='site_daum_test':
    ...
    ret=SystemLogicSite.get_daum_cookie_by_selenium(notify=A)
    return jsonify(ret)
   elif sub=='scheduler':
   ...
   ret['ret']=i
   ret['log']=z(traceback.format_exc())
  return jsonify(ret)
 @N
 ...
 @N
 def scheduler_start():
  job=Job(package_name,'%s_site'%package_name,ModelSetting.get('site_daum_interval'),SystemLogicSite.scheduler_function,u"Daum cookie refresh",i)
 ...
 def scheduler_function():
  try:
   ...
   logger.error(traceback.format_exc())
 @N 
 def get_daum_cookie_by_selenium(notify=i):
  try:
   ret={}
   ret['ret']=i
   ...
   if notify:
    ...
    socketio.emit("notify",data,namespace='/framework',broadcast=A) 
